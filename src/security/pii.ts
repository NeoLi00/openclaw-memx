...
const TOKEN_REGEX =
  /\b(?:sk-[a-z0-9][a-z0-9_-]{7,}|gh[pus]_[a-z0-9_-]{12,}|xox[baprs]-[a-z0-9-]{10,}|Bearer\s+[a-z0-9._-]{12,})\b/i;
const SECRET_ASSIGNMENT_REGEX =
  /\b(?:password|passwd|api[_-]?key|access[_-]?token|secret)\s*[:=]\s*\S+/i;
const SECRET_VALUE_CONTEXT_REGEX =
  /\b(?:token|api[_-]?key|access[_-]?token|secret|credential)\b.{0,12}\b[a-z0-9._-]{10,}\b/i;

export function redactSensitiveText(text: string, piiMode: MemoryPiiMode): string {
  if (piiMode === "allow") {
    return text;
  }
  if (piiMode === "off") {
  ...
  return text
    ...
    .replace(
      /\b(?:sk-[a-z0-9][a-z0-9_-]{7,}|gh[pus]_[a-z0-9_-]{12,}|xox[baprs]-[a-z0-9-]{10,}|Bearer\s+[a-z0-9._-]{12,})\b/gi,
      "[redacted-token]",
    )
