...
const BIDIRECTIONAL_GRAPH_RELATION_TYPES = new Set<GraphTraversalRelationType>([
  "contradicts",
  "related_to",
  "targets",
  "resolved_by",
...
function graphPathRecencyScore(updatedAt: string | undefined, now: string): number {
  if (!updatedAt) {
    return 0.42;
  }
...
function graphPathRelationFit(relType: GraphTraversalRelationType): number {
  switch (relType) {
    case "resolved_by":
      return 0.96;
    case "caused_by":
...
function graphPathLengthPenalty(edgeCount: number): number {
  if (edgeCount <= 1) {
    return 0.05;
  }
  if (edgeCount === 2) {
...
function graphPathContradictionPenalty(edges: GraphEvidenceEdge[]): number {
...
function buildGraphPathCandidate(params: {
  ...
  const seedNodeIds =
    params.prioritySeedNodeIds && params.prioritySeedNodeIds.length > 0
      ? params.prioritySeedNodeIds
      : params.seedNodeIds;
  const entityMatch = clamp01(
    uniqueNodeIds.filter((nodeId) => seedNodeIds.includes(nodeId)).length /
      Math.max(1, uniqueNodeIds.length),
  );
  const edgeConfidence =
    params.edges.reduce((sum, edge) => sum + edge.confidence, 0) / Math.max(1, params.edges.length);
  const recency =
    params.edges.reduce((sum, edge) => sum + graphPathRecencyScore(edge.updatedAt, params.now), 0) /
    Math.max(1, params.edges.length);
  const supportDiversity =
    new Set(params.edges.map((edge) => edge.evidenceRef).filter(Boolean)).size /
    Math.max(1, params.edges.length);
  const relationFit =
    params.edges.reduce((sum, edge) => sum + graphPathRelationFit(edge.relType), 0) /
    Math.max(1, params.edges.length);
  ...
  const nonEntityNodeRatio =
    uniqueNodeIds.filter((nodeId) => params.nodes.get(nodeId)?.nodeKind !== "entity").length /
    Math.max(1, uniqueNodeIds.length);
  const structuralRelationRatio =
    params.edges.filter((edge) => GRAPH_STRUCTURAL_RELATION_TYPES.has(edge.relType)).length /
    Math.max(1, params.edges.length);
  const targetEdgeRatio =
    params.edges.filter((edge) => edge.relType === "targets").length /
    Math.max(1, params.edges.length);
  ...
  const targetBridgePenalty = clamp01(
    targetEdgeRatio *
...
export function buildGraphPathCandidates(params: {
  ...
  const adjacency = new Map<string, Array<{ edge: GraphEvidenceEdge; nextNodeId: string }>>();
  ...
  const candidates = new Map<string, GraphPathCandidate>();
  const addCandidate = (nodeIds: string[], edges: GraphEvidenceEdge[]) => {
    if (edges.length === 0) {
      return;
    }
  ...
  const walk = (
    ...
    for (const entry of adjacency.get(currentNodeId) ?? []) {
      ...
      walk(
        ...
        nextNodeIds,
        nextEdges,
        new Set([...visitedEdgeIds, edge.edgeId]),
        depth + 1,
      );
    }
  };
