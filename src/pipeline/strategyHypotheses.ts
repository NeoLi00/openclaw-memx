...
function phaseWeight(phase: string | undefined): number {
  switch (phase) {
    case "resolved":
      return 1;
    case "validated":
...
function shouldConsiderTaskBelief(belief: MemoryBeliefRecord, task: ConversationTask): boolean {
  const metadata = task.metadataJson ?? {};
  const hasOutcome = Boolean(
    truthyString(metadata.lastEmittedOutcomeKey) ||
    truthyString(metadata.lastPromotedOutcomeHypothesisId),
  );
  const promotionAvg =
    typeof belief.metadataJson.promotionAvg === "number" ? belief.metadataJson.promotionAvg : 0;
  return (
    belief.memoryKind === "task" &&
...
function resolveStrategyDomainEntityId(
...
function buildTaskStrategyCandidate(
  store: MemxStoreBundle,
  ctx: MemoryOperationContext,
  belief: MemoryBeliefRecord,
): TaskStrategyCandidate | null {
...
function structuredDomainParts(candidate: TaskStrategyCandidate): string[] {
...
function structuredBucketKey(candidate: TaskStrategyCandidate): string {
  if (candidate.canonicalEntityId) {
    return `${candidate.task.scope}:entity:${candidate.canonicalEntityId}`;
  }
  const primary =
    structuredDomainParts(candidate)[0] ??
...
function embeddingSimilarity(
  left: TaskStrategyCandidate,
  right: TaskStrategyCandidate,
  embeddingByTaskId?: Map<string, number[]>,
): number | null {
...
function clusterCandidates(
  candidates: TaskStrategyCandidate[],
  embeddingByTaskId?: Map<string, number[]>,
): TaskStrategyCandidate[][] {
  ...
  const clusters: TaskStrategyCandidate[][] = [];
  while (remaining.length > 0) {
    const anchor = remaining.shift()!;
    const cluster = [anchor];
    const leftovers: TaskStrategyCandidate[] = [];
    for (const candidate of remaining) {
      ...
      const sameStructuredResolution =
        normalizeText(anchor.resolutionText) === normalizeText(candidate.resolutionText);
      const matches =
        embeddingScore !== null
...
function selectDomainLabel(cluster: TaskStrategyCandidate[]): string {
  ...
  for (const candidate of cluster) {
    const terms = tokenizeSearchTerms(
      `${candidate.domainText} ${candidate.resolutionText}`,
      STRATEGY_STOPWORDS,
    );
...
function selectStrategyAnchorCandidate(cluster: TaskStrategyCandidate[]): TaskStrategyCandidate {
...
function stableStrategyKeyPart(text: string, fallback: string, maxTerms: number): string {
...
function stableStrategyDomainKey(cluster: TaskStrategyCandidate[], domainLabel: string): string {
...
function buildStrategySummary(cluster: TaskStrategyCandidate[], domainLabel: string): string {
  ...
  const toolGroundedRatio =
    cluster.filter((candidate) => candidate.hasToolEvidence).length / Math.max(cluster.length, 1);
  const validationSuffix =
    toolGroundedRatio >= 0.5
      ? " Validate the result with tool or test feedback before treating it as resolved."
      : "";
  return truncateText(
    `When handling ${domainLabel || "similar tasks"}, prefer: ${resolution}.${validationSuffix}`,
    240,
  );
}
...
function semanticSourceFromMetadata(metadata: Record<string, unknown>): MaintenanceSemanticSource {
...
function semanticSourcesFromMetadata(
  metadata: Record<string, unknown>,
): MaintenanceSemanticSource[] {
  return stringArray(metadata.semanticSources).filter(
...
function strategyMaintenanceMetadata(params: {
  pattern: WorkflowPatternSummary;
  authoritySource: MaintenanceAuthoritySource;
  materializedEpoch: number;
}): Record<string, unknown> {
  const supportContentRefs = uniqueMaintenanceRefs(
    params.pattern.supportTaskIds.map((taskId) => `task:${taskId}`),
  );
...
export async function deriveStrategyHypotheses(
  store: MemxStoreBundle,
  ctx: MemoryOperationContext,
): Promise<StrategyDerivationStats> {
  ...
  const stats: StrategyDerivationStats = {
...
async function deriveWorkflowPatternSummariesDetailed(
  store: MemxStoreBundle,
  ctx: MemoryOperationContext,
): Promise<{
  summaries: WorkflowPatternSummary[];
  embeddingBudget: number;
  embeddingCandidatesConsidered: number;
  embeddingCandidatesEmbedded: number;
  authoritySource: MaintenanceAuthoritySource;
  ...
  const candidates = taskBeliefs
    ...
    .filter((candidate): candidate is TaskStrategyCandidate => Boolean(candidate))
  ...
  const embeddingByTaskId =
    ctx.config.advanced.enableEmbeddingClustering && embeddingCandidates.length > 1
      ? new Map(embeddingCandidates.map((candidate) => [candidate.task.taskId, [] as number[]]))
      : undefined;
  if (embeddingByTaskId) {
    const embeddings = await store.retrievalBackend.embedTextsBatch(
      embeddingCandidates.map((candidate) => candidate.canonicalText),
      "passage",
    );
  ...
  const rawClusters = clusterCandidates(candidates, embeddingByTaskId).filter(
    (cluster) => new Set(cluster.map((candidate) => candidate.task.taskId)).size >= 2,
  );
  const clusters: TaskStrategyCandidate[][] = rawClusters;
  const summaries: WorkflowPatternSummary[] = [];
  const semanticSources = new Set<MaintenanceSemanticSource>(["upstream_structured"]);
  for (const cluster of clusters) {
    ...
    const cohesion = average(
      cluster.map((candidate) => candidateSimilarity(cluster[0]!, candidate, embeddingByTaskId)),
    );
    ...
    const avgContradiction = average(
      cluster.map((candidate) => candidate.belief.contradictionScore),
    );
    const avgOutcomeSupport = average(
      cluster.map((candidate) => candidate.belief.outcomeSupportScore),
    );
    const sessionDiversity = clamp01(
      new Set(cluster.map((candidate) => candidate.task.sessionKey)).size / 2,
    );
    const toolGroundedRatio =
      cluster.filter((candidate) => candidate.hasToolEvidence).length / Math.max(cluster.length, 1);
    ...
    const clusterUsesEmbedding = cluster.some(
      (candidate) => (embeddingByTaskId?.get(candidate.task.taskId)?.length ?? 0) > 0,
    );
    const clusterSemanticSource: MaintenanceSemanticSource = clusterUsesEmbedding
      ? "embedding_clustered"
      : "lexical_fallback";
...
export async function deriveWorkflowPatternSummaries(
  store: MemxStoreBundle,
  ctx: MemoryOperationContext,
): Promise<WorkflowPatternSummary[]> {
