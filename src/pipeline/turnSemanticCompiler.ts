...
import { analyzeSemanticHints } from "./semantics.js";
import { buildDeterministicTaskProposal } from "./taskJudge.js";
...
const TURN_SEMANTIC_COMPILER_CUTOVER_CRITERIA = {
  ...
  fallbackRateMax: 0.1,
  maxWriteDiffRate: 0.15,
} as const;

const STRATEGY_PROCEDURE_PATTERN =
  /(?:先.+再|先检查|再检查|然后|最后|回头|优先|步骤|顺序|流程|策略|原则|排查|验证过|记着|最近版本记着|最近两次)/iu;
...
function familyHintsFromStructuredHints(
  text: string,
  structuredHints: MemoryCandidateStructuredHints | undefined,
): TurnSemanticAssertionDraft["familyHint"][] {
  const families = new Set<TurnSemanticAssertionDraft["familyHint"]>();
  const relationConfidence = Math.max(
    structuredHints?.relation?.confidence ?? 0,
    ...(structuredHints?.relations?.map((entry) => entry.confidence ?? 0) ?? [0]),
  );
  if (structuredHints?.workflow || (structuredHints?.workflows && structuredHints.workflows.length > 0)) {
    families.add("workflow");
  }
  if (structuredHints?.preference) {
    families.add("preference");
  }
  if (
    !structuredHints?.correction &&
    relationConfidence >= 0.82 &&
    !STRATEGY_PROCEDURE_PATTERN.test(text) &&
    (structuredHints?.relation || (structuredHints?.relations && structuredHints.relations.length > 0))
  ) {
    families.add("relation_like");
  }
  if (
    structuredHints?.decision &&
    (structuredHints.timeHints?.length ?? 0) === 0 &&
    text.length <= 140
  ) {
    families.add("fact_like");
  }
  if (
    STRATEGY_PROCEDURE_PATTERN.test(text) &&
    !structuredHints?.relation &&
    !(structuredHints?.relations && structuredHints.relations.length > 0)
  ) {
    families.add("strategy_like");
  }
  if (structuredHints?.correction) {
    families.add(
      structuredHints.correction.timeframe === "historical" ||
        structuredHints.correction.timeframe === "compare"
        ? "event_like"
        : "fact_like",
    );
  }
...
function timeframeHint(
  correction: MemoryCandidateCorrectionHint | undefined,
): TurnSemanticAssertionDraft["timeframeHint"] {
  if (!correction) {
    return "timeless";
  }
...
function deterministicCompile(params: CompileTurnSemanticsParams): TurnSemanticFrame {
  ...
  const assertionDrafts: TurnSemanticAssertionDraft[] = [];
  const correctionDrafts: TurnSemanticCorrectionDraft[] = [];
  const relationDrafts: TurnSemanticRelationDraft[] = [];
  const supportSpans: TurnSemanticFrame["supportSpans"] = [];

  for (const message of messages) {
    const sourceRef = sourceRefForMessage(message);
    const hints = analyzeSemanticHints(message.content);
    const structuredHints: MemoryCandidateStructuredHints = {
      entities: hints.entities,
      timeHints: hints.timeHints,
      ...(hints.preference ? { preference: hints.preference, preferenceHint: true } : {}),
      ...(hints.workflow ? { workflow: hints.workflow, workflows: hints.workflows, taskStateHint: true } : {}),
      ...(hints.relation ? { relation: hints.relation, relations: hints.relations, relationHint: true } : {}),
      ...(hints.decision ? { decision: hints.decision, decisionHint: true } : {}),
      ...(hints.correction ? { correction: hints.correction, correctionHint: true } : {}),
    };
    ...
    for (const family of families) {
      assertionDrafts.push({
        ...
        slotHints:
          structuredHints.workflow?.key ? [structuredHints.workflow.key] : undefined,
    ...
    for (const relation of structuredHints.relations ?? []) {
      relationDrafts.push({
        sourceRef,
        relation: {
          ...relation,
    ...
    if (structuredHints.correction) {
      correctionDrafts.push({
        sourceRef,
        correction: structuredHints.correction,
        supportSpans: [{ text: truncateText(message.content.trim(), 240) }],
        confidence: structuredHints.correction.confidence ?? 0.72,
        lineage: {
          sourceKind: "chunk",
          sourceId: message.turnId,
          sourceRef,
  ...
  const summaryChunks: ConversationChunk[] = messages.map((message, index) => ({
  ...
  const deterministicTaskSummary = summarizeTaskHeuristically(summaryChunks);
  const taskProposal: TurnSemanticTaskProposal = {
  ...
  return {
    ...
    compilerProvenance: {
      ...
      mode: "deterministic",
      reasons: [
        `criteria=fallback<=${TURN_SEMANTIC_COMPILER_CUTOVER_CRITERIA.fallbackRateMax}`,
      ],
    },
  };
}
...
function semanticDraftForSourceRef(
  ...
  const relationDrafts = (frame.relationDrafts ?? []).filter(
    (entry) => entry.sourceRef === sourceRef || isTurnPrimarySource,
  ).map((entry) => ({
    ...entry,
    relation: {
      ...entry.relation,
...
function affirmedRelationDrafts(
  ...
  return (draft.relationDrafts ?? []).filter(
    (entry) => entry.relation.polarity !== "negated",
  );
}
