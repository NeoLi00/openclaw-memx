...
type ProjectDescriptorStripOptions = {
  allowLatinDescriptorTail?: boolean;
  allowHanDescriptorTail?: boolean;
};

function isLatinProjectHeadword(value: string): boolean {
  return LATIN_PROJECT_HEADWORDS.has(value.toLowerCase());
}

function stripProjectDescriptorTail(
  value: string,
  options: ProjectDescriptorStripOptions = {},
): string {
...
function compactProjectIdentity(
  value: string,
  options: ProjectDescriptorStripOptions = {},
): string {
...
function buildProjectAliasVariants(
  value: string,
  options: ProjectDescriptorStripOptions,
): string[] {
...
export function projectIdentityKey(value: string): string {
...
export function looksLikeProjectDescriptor(value: string): boolean {
  ...
  const compact = normalized.replace(/\s+/g, "");
  return (
    [...LATIN_PROJECT_HEADWORDS].some((suffix) => compact.toLowerCase() === suffix) ||
    HAN_PROJECT_HEADWORDS.some((suffix) => compact === suffix)
  );
}

export function projectAliasVariants(value: string): string[] {
...
export function isProjectProfileStateKey(key: string): boolean {
  return key.startsWith("project.") && key !== "project.active_project";
}

export function projectCodeFromStateKey(key: string): string | undefined {
