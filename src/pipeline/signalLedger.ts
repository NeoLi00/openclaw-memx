...
type OutcomeEvidenceRoleSummary = {
  assistantCount: number;
  userCount: number;
  toolCount: number;
  nonAssistantCount: number;
...
function recordSignal(
  ...
  ctx: Pick<MemoryOperationContext, "agentId" | "sessionKey" | "now" | "scopes">,
  params: Omit<MemorySignalEventRecord, "signalId" | "agentId" | "scope" | "createdAt"> & {
    signalId?: string;
  },
): void {
...
function sourceFamily(sourceRef: string): string {
...
function maintenanceSignalId(params: {
  agentId: string;
  beliefId: string;
  signalType: MemorySignalEventRecord["signalType"];
  bucket: string;
}): string {
  return stableHash([
    "maintenance-signal",
...
function temporalStabilityWindowDays(memoryKind: MemoryBeliefRecord["memoryKind"]): number {
  switch (memoryKind) {
    case "state":
      return 5;
    case "task":
...
function futureUsefulnessValue(params: {
  ...
  rank?: number;
  recallMode: "full" | "background-only";
}): number {
...
type BeliefSignalSummary = {
  positiveSignals: MemorySignalEventRecord[];
  contradictionSignals: MemorySignalEventRecord[];
  distinctSupportDays: number;
  distinctSupportSources: number;
  positiveAverage: number;
  contradictionAverage: number;
};

function summarizeBeliefSignals(signals: MemorySignalEventRecord[]): BeliefSignalSummary {
  const positiveSignals = signals.filter(
    (signal) =>
      signal.signalType === "retrieval_support" ||
...
function summarizeOutcomeEvidenceRoles(chunks: ConversationChunk[]): OutcomeEvidenceRoleSummary {
...
function signalTargetForTaskOutcome(
...
function signalTargetForStateRow(row: EvidenceRow): SignalTarget {
  return {
    memoryKind: "state",
    contentRef: row.id,
...
function signalTargetForTaskRow(row: EvidenceRow): SignalTarget {
  return {
    memoryKind: "task",
    contentRef: row.id,
...
function signalTargetForGraphEdge(edge: GraphEvidenceEdge): SignalTarget {
...
function signalTargetForGraphEdgeId(
  store: MemxStoreBundle,
  ctx: Pick<MemoryOperationContext, "agentId">,
  ...
  const row = store.client
    ...
    .get(ctx.agentId, edgeId) as GraphSignalRow | undefined;
  if (!row) {
    return null;
  }
  return {
...
function signalTargetForNormalizedState(state: NormalizedState): SignalTarget {
  return {
    memoryKind: "state",
...
function signalTargetForNormalizedFact(fact: NormalizedFact): SignalTarget {
  return {
    memoryKind: "fact",
...
function signalTargetForNormalizedEvent(event: NormalizedEvent): SignalTarget {
  return {
    memoryKind: "event",
...
function signalTargetForNormalizedGraphEdge(edge: NormalizedGraphEdge): SignalTarget {
  return {
    memoryKind: "graph_edge",
    contentRef: edge.edgeId,
...
function emitWriteSignal(
  ...
  params: {
    target: SignalTarget;
    signalType: "promotion" | "assistant_grounding";
    value: number;
    sourceRef: string;
...
export function emitWriteMaterializationSignals(
  store: MemxStoreBundle,
  ctx: Pick<MemoryOperationContext, "agentId" | "sessionKey" | "now" | "scopes">,
  params: {
...
export function signalTargetForMemoryObject(object: MemoryObject): SignalTarget | null {
  switch (object.kind) {
    case "state":
...
function idAfterPrefix(value: string | undefined, prefix: string): string | undefined {
...
function signalTargetForPromptEvidence(
  store: MemxStoreBundle,
  ctx: Pick<MemoryOperationContext, "agentId" | "scopes" | "now">,
  entry: PromptEvidenceCandidate,
): SignalTarget | null {
  const factId =
    canonicalOrSourceId(entry, "fact") ??
  ...
  const eventId =
    canonicalOrSourceId(entry, "event") ??
  ...
  const graphEdgeId =
    canonicalOrSourceId(entry, "graph_edge") ??
  ...
  const stateKey =
    canonicalOrSourceId(entry, "state") ??
  ...
  const chunkId =
    canonicalOrSourceId(entry, "chunk") ??
...
function promptEvidenceSignalSourceRef(
  entry: PromptEvidenceCandidate,
  auditSourceRef: string,
): string {
...
function promptEvidenceSupportValue(entry: PromptEvidenceCandidate): number {
  return clamp01(
    entry.injectionScore ??
...
function emitRetrievalSignalPair(
  store: MemxStoreBundle,
  ctx: Pick<MemoryOperationContext, "agentId" | "now" | "scopes">,
  params: {
    target: SignalTarget;
    sourceRef: string;
  ...
  const usefulnessValue = futureUsefulnessValue({
    ...
    routeConfidence:
      typeof params.metadataJson.routeConfidence === "number"
...
export function emitBackgroundRetrievalSignals(
  store: MemxStoreBundle,
  ctx: Pick<MemoryOperationContext, "agentId" | "sessionKey" | "now" | "scopes">,
  params: {
    auditId: string;
...
export function emitFullRetrievalSignals(
  store: MemxStoreBundle,
  ctx: Pick<MemoryOperationContext, "agentId" | "sessionKey" | "now" | "scopes">,
  params: {
    auditId: string;
  ...
  for (const [rank, entry] of params.scheduled.entries()) {
    ...
    const selected =
      (entry.object.kind === "state" && selectedStateIds.has(rowId)) ||
  ...
  const injectedPromptEvidence = params.bundle.promptEvidence.filter(
    (entry) => entry.injected === true || (entry.role === "protected" && !entry.dropReason),
  );
...
function loadFactSignalRows(store: MemxStoreBundle, factIds: string[]): FactSignalRow[] {
  if (factIds.length === 0) {
    return [];
  }
  ...
  return store.client
    ...
    .all(...factIds) as FactSignalRow[];
}

export function emitContradictionSignals(
  store: MemxStoreBundle,
  ctx: Pick<MemoryOperationContext, "agentId" | "sessionKey" | "now" | "scopes">,
  params: {
    query: string;
...
export function emitOutcomeFeedbackSignal(
  store: MemxStoreBundle,
  ctx: Pick<MemoryOperationContext, "agentId" | "sessionKey" | "now" | "scopes">,
  params: {
    task: ConversationTask;
...
export function emitAssistantOutcomeLearningSignals(
  store: MemxStoreBundle,
  ctx: Pick<MemoryOperationContext, "agentId" | "sessionKey" | "now" | "scopes">,
  params: {
    task: ConversationTask;
  ...
  if (roles.hasNonAssistantGrounding) {
    const groundingEvidenceStrength = roles.hasToolEvidence
      ...
      : roles.hasUserEvidence
        ? 0.82
        : 0.68;
    const groundingValue = clamp01(
      groundingEvidenceStrength *
        (0.45 +
  ...
  if (params.shouldPromote) {
    const promotionValue = clamp01(
      (params.outcome.promotionScore || params.outcome.confidence) *
  ...
  const demotionValue = clamp01(
    (1 - (params.outcome.promotionScore || params.outcome.confidence)) * 0.65 +
...
export function emitBeliefMaintenanceSignals(
  store: MemxStoreBundle,
  ctx: Pick<MemoryOperationContext, "agentId" | "sessionKey" | "now" | "scopes">,
): void {
  ...
  const groupedSignals = new Map<string, MemorySignalEventRecord[]>();
  ...
  for (const belief of beliefs) {
    ...
    if (
      ...
      const selfConsistencyValue = clamp01(
        summary.positiveAverage * 0.44 +
    ...
    if (stabilityEligible) {
      const temporalValue = clamp01(
        Math.min(ageDays / Math.max(windowDays * 2, 1), 1) * 0.44 +
