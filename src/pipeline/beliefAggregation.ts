...
import {
  ...
  POSTERIOR_CONSISTENCY_WEIGHT,
  POSTERIOR_CONTRADICTION_WEIGHT,
  POSTERIOR_CORRECTION_WEIGHT,
  POSTERIOR_DEMOTION_WEIGHT,
  POSTERIOR_MIDPOINT,
  POSTERIOR_OUTCOME_WEIGHT,
  POSTERIOR_PROMOTION_WEIGHT,
  POSTERIOR_REPEATED_USE_WEIGHT,
  POSTERIOR_STABILITY_WEIGHT,
  POSTERIOR_STALE_DECAY_WEIGHT,
  POSTERIOR_TEMPORAL_WEIGHT,
  POSTERIOR_USEFULNESS_WEIGHT,
  SOURCE_RELIABILITY,
} from "./constants.js";
...
type SignalSummary = {
  retrievalSupportAvg: number;
  futureUsefulnessAvg: number;
  outcomeFeedbackAvg: number;
  contradictionAvg: number;
  selfConsistencyAvg: number;
  temporalStabilityAvg: number;
  assistantGroundingAvg: number;
  promotionAvg: number;
  demotionAvg: number;
  correctionAvg: number;
  repeatedUseAvg: number;
  staleDecayAvg: number;
  useCount: number;
...
type SignalGroup = {
  ...
  contentRef?: string;
  semanticKey: string;
  signals: MemorySignalEventRecord[];
};
...
type StagePolicy = {
  probationaryPosterior: number;
  activePosterior: number;
  quarantineContradiction: number;
  supersessionGap: number;
  decayAfterDays: number;
  minSupportForProbationary: number;
  minSupportForActive: number;
};

type ProvisionalBelief = {
  record: MemoryBeliefRecord;
...
function sourceReliabilityFromMarker(marker: string | undefined): number {
  switch (marker) {
    case "tool":
...
function fallbackPriorConfidence(memoryKind: MemoryBeliefKind): number {
  return FALLBACK_PRIOR_CONFIDENCE[memoryKind];
}

function fallbackSourceReliability(memoryKind: MemoryBeliefKind): number {
  return (
    FALLBACK_SOURCE_RELIABILITY[memoryKind as keyof typeof FALLBACK_SOURCE_RELIABILITY] ??
    FALLBACK_SOURCE_RELIABILITY.default
  );
}

function stagePolicy(memoryKind: MemoryBeliefKind): StagePolicy {
  switch (memoryKind) {
    case "state":
      return {
  ...
  switch (memoryKind) {
    case "state": {
      const row = store.client
        .prepare(
          `SELECT scope, confidence, source_ref AS sourceRef, updated_at AS updatedAt
             FROM state_kv
            WHERE agent_id = ?
              AND scope || ':' || key = ?
            LIMIT 1`,
        )
        .get(agentId, contentRef) as
        | { scope: string; confidence: number; sourceRef: string; updatedAt: string }
        | undefined;
      if (!row) {
        return null;
      }
    ...
    case "task": {
      const row = store.client
        .prepare(
          `SELECT scope, started_at AS startedAt, updated_at AS updatedAt, status
        ...
        .get(agentId, contentRef) as
        | { scope: string; startedAt: string; updatedAt: string; status: string }
        | undefined;
      if (!row) {
        return null;
      }
    ...
    case "fact": {
      const row = store.client
        .prepare(
          `SELECT scope, confidence, created_at AS createdAt, updated_at AS updatedAt
             FROM facts
            WHERE agent_id = ?
              AND fact_id = ?
            LIMIT 1`,
        ...
        .get(agentId, contentRef) as
        | { scope: string; confidence: number; createdAt: string; updatedAt: string }
        | undefined;
      if (!row) {
        return null;
      }
    ...
    case "event": {
      const row = store.client
        .prepare(
          `SELECT scope, confidence, observed_at AS observedAt, source_kind AS sourceKind
        ...
        .get(agentId, contentRef) as
        | { scope: string; confidence: number; observedAt: string; sourceKind: string }
        | undefined;
      if (!row) {
        return null;
      }
    ...
    case "graph_edge": {
      const row = store.client
        .prepare(
          `SELECT scope, confidence, evidence_ref AS evidenceRef, created_at AS createdAt, updated_at AS updatedAt
             FROM graph_edges
            WHERE agent_id = ?
              AND edge_id = ?
            LIMIT 1`,
    ...
    case "chunk": {
      const row = store.client
        .prepare(
          `SELECT scope, role, created_at AS createdAt, updated_at AS updatedAt
        ...
        .get(agentId, contentRef) as
        | { scope: string; role: string; createdAt: string; updatedAt: string }
        | undefined;
      if (!row) {
        return null;
      }
...
function summarizeSignals(signals: MemorySignalEventRecord[]): SignalSummary {
  const byType = new Map<string, MemorySignalEventRecord[]>();
  ...
  const counts = Object.fromEntries(
    [...byType.entries()].map(([signalType, bucket]) => [signalType, bucket.length]),
  ...
  return {
    retrievalSupportAvg: average(
      (byType.get("retrieval_support") ?? []).map((signal) => signal.value),
    ),
    futureUsefulnessAvg: average(
      (byType.get("future_usefulness") ?? []).map((signal) => signal.value),
    ),
    outcomeFeedbackAvg: average(
      (byType.get("outcome_feedback") ?? []).map((signal) => signal.value),
    ),
    contradictionAvg: average((byType.get("contradiction") ?? []).map((signal) => signal.value)),
    selfConsistencyAvg: average(
      (byType.get("self_consistency") ?? []).map((signal) => signal.value),
    ),
    temporalStabilityAvg: average(
      (byType.get("temporal_stability") ?? []).map((signal) => signal.value),
    ),
    assistantGroundingAvg: average(
      (byType.get("assistant_grounding") ?? []).map((signal) => signal.value),
    ),
    ...
    useCount:
      (byType.get("future_usefulness") ?? []).length + (byType.get("repeated_use") ?? []).length,
    counts,
...
function posteriorConfidence(params: {
  ...
  const contradictionPenalty = Math.min(
    Math.sqrt(params.contradictionScore) * POSTERIOR_CONTRADICTION_WEIGHT,
    0.25,
  );
  const demotionPenalty = Math.min(Math.sqrt(params.demotion) * POSTERIOR_DEMOTION_WEIGHT, 0.12);
  const correctionPenalty = Math.min(
    Math.sqrt(params.correction) * POSTERIOR_CORRECTION_WEIGHT,
    0.22,
  );
  const staleDecayPenalty = Math.min(
    Math.sqrt(params.staleDecay) * POSTERIOR_STALE_DECAY_WEIGHT,
    0.1,
  );
  return clamp01(
...
function signalEvidenceSupportScore(summary: SignalSummary): number {
  return clamp01(
    summary.retrievalSupportAvg * 0.26 +
...
function isEntityScopedSemanticKey(semanticKey: string): boolean {
  return semanticKey.startsWith("entity:");
}

function entityIdFromScopedSemanticKey(semanticKey: string): string | undefined {
  const match = semanticKey.match(/^entity:([^:]+):/u);
  return match?.[1];
}

function canonicalTargetContentRef(
  contentRef: string | undefined,
  semanticKey: string,
): string | undefined {
...
function targetGroupKey(params: {
  ...
  return `${params.memoryKind}:${
    canonicalTargetContentRef(params.contentRef, params.semanticKey) ?? params.semanticKey
  }`;
}

function taskPredicateFromSemanticKey(signal: MemorySignalEventRecord): string {
  if (signal.memoryKind === "task" && signal.contentRef) {
    const escapedContentRef = signal.contentRef.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const taskOutcomeMatch = signal.semanticKey.match(
      new RegExp(`^task_outcome:${escapedContentRef}:(.+)$`, "u"),
    );
...
export function resolveBeliefTargetKey(
  store: MemxStoreBundle,
  ctx: MemoryOperationContext,
  scope: string,
  signal: MemorySignalEventRecord,
): BeliefTargetResolution {
  const base: BeliefTargetResolution = {
  ...
  if (signal.sessionKey) {
    return {
      ...base,
      semanticKey: `session:${signal.sessionKey}:${normalizeText(signal.semanticKey).replace(
        /:/g,
        ".",
      )}`,
...
function canonicalizeSignalEntityTarget(
  store: MemxStoreBundle,
  ctx: MemoryOperationContext,
  scope: string,
  signal: MemorySignalEventRecord,
  ...
  if (
    resolved.semanticKey === signal.semanticKey &&
...
function canonicalizeSignalEntityTargets(
  store: MemxStoreBundle,
  ctx: MemoryOperationContext,
  scope: string,
  signals: MemorySignalEventRecord[],
...
function signalSupportContentRef(signal: MemorySignalEventRecord): string | undefined {
...
function groupSignals(signals: MemorySignalEventRecord[]): SignalGroup[] {
...
function mergeGroupsWithExisting(
  signals: MemorySignalEventRecord[],
  existingBeliefs: MemoryBeliefRecord[],
): SignalGroup[] {
  const groups = new Map<string, SignalGroup>();
...
function buildBeliefId(params: {
  agentId: string;
  scope: string;
  memoryKind: MemoryBeliefRecord["memoryKind"];
  contentRef?: string;
  semanticKey: string;
}): string {
  return stableHash([
...
export function aggregateBeliefs(
  ...
  const dueBeliefs = existingBeliefs.filter(
    (belief) =>
      (!options.scopes || options.scopes.includes(belief.scope)) &&
  ...
  const canonicalDeltaSignals = canonicalizeSignalEntityTargets(
    store,
    ctx,
    resolverScope,
    deltaSignals,
  );
  const touchedTargetKeys = new Set<string>();
  const touchedFamilyKeys = new Set<string>();
  const touchedTargets: BeliefAggregationTarget[] = [];
  for (const signal of canonicalDeltaSignals) {
    const contentRef = canonicalTargetContentRef(signal.contentRef, signal.semanticKey);
    const target: BeliefAggregationTarget = {
      memoryKind: signal.memoryKind,
      contentRef,
      semanticKey: signal.semanticKey,
    };
  ...
  const signalsById = new Map(
    canonicalizeSignalEntityTargets(store, ctx, resolverScope, rawSignals).map((signal) => [
      signal.signalId,
      signal,
    ]),
  );
  const touchedEntityTargetKeys = new Set(
    [...touchedTargetKeys].filter((key) => key.includes(":entity:")),
  );
  if (options.signalWindow && touchedEntityTargetKeys.size > 0) {
    for (const signal of canonicalizeSignalEntityTargets(
      store,
      ctx,
      resolverScope,
      store.auditRepo.listSignals({
  ...
  const relevantExistingBeliefs = options.signalWindow
    ? existingBeliefs.filter(
        (belief) =>
          touchedTargetKeys.has(
            beliefTargetKey({
              memoryKind: belief.memoryKind,
              contentRef: belief.contentRef,
              semanticKey: belief.semanticKey,
  ...
  for (const group of mergeGroupsWithExisting(signals, relevantExistingBeliefs)) {
    ...
    const outcomeSupportScore = clamp01(
      summary.outcomeFeedbackAvg * 0.75 + summary.assistantGroundingAvg * 0.25,
    );
    ...
    const firstSeenAt =
      minIso([existing?.firstSeenAt, snapshot?.firstSeenAt, firstSignal?.createdAt]) ?? ctx.now;
    const lastSeenAt =
      maxIso([
        existing?.lastSeenAt,
        snapshot?.lastSeenAt,
    ...
    const record: MemoryBeliefRecord = {
      beliefId,
      agentId: ctx.agentId,
      scope,
      memoryKind: group.memoryKind,
      ...
      metadataJson: {
        ...
        supportContentRefs: [
          ...new Set(
            group.signals.map((signal) => signalSupportContentRef(signal)).filter(Boolean),
          ),
        ],
  ...
  const stageRank: Record<MemoryBeliefStage, number> = {
    candidate: 0,
    probationary: 1,
    active: 2,
    decaying: -1,
  ...
  for (const provisional of provisionalRecords) {
    ...
    const contradictionTriggered =
      provisional.record.contradictionScore >= policy.quarantineContradiction ||
      (strongDemotion &&
        provisional.record.contradictionScore >=
          Math.max(policy.quarantineContradiction * 0.55, 0.32));
    let stage: MemoryBeliefStage = "candidate";
    ...
    if (contradictionTriggered) {
      ...
      if (
        provisional.record.posteriorConfidence >= policy.probationaryPosterior &&
      ...
      provisional.record.posteriorConfidence >= policy.activePosterior &&
  ...
  const families = new Map<string, ProvisionalBelief[]>();
  for (const provisional of provisionalRecords) {
  ...
  for (const bucket of families.values()) {
    ...
    for (const provisional of bucket) {
      if (
        provisional.record.beliefId === dominant.record.beliefId ||
  ...
  for (const provisional of provisionalRecords) {
    ...
    provisional.record.metadataJson = {
      ...
      stageTransitionAt:
        previousStage && previousStage !== provisional.record.stage ? ctx.now : undefined,
    };
