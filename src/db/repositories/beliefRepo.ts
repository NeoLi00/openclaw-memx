...
type BeliefRow = {
  ...
  agent_id: string;
  scope: string;
  memory_kind: MemoryBeliefRecord["memoryKind"];
  content_ref: string | null;
  semantic_key: string;
  stage: MemoryBeliefRecord["stage"];
  prior_confidence: number;
  posterior_confidence: number;
  usefulness_score: number;
  stability_score: number;
  contradiction_score: number;
  outcome_support_score: number;
...
export class BeliefRepo {
  ...
  listByAgent(params: { agentId: string; limit?: number; readEpoch?: number }): MemoryBeliefRecord[] {
    ...
    return (this.db.prepare(sql).all(...values) as BeliefRow[]).map((row) => this.toBelief(row));
  }
  ...
  getById(beliefId: string): MemoryBeliefRecord | undefined {
    const row = this.db
      ...
      .get(beliefId) as BeliefRow | undefined;
    return row ? this.toBelief(row) : undefined;
  }

  private toBelief(row: BeliefRow): MemoryBeliefRecord {
    return {
