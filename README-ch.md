...
如果 OpenClaw 已经配置了 LLM provider，MemX 不需要单独配置一份 LLM key。它会复用
`models.providers.<provider>.apiKey` 和 headers。

默认本地 embedding 不需要 API key。如果你选择远程 OpenAI-compatible embedding，需要单独配置
MemX 的 embedding key：
