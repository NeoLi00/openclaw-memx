...
推荐的一行安装会一次性配置 OpenClaw、MemX 的 LLM 和本地 embedding：
...
- MemX 语义编译模型：`deepseek/deepseek-v4-flash`
...
`openclaw plugins install @neoli00/memory-memx` 安装 MemX 插件，写入 MemX 配置，重启 Gateway，
...
使用其它 OpenAI-compatible LLM provider：
