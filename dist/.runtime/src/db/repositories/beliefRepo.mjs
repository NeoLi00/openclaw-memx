...
//#region src/db/repositories/beliefRepo.ts
var BeliefRepo = class {
	db;
	constructor(db) {
		this.db = db;
	}
	upsert(record) {
		this.db.prepare(`INSERT INTO memory_beliefs(
	...
	*/
	markSupersededByContentRef(params) {
		const result = this.db.prepare(`UPDATE memory_beliefs
            SET stage = 'superseded', updated_at = ?
          WHERE agent_id = ?
            AND content_ref = ?
            AND stage IN ('active', 'probationary', 'candidate')`).run(params.updatedAt, params.agentId, params.contentRef);
		return Number(result.changes ?? 0);
	}
	getById(beliefId) {
		const row = this.db.prepare(`SELECT belief_id, agent_id, scope, memory_kind, content_ref, semantic_key, stage,
                prior_confidence, posterior_confidence, usefulness_score, stability_score,
                contradiction_score, outcome_support_score, source_reliability, first_seen_at,
                last_seen_at, last_used_at, use_count, reevaluation_due_at, derived_from_min_epoch,
                derived_from_max_epoch, materialized_epoch, metadata_json, created_at, updated_at
           FROM memory_beliefs
          WHERE belief_id = ?`).get(beliefId);
		return row ? this.toBelief(row) : void 0;
	}
	toBelief(row) {
		return {
			...
			contentRef: row.content_ref ?? void 0,
			...
			reevaluationDueAt: row.reevaluation_due_at ?? void 0,
