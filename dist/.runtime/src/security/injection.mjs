//#region src/security/injection.ts
...
const CONTROL_VERB_PATTERN = /\b(?:ignore|bypass|override|rewrite|replace|execute|invoke|call|run)\b/i;
...
function looksLikePromptInjection(text) {
	...
	if (EXPLICIT_PROMPT_INJECTION_PATTERNS.some((pattern) => pattern.test(normalized))) return true;
	...
	if (hasRoleTags && (hasToolExecution || hasControlVerb)) return true;
	if (hasPrivilegedPromptReference && hasControlVerb && hasPrivilegedTarget) return true;
	if (hasStructuredControlBlock && (hasRoleTags || hasPrivilegedPromptReference) && (hasToolExecution || hasControlVerb)) return true;
	return false;
}
...
//#endregion
export { containsLikelySecret, looksLikePromptInjection };
