import { clamp01, cosineSimilarity, normalizeText, stableHash, truncateText } from "../support.mjs";
import { tokenizeSearchTerms } from "./semantic/heuristics.mjs";
...
import { buildMaintenanceContractMetadata, uniqueMaintenanceRefs } from "./maintenanceContract.mjs";
//#region src/pipeline/strategyHypotheses.ts
...
function phaseWeight(phase) {
	switch (phase) {
		...
		case "validated": return .92;
		case "attempting": return .4;
		default: return .55;
	}
}
function shouldConsiderTaskBelief(belief, task) {
	const metadata = task.metadataJson ?? {};
	const hasOutcome = Boolean(truthyString(metadata.lastEmittedOutcomeKey) || truthyString(metadata.lastPromotedOutcomeHypothesisId));
	const promotionAvg = typeof belief.metadataJson.promotionAvg === "number" ? belief.metadataJson.promotionAvg : 0;
	return belief.memoryKind === "task" && Boolean(belief.contentRef) && belief.stage !== "quarantined" && belief.stage !== "superseded" && (belief.posteriorConfidence >= .66 || belief.outcomeSupportScore >= .68 || promotionAvg >= .72) && belief.outcomeSupportScore >= .5 && belief.contradictionScore < .45 && hasOutcome;
}
function resolveStrategyDomainEntityId(store, ctx, task) {
	...
	for (const part of parts) {
		const result = resolveEntityMention(store, ctx, buildEntityMention({
			...
			metadataJson: { generatedFrom: "strategy-domain-resolution" }
		}), {
			createIfMissing: false,
			persist: false
		});
		if (result.method !== "uncertain") return result.entity.entityId;
	}
	const semanticEntity = task.metadataJson?.canonicalEntityId;
	return typeof semanticEntity === "string" && semanticEntity.trim() ? semanticEntity.trim() : void 0;
}
function buildTaskStrategyCandidate(store, ctx, belief) {
	...
	if (!task || !shouldConsiderTaskBelief(belief, task)) return null;
	const metadata = task.metadataJson ?? {};
...
function structuredBucketKey(candidate) {
	if (candidate.canonicalEntityId) return `${candidate.task.scope}:entity:${candidate.canonicalEntityId}`;
	const primary = structuredDomainParts(candidate)[0] ?? truthyString(candidate.task.metadataJson?.currentTask) ?? truthyString(candidate.task.metadataJson?.project) ?? truthyString(candidate.task.metadataJson?.candidateResolution) ?? candidate.outcomeKey ?? candidate.task.taskId;
...
function embeddingSimilarity(left, right, embeddingByTaskId) {
	...
	if (!leftEmbedding || !rightEmbedding || leftEmbedding.length === 0 || rightEmbedding.length === 0) return null;
...
function candidateSimilarity(left, right, embeddingByTaskId) {
	...
	const lexicalSimilarity = Math.max(combined, domain * .45 + resolution * .55);
	const embeddingScore = embeddingSimilarity(left, right, embeddingByTaskId);
	if (embeddingScore === null) return lexicalSimilarity;
	return clamp01(embeddingScore * .85 + lexicalSimilarity * .15);
}
function clusterCandidates(candidates, embeddingByTaskId) {
	const remaining = [...candidates].sort((left, right) => {
		if (structuredBucketKey(left) !== structuredBucketKey(right)) return structuredBucketKey(left).localeCompare(structuredBucketKey(right));
	...
	while (remaining.length > 0) {
		...
		for (const candidate of remaining) {
			...
			if (embeddingScore !== null ? sameStructuredResolution || embeddingScore >= .78 || embeddingScore >= .72 && resolution >= .58 : similarity >= .74 || domain >= .56 && resolution >= .62) cluster.push(candidate);
...
function selectDomainLabel(cluster) {
	const structuredCounts = /* @__PURE__ */ new Map();
	for (const candidate of cluster) for (const part of structuredDomainParts(candidate)) structuredCounts.set(part, (structuredCounts.get(part) ?? 0) + 1);
	const structuredLabel = [...structuredCounts.entries()].sort((left, right) => {
		if (right[1] !== left[1]) return right[1] - left[1];
		if (right[0].length !== left[0].length) return right[0].length - left[0].length;
		return left[0].localeCompare(right[0]);
	}).map(([label]) => label).find(Boolean);
	if (structuredLabel) return truncateText(structuredLabel, 48);
	...
	const tokens = [...counts.entries()].filter(([, count]) => count >= threshold).sort((left, right) => {
		if (right[1] !== left[1]) return right[1] - left[1];
		if (right[0].length !== left[0].length) return right[0].length - left[0].length;
		return left[0].localeCompare(right[0]);
...
function selectStrategyAnchorCandidate(cluster) {
	return [...cluster].sort((left, right) => {
		if (right.belief.posteriorConfidence !== left.belief.posteriorConfidence) return right.belief.posteriorConfidence - left.belief.posteriorConfidence;
		if (right.belief.outcomeSupportScore !== left.belief.outcomeSupportScore) return right.belief.outcomeSupportScore - left.belief.outcomeSupportScore;
...
function stableStrategyKeyPart(text, fallback, maxTerms) {
	...
	if (tokens.length > 0) return tokens.join(".");
	return normalizeText(fallback).replace(/[^\p{L}\p{N}]+/gu, ".").replace(/^\.+|\.+$/g, "") || "";
}
function stableStrategyDomainKey(cluster, domainLabel) {
	const anchor = selectStrategyAnchorCandidate(cluster);
	const domainPart = stableStrategyKeyPart(structuredDomainParts(anchor).join(" ").trim() || anchor.domainText, domainLabel, 3);
	...
	if (anchor.canonicalEntityId) return `entity:${anchor.canonicalEntityId}:${stableKey || resolutionPart}`;
...
function buildStrategySummary(cluster, domainLabel) {
	const resolution = [...cluster].sort((left, right) => {
		if (right.belief.outcomeSupportScore !== left.belief.outcomeSupportScore) return right.belief.outcomeSupportScore - left.belief.outcomeSupportScore;
		return right.belief.posteriorConfidence - left.belief.posteriorConfidence;
	})[0].resolutionText.replace(/[。.!！?？]+$/u, "").trim();
	const validationSuffix = cluster.filter((candidate) => candidate.hasToolEvidence).length / Math.max(cluster.length, 1) >= .5 ? " Validate the result with tool or test feedback before treating it as resolved." : "";
...
function semanticSourceFromMetadata(metadata) {
	...
	return source === "upstream_structured" || source === "embedding_clustered" || source === "llm_upgrade" || source === "deterministic_lifecycle" || source === "lexical_fallback" ? source : "upstream_structured";
}
function semanticSourcesFromMetadata(metadata) {
	return stringArray(metadata.semanticSources).filter((source) => source === "upstream_structured" || source === "embedding_clustered" || source === "llm_upgrade" || source === "deterministic_lifecycle" || source === "lexical_fallback");
}
...
function inferStrategyHypothesisStage(params) {
	...
	const hasGroundedWorkflowEvidence = params.explicitInstruction === true || params.groundedResolution === true || resolvedPhase && roles.has("user") && (roles.has("tool") || roles.has("assistant"));
	if (params.confidence >= .78 && params.usefulnessScore >= .62 && params.stabilityScore >= .62) return "active";
	if (hasGroundedWorkflowEvidence && params.contradictionScore <= .24 && params.confidence >= .64 && params.usefulnessScore >= .56 && params.stabilityScore >= .56) return "active";
	return "candidate";
}
async function deriveStrategyHypotheses(store, ctx) {
	...
	for (const pattern of workflowPatterns) {
		...
		if (stage === "active") stats.activeStrategies += 1;
		else if (stage === "quarantined") stats.quarantinedStrategies += 1;
		else stats.candidateStrategies += 1;
	}
	return stats;
}
async function deriveWorkflowPatternSummariesDetailed(store, ctx) {
	const candidates = store.beliefRepo.listByAgent({ agentId: ctx.agentId }).filter((belief) => belief.memoryKind === "task").map((belief) => buildTaskStrategyCandidate(store, ctx, belief)).filter((candidate) => Boolean(candidate)).filter((candidate) => ctx.scopes.includes(candidate.task.scope));
	const embeddingCandidates = [...candidates].sort((left, right) => {
		if (right.belief.posteriorConfidence !== left.belief.posteriorConfidence) return right.belief.posteriorConfidence - left.belief.posteriorConfidence;
		if (right.belief.usefulnessScore !== left.belief.usefulnessScore) return right.belief.usefulnessScore - left.belief.usefulnessScore;
		return right.belief.stabilityScore - left.belief.stabilityScore;
	}).slice(0, STRATEGY_EMBEDDING_BUDGET);
	const embeddingByTaskId = ctx.config.advanced.enableEmbeddingClustering && embeddingCandidates.length > 1 ? new Map(embeddingCandidates.map((candidate) => [candidate.task.taskId, []])) : void 0;
	...
	const clusters = clusterCandidates(candidates, embeddingByTaskId).filter((cluster) => new Set(cluster.map((candidate) => candidate.task.taskId)).size >= 2);
	const summaries = [];
	...
	for (const cluster of clusters) {
		...
		const confidence = clamp01(avgPosterior * .38 + avgOutcomeSupport * .22 + cohesion * .18 + repeatedSupport * .12 + phaseScore * .1 - avgContradiction * .25);
		const usefulnessScore = clamp01(avgUsefulness * .2 + repeatedSupport * .4 + sessionDiversity * .25 + avgOutcomeSupport * .15);
		const stabilityScore = clamp01(avgStability * .46 + cohesion * .24 + repeatedSupport * .14 + toolGroundedRatio * .08 + phaseScore * .08);
		const contradictionScore = clamp01(avgContradiction);
		const anchor = cluster[0];
		const clusterSemanticSource = cluster.some((candidate) => (embeddingByTaskId?.get(candidate.task.taskId)?.length ?? 0) > 0) ? "embedding_clustered" : "lexical_fallback";
		...
		summaries.push({
			...
			metadataJson: {
				...
				canonicalEntityIds: [...new Set(cluster.map((candidate) => candidate.canonicalEntityId).filter(Boolean))],
...
async function deriveWorkflowPatternSummaries(store, ctx) {
	return (await deriveWorkflowPatternSummariesDetailed(store, ctx)).summaries;
}
//#endregion
export { deriveStrategyHypotheses, deriveWorkflowPatternSummaries, inferStrategyHypothesisStage };
