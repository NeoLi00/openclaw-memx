import { clamp01, normalizeText, randomId, stableHash } from "../support.mjs";
//#region src/pipeline/signalLedger.ts
...
function beliefSignalGroupKey(params) {
	const contentRef = params.semanticKey.startsWith("entity:") ? void 0 : params.contentRef;
...
function temporalStabilityWindowDays(memoryKind) {
	switch (memoryKind) {
		...
		case "task": return 7;
		case "fact": return 30;
		case "event": return 14;
		case "graph_edge": return 45;
		case "chunk": return 3;
		case "strategy": return 45;
	}
}
function futureUsefulnessValue(params) {
	const rankBonus = Math.max(0, .18 - (params.rank ?? 0) * .035);
	const confidenceBoost = clamp01(params.routeConfidence ?? 0) * .2;
	const recallModeBias = params.recallMode === "full" ? .16 : .08;
	return clamp01(params.supportValue * .42 + confidenceBoost + rankBonus + recallModeBias);
}
function summarizeBeliefSignals(signals) {
	const positiveSignals = signals.filter((signal) => signal.signalType === "retrieval_support" || signal.signalType === "future_usefulness" || signal.signalType === "outcome_feedback" || signal.signalType === "assistant_grounding" || signal.signalType === "promotion");
	...
	return {
		...
		distinctSupportSources: new Set(positiveSignals.map((signal) => sourceFamily(signal.sourceRef))).size,
...
function signalTargetForGraphEdgeId(store, ctx, edgeId) {
	...
	const row = store.client.prepare(`SELECT edge_id AS edgeId,
        ...
        WHERE agent_id = ?
          AND edge_id = ?
        LIMIT 1`).get(ctx.agentId, edgeId);
	if (!row) return null;
	return {
		memoryKind: "graph_edge",
...
function emitWriteMaterializationSignals(store, ctx, params) {
	for (const state of params.states ?? []) emitWriteSignal(store, ctx, {
	...
	for (const fact of params.facts ?? []) emitWriteSignal(store, ctx, {
	...
	for (const event of params.events ?? []) emitWriteSignal(store, ctx, {
	...
	for (const edge of params.graphEdges ?? []) emitWriteSignal(store, ctx, {
...
function signalTargetForMemoryObject(object) {
	switch (object.kind) {
		case "state": return signalTargetForStateRow(object.row);
		case "task": return signalTargetForTaskRow(object.row);
		case "fact": {
			const subject = object.attributes.factSubject?.trim();
			const predicate = object.attributes.factPredicate?.trim();
			const semanticKey = subject && predicate ? `fact:${normalizeText(subject)}:${predicate}` : `fact:${normalizeText(object.row.id)}`;
			return {
				memoryKind: "fact",
...
function signalTargetForPromptEvidence(store, ctx, entry) {
	...
	if (factId) {
		...
		if (fact) return signalTargetForNormalizedFact(fact);
	}
	const eventId = canonicalOrSourceId(entry, "event") ?? idAfterPrefix(entry.id, "event:") ?? (entry.surface === "event" ? entry.id : void 0);
	if (eventId) {
		const event = store.eventRepo.get(eventId);
		if (event) return signalTargetForNormalizedEvent(event);
	}
	const graphTarget = signalTargetForGraphEdgeId(store, ctx, canonicalOrSourceId(entry, "graph_edge") ?? idAfterPrefix(entry.id, "edge:") ?? idAfterPrefix(entry.id, "graph:"));
	if (graphTarget) return graphTarget;
	const stateKey = canonicalOrSourceId(entry, "state") ?? idAfterPrefix(entry.id, "state:") ?? (entry.metadata && typeof entry.metadata.stateKey === "string" ? entry.metadata.stateKey : void 0);
	if (stateKey) {
		...
		if (state) return signalTargetForNormalizedState(state);
	}
	const chunkId = canonicalOrSourceId(entry, "chunk") ?? (entry.lineage?.sourceKind === "chunk" ? entry.lineage.sourceId : void 0) ?? idAfterPrefix(entry.id, "source-expansion:") ?? idAfterPrefix(entry.id, "event:chunk:") ?? idAfterPrefix(entry.id, "chunk:") ?? (entry.surface === "chunk" ? entry.id : void 0);
	if (chunkId) {
...
function promptEvidenceSignalSourceRef(entry, auditSourceRef) {
	if (entry.sourceRef?.trim()) return entry.sourceRef.trim();
	if (entry.lineage?.sourceRef?.trim()) return entry.lineage.sourceRef.trim();
	return (entry.mergedSourceRefs?.find((sourceRef) => sourceRef.trim()))?.trim() ?? auditSourceRef;
}
function promptEvidenceSupportValue(entry) {
	return clamp01(entry.injectionScore ?? entry.grade?.finalScore ?? entry.priority ?? entry.semanticScore ?? entry.goalScore ?? .5);
}
function emitRetrievalSignalPair(store, ctx, params) {
	...
	const usefulnessValue = futureUsefulnessValue({
		...
		routeConfidence: typeof params.metadataJson.routeConfidence === "number" ? params.metadataJson.routeConfidence : void 0,
		rank: typeof params.metadataJson.rank === "number" ? params.metadataJson.rank : void 0,
		recallMode: params.recallMode
	});
	recordSignal(store, ctx, {
...
function emitBackgroundRetrievalSignals(store, ctx, params) {
	const sourceRef = `audit:${params.auditId}`;
	for (const [rank, row] of params.bundle.states.entries()) emitRetrievalSignalPair(store, ctx, {
	...
	for (const [rank, row] of params.bundle.tasks.entries()) emitRetrievalSignalPair(store, ctx, {
...
function emitFullRetrievalSignals(store, ctx, params) {
	...
	const emitOnce = (paramsForSignal) => {
		...
		if (emittedTargets.has(key)) return;
	...
	for (const [rank, entry] of params.scheduled.entries()) {
		...
		if (!(entry.object.kind === "state" && selectedStateIds.has(rowId) || entry.object.kind === "task" && selectedTaskIds.has(rowId) || entry.object.kind === "fact" && selectedFactIds.has(rowId) || entry.object.kind === "event" && selectedEventIds.has(rowId) || entry.object.kind === "chunk" && recalledChunkIds.has(rowId))) continue;
		emitOnce({
			target,
	...
	for (const [rank, edge] of params.bundle.graph.edges.entries()) emitOnce({
...
function loadFactSignalRows(store, factIds) {
	...
	return store.client.prepare(`SELECT fact_id AS factId,
              canonical_subject AS canonicalSubject,
              predicate,
              canonical_object AS canonicalObject
         FROM facts
        WHERE fact_id IN (${placeholders})`).all(...factIds);
}
function emitContradictionSignals(store, ctx, params) {
	const contradictionEdges = params.graphEdges.filter((edge) => edge.relType === "contradicts");
	if (contradictionEdges.length === 0) return;
	const sourceRef = `contradiction:${stableHash([
		params.routeType,
		params.query,
		ctx.now
	])}`;
	for (const edge of contradictionEdges) {
		const factIds = (edge.evidenceRef ?? "").split("|").map((value) => value.trim()).filter(Boolean);
		const factRows = loadFactSignalRows(store, factIds);
		for (const row of factRows) recordSignal(store, ctx, {
...
function emitOutcomeFeedbackSignal(store, ctx, params) {
	const value = params.emitted ? clamp01(params.outcome.promotionScore || params.outcome.confidence) : clamp01((params.outcome.promotionScore || params.outcome.confidence) * .5);
	recordSignal(store, ctx, {
...
function emitAssistantOutcomeLearningSignals(store, ctx, params) {
	const roles = summarizeOutcomeEvidenceRoles(params.evidenceChunks);
	if (!roles.hasAssistantEvidence) return;
	...
	if (roles.hasNonAssistantGrounding) {
		const groundingValue = clamp01((roles.hasToolEvidence ? 1 : roles.hasUserEvidence ? .82 : .68) * (.45 + params.outcome.verificationScore * .3 + params.outcome.closureScore * .25 - params.outcome.contradictionRisk * .15));
		recordSignal(store, ctx, {
	...
	if (params.shouldPromote) {
		const promotionValue = clamp01((params.outcome.promotionScore || params.outcome.confidence) * (roles.hasNonAssistantGrounding ? 1 : .4) * (params.emitted ? 1 : .6));
		recordSignal(store, ctx, {
			signalType: "promotion",
			memoryKind: target.memoryKind,
			contentRef: target.contentRef,
	...
	const demotionValue = clamp01((1 - (params.outcome.promotionScore || params.outcome.confidence)) * .65 + params.outcome.contradictionRisk * .25 + (roles.hasNonAssistantGrounding ? 0 : .1));
	recordSignal(store, ctx, {
		signalType: "demotion",
		memoryKind: target.memoryKind,
		contentRef: target.contentRef,
...
function emitBeliefMaintenanceSignals(store, ctx) {
	const beliefs = store.beliefRepo.listByAgent({ agentId: ctx.agentId }).filter((belief) => ctx.scopes.includes(belief.scope));
	...
	for (const belief of beliefs) {
		if (belief.memoryKind === "strategy" || belief.stage === "quarantined" || belief.stage === "superseded") continue;
		const key = beliefSignalGroupKey({
		...
		if (summary.positiveSignals.length >= 2 && (summary.distinctSupportDays >= 2 || summary.distinctSupportSources >= 2 || belief.useCount >= 2)) {
			const selfConsistencyValue = clamp01(summary.positiveAverage * .44 + Math.min(summary.positiveSignals.length / 4, 1) * .2 + Math.min(summary.distinctSupportDays / 3, 1) * .22 + Math.min(summary.distinctSupportSources / 3, 1) * .14 - summary.contradictionAverage * .22);
			if (selfConsistencyValue > .25) recordSignal(store, ctx, {
		...
		if (ageDays >= windowDays && summary.contradictionAverage < .35 && belief.stage !== "candidate" && (summary.positiveSignals.length >= 2 || summary.distinctSupportDays >= 2 || belief.useCount >= 3)) {
			const temporalValue = clamp01(Math.min(ageDays / Math.max(windowDays * 2, 1), 1) * .44 + summary.positiveAverage * .22 + Math.min(summary.distinctSupportDays / 3, 1) * .12 + (1 - summary.contradictionAverage) * .14 + (belief.stage === "active" ? .08 : belief.stage === "probationary" ? .04 : 0));
			if (temporalValue > .24) recordSignal(store, ctx, {
				signalId: maintenanceSignalId({
					agentId: ctx.agentId,
					beliefId: belief.beliefId,
...
//#endregion
export { emitAssistantOutcomeLearningSignals, emitBackgroundRetrievalSignals, emitBeliefMaintenanceSignals, emitContradictionSignals, emitFullRetrievalSignals, emitOutcomeFeedbackSignal, emitWriteMaterializationSignals };
