//#region src/pipeline/messageText.ts
...
function stripInboundMetadata(text) {
	let cleaned = stripEnvelopePrefix(text).replace(/\[message_id:\s*[a-f0-9-]+\]/gi, "").replace(/\[\[reply_to_current\]\]/gi, "").trim();
	if (!META_FAST_RE.test(cleaned)) return cleaned;
...
//#endregion
export { readMessageText, stripInboundMetadata };
