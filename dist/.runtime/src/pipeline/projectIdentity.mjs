import { normalizeName } from "../support.mjs";
//#region src/pipeline/projectIdentity.ts
...
function stripProjectDescriptorTail(value, options = {}) {
	...
	if (allowLatinDescriptorTail) {
		...
		if (parts.length > 1) {
			...
			while (reducedParts.length > 1 && isLatinProjectHeadword(reducedParts.at(-1))) reducedParts.pop();
			if (reducedParts.length !== parts.length) return reducedParts.join(" ");
		}
	}
	const compact = normalized.replace(/\s+/g, "");
	if (allowHanDescriptorTail) {
		const mixedHanSuffix = compact.match(/^([a-z0-9_.-]{2,}?)([\p{Script=Han}]{2,8})$/iu);
		if (mixedHanSuffix && HAN_PROJECT_HEADWORDS.some((suffix) => mixedHanSuffix[2].endsWith(suffix))) return mixedHanSuffix[1];
	}
	if (allowLatinDescriptorTail) {
	...
	if (allowHanDescriptorTail) {
		...
		if (separatedHanTail && HAN_PROJECT_HEADWORDS.some((suffix) => separatedHanTail[2].endsWith(suffix))) {
...
function compactProjectIdentity(value, options = {}) {
	return (stripProjectDescriptorTail(value, options) || normalizeName(value)).replace(/\s+/g, "");
}
...
function projectAliasEquivalent(left, right) {
	...
	for (const alias of leftAliases) if (alias && rightAliases.has(alias)) return true;
	return false;
}
...
function projectNamesMatch(left, right) {
...
function looksLikeProjectDescriptor(value) {
	...
	if (isLatinProjectHeadword(normalized)) return true;
	if (HAN_PROJECT_HEADWORDS.includes(normalized)) return true;
	const compact = normalized.replace(/\s+/g, "");
	return [...LATIN_PROJECT_HEADWORDS].some((suffix) => compact.toLowerCase() === suffix) || HAN_PROJECT_HEADWORDS.some((suffix) => compact === suffix);
}
function projectAliasVariants(value) {
	return buildProjectAliasVariants(value, {
		allowLatinDescriptorTail: true,
		allowHanDescriptorTail: true
	});
}
function resolveProjectReference(value, params) {
	const trimmed = value.trim();
	if (!trimmed) return trimmed;
	const knownProjects = [...new Set((params.knownProjects ?? []).map((entry) => entry.trim()).filter(Boolean))];
	const exact = knownProjects.find((entry) => projectNamesMatch(entry, trimmed));
	if (exact) return exact;
	const currentProject = params.currentProject?.trim();
	if (currentProject && projectNamesMatch(currentProject, trimmed)) return currentProject;
	if (params.allowDescriptorAlias) {
		if (currentProject && projectAliasEquivalent(currentProject, trimmed)) return currentProject;
		if (knownProjects.length === 1 && projectAliasEquivalent(knownProjects[0], trimmed)) return knownProjects[0];
		if (currentProject && looksLikeProjectDescriptor(trimmed)) return currentProject;
		if (knownProjects.length === 1 && looksLikeProjectDescriptor(trimmed)) return knownProjects[0];
	}
	return trimmed;
}
function isProjectProfileStateKey(key) {
	return key.startsWith("project.") && key !== "project.active_project";
}
function projectCodeFromStateKey(key) {
	if (!isProjectProfileStateKey(key)) return;
	return key.slice(8).trim() || void 0;
}
//#endregion
export { isProjectProfileStateKey, looksLikeProjectDescriptor, projectAliasVariants, projectCodeFromStateKey, projectIdentityKey, projectNamesMatch, resolveProjectReference };
