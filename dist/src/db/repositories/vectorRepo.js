...
export class VectorRepo {
    ...
    keywordSearch(params) {
        ...
        const rows = this.db
            .prepare(`SELECT doc_id, text, metadata_json, rank
           FROM (
    ...
    upsertEmbedding(params) {
        this.db
            .prepare(`INSERT INTO vector_embeddings(doc_id, agent_id, scope, dimensions, embedding_json, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)
         ON CONFLICT(doc_id) DO UPDATE SET
           agent_id = excluded.agent_id,
           scope = excluded.scope,
           ...
           updated_at = excluded.updated_at`)
            .run(params.docId, params.agentId, params.scope, params.embedding.length, JSON.stringify(params.embedding), params.updatedAt);
    }
    listEmbeddings(params) {
        ...
        this.appendDocFilters(whereClauses, values, params);
        const rows = this.db
            .prepare(`SELECT vector_embeddings.doc_id, vector_embeddings.scope, vector_embeddings.embedding_json, vector_embeddings.updated_at
    ...
    getDoc(docId, readEpoch) {
        const row = this.db
            .prepare(`SELECT doc_id, doc_kind, source_id, scope, agent_id, text, metadata_json, created_at, updated_at, materialized_epoch
           FROM vector_docs
          WHERE doc_id = ?
            ${typeof readEpoch === "number" ? "AND materialized_epoch <= ?" : ""}`)
            .get(docId, ...(typeof readEpoch === "number" ? [readEpoch] : []));
        return row
            ? {
                docId: row.doc_id,
                docKind: row.doc_kind,
