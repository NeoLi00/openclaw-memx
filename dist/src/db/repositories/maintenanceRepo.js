...
function toStateRecord(row) {
    return {
        ...
        inflightReason: row.inflight_reason === "threshold" ||
...
export class MaintenanceRepo {
    ...
    getState(agentId, sessionKey) {
        const row = this.db
            .prepare(`SELECT agent_id, session_key, pending_turn_count, pending_turn_ids_json,
    ...
    recordPendingTurn(params) {
        ...
        return this.db.withTransaction(() => {
            const current = this.db
              ...
              WHERE agent_id = ?
                AND session_key = ?`)
                .get(params.agentId, params.sessionKey) ??
                rowDefaults({
            ...
            const next = {
                ...
                first_pending_observed_at: minIso([current.first_pending_observed_at ?? undefined, params.observedAt]) ?? null,
                last_pending_observed_at: maxIso([current.last_pending_observed_at ?? undefined, params.observedAt]) ?? null,
    ...
    claimBatch(params) {
        return this.db.withTransaction(() => {
            ...
            let working = { ...current };
            const leaseExpired = working.status === "running" &&
        ...
        return this.db.withTransaction(() => {
            ...
            if (params.success) {
                next = {
                    ...
                    lease_owner: null,
                    lease_expires_at: null,
                    last_event_watermark: maxIso([current.last_event_watermark ?? undefined, params.upperWatermarks?.event]) ?? null,
                    last_signal_watermark: maxIso([current.last_signal_watermark ?? undefined, params.upperWatermarks?.signal]) ?? null,
                    last_task_watermark: maxIso([current.last_task_watermark ?? undefined, params.upperWatermarks?.task]) ?? null,
    ...
    persistRow(row) {
        this.db
            .prepare(`INSERT INTO maintenance_scheduler_state(
