...
export class EventRepo {
    ...
    get(eventId) {
        const row = this.db
            .prepare(`SELECT event_id, agent_id, scope, event_type, text, normalized_text, observed_at, valid_from,
    ...
    findNearDuplicate(params) {
        const row = this.db
          ...
          LIMIT 1`)
            .get(params.agentId, params.scope, params.normalizedText, params.observedAfter);
        return row ? this.toEvent(row) : null;
    }
    append(event) {
        this.db
            .prepare(`INSERT OR REPLACE INTO episodic_events(
          event_id, agent_id, scope, event_type, text, normalized_text, observed_at, valid_from,
          valid_to, source_kind, source_ref, session_key, tool_name, confidence, metadata_json, materialized_epoch
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(event.eventId, event.agentId, event.scope, event.eventType, event.text, event.normalizedText, event.observedAt, event.validFrom ?? null, event.validTo ?? null, event.sourceKind, event.sourceRef, event.sessionKey ?? null, event.toolName ?? null, event.confidence, JSON.stringify(event.metadataJson), event.materializedEpoch ?? 0);
    }
    search(params) {
