...
export class AuditRepo {
    ...
    recordPolicyDecision(params) {
        this.db
            .prepare(`INSERT INTO policy_decisions(
          decision_id, agent_id, source_ref, candidate_hash, salience_score, utility_score, chosen_action, reasons_json, created_at, metadata_json
            ...
            .run(randomId("decision"), params.agentId, params.sourceRef, stableHash([params.candidateText]), params.decision.salienceScore, params.decision.expectedFutureUtility, params.decision.action, JSON.stringify(params.decision.reasons), params.createdAt, JSON.stringify(params.metadataJson ?? {}));
    }
    recordRetrieval(audit) {
        this.db
            .prepare(`INSERT INTO retrieval_audit(
          audit_id, agent_id, scope, route_type, query_text, query_hash, selected_items_json, injected_chars, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(audit.auditId, audit.agentId, audit.scope, audit.routeType, audit.queryText, audit.queryHash, JSON.stringify(audit.selectedItemsJson), audit.injectedChars, audit.createdAt);
    }
    recordSignal(signal) {
        this.db
            .prepare(`INSERT OR IGNORE INTO memory_signal_events(
          signal_id, agent_id, scope, session_key, signal_type, memory_kind, content_ref, semantic_key, value, source_ref, metadata_json, created_at
            ...
            .run(signal.signalId, signal.agentId, signal.scope, signal.sessionKey ?? null, signal.signalType, signal.memoryKind, signal.contentRef ?? null, signal.semanticKey, signal.value, signal.sourceRef, JSON.stringify(signal.metadataJson), signal.createdAt);
    }
    listSignals(params) {
    ...
    startMaintenance(params) {
        ...
        this.db
            .prepare(`INSERT INTO maintenance_runs(
    ...
    finishMaintenance(run) {
        this.db
            .prepare(`UPDATE maintenance_runs
