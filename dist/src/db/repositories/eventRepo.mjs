...
//#region src/db/repositories/eventRepo.ts
...
function structuredEventSummary(metadataJson) {
...
var EventRepo = class {
	...
	get(eventId) {
		const row = this.db.prepare(`SELECT event_id, agent_id, scope, event_type, text, normalized_text, observed_at, valid_from,
	...
	findNearDuplicate(params) {
		const row = this.db.prepare(`SELECT event_id, agent_id, scope, event_type, text, normalized_text, observed_at, valid_from,
          ...
          LIMIT 1`).get(params.agentId, params.scope, params.normalizedText, params.observedAfter);
		return row ? this.toEvent(row) : null;
	}
	append(event) {
		this.db.prepare(`INSERT OR REPLACE INTO episodic_events(
          event_id, agent_id, scope, event_type, text, normalized_text, observed_at, valid_from,
          valid_to, source_kind, source_ref, session_key, tool_name, confidence, metadata_json, materialized_epoch
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(event.eventId, event.agentId, event.scope, event.eventType, event.text, event.normalizedText, event.observedAt, event.validFrom ?? null, event.validTo ?? null, event.sourceKind, event.sourceRef, event.sessionKey ?? null, event.toolName ?? null, event.confidence, JSON.stringify(event.metadataJson), event.materializedEpoch ?? 0);
	}
	search(params) {
    ...
    `;
		...
		const events = this.db.prepare(sql).all(...values).map((row) => this.toEvent(row));
		if (!params.text) return events.slice(0, params.limit ?? events.length);
		const terms = tokenizeSearch(params.text);
		if (terms.length === 0) return events.slice(0, params.limit ?? events.length);
		return events.map((event) => {
			const haystack = `${event.text} ${event.normalizedText} ${event.eventType} ${structuredEventSummary(event.metadataJson)}`.toLowerCase();
			return {
				event,
		...
		}).slice(0, params.limit ?? events.length).map((entry) => entry.event);
	}
	delete(params) {
		...
		const result = this.db.prepare(`DELETE FROM episodic_events WHERE ${clauses.join(" AND ")}`).run(...values);
    ...
    `;
		...
		return this.db.prepare(sql).get(...values)?.observedAt ?? void 0;
	}
	toEvent(row) {
		return {
