...
export class GraphRepo {
    ...
    upsertEntity(entity) {
        ...
        ON CONFLICT(entity_id) DO UPDATE SET
          ...
          updated_at = excluded.updated_at`)
            .run(entity.entityId, entity.canonicalName, entity.entityType, entity.normalizedName, JSON.stringify(aliases), entity.confidence, new Date().toISOString(), new Date().toISOString());
        for (const alias of aliases) {
            this.db
                .prepare(`INSERT OR IGNORE INTO entity_aliases(alias_id, entity_id, alias_text, normalized_alias)
           VALUES (?, ?, ?, ?)`)
    ...
    lookupEntityByName(name, preferredType) {
        const normalized = normalizeName(name);
        const row = this.db
            .prepare(`SELECT entity_id, canonical_name, entity_type, normalized_name, aliases_json, confidence, created_at, updated_at
           FROM entities
        ...
        const fuzzy = projectRows.find((candidate) => {
            ...
            const aliases = safeJsonParse(candidate.aliases_json, []);
    ...
    lookupEntityByNormalizedName(normalizedName, preferredType) {
        const row = this.db
          ...
          ORDER BY
            ...
            updated_at DESC
          LIMIT 1`)
            .get(normalizedName, preferredType ?? "");
        if (!row) {
            return null;
        }
    ...
    lookupEntityByAlias(normalizedAlias, preferredType) {
        ...
        const row = this.db
            .prepare(`SELECT e.entity_id, e.canonical_name, e.entity_type, e.normalized_name, e.aliases_json, e.confidence, e.created_at, e.updated_at
           FROM entities e
          ...
          ORDER BY
            ...
            e.updated_at DESC
          LIMIT 1`)
            .get(aliasKey, aliasKey, preferredType ?? "");
        if (!row) {
            return null;
        }
        if (!preferredType || row.entity_type === preferredType || row.entity_type === "unknown") {
    ...
    listResolvedMentionsByNormalized(params) {
        const rows = this.db
            .prepare(`SELECT mention_id, agent_id, scope, raw_text, normalized_text, proposed_type, semantic_role, source_ref,
          ...
          LIMIT ?`)
            .all(params.agentId, params.scope, params.normalizedText, ...(params.sessionKey ? [params.sessionKey] : []), params.limit ?? 8);
        return rows.map((row) => this.toEntityMention(row));
    }
    upsertEntityMention(mention) {
        this.db
            .prepare(`INSERT INTO entity_mentions(
    ...
    upsertEntityAliasSource(params) {
        ...
        this.db
            .prepare(`INSERT INTO entity_alias_sources(
        ...
        ON CONFLICT(alias_source_id) DO UPDATE SET
          ...
          metadata_json = excluded.metadata_json`)
            .run(aliasSourceId, params.entityId, params.aliasText, normalizedAlias, params.sourceRef, params.confidence, params.createdAt, JSON.stringify(params.metadataJson ?? {}));
    }
    upsertIdentityLink(params) {
        ...
        this.db
            .prepare(`INSERT INTO entity_identity_links(
        ...
        ON CONFLICT(link_id) DO UPDATE SET
          ...
          updated_at = excluded.updated_at,
          metadata_json = excluded.metadata_json`)
            .run(linkId, params.srcEntityId, params.dstEntityId, params.linkType, params.confidence, params.evidenceRef, params.status ?? "active", params.at, params.at, JSON.stringify(params.metadataJson ?? {}));
    }
    linkedEntityIds(params) {
        ...
        const rows = this.db
          ...
          ORDER BY confidence DESC, updated_at DESC
          LIMIT ?`)
            .all(params.entityId, params.status ?? "active", ...linkTypes, params.entityId, params.entityId, params.limit ?? 8);
        return [...new Set(rows.map((row) => row.entity_id).filter(Boolean))];
    }
    findSupersedingEntity(entityId) {
        const row = this.db
            .prepare(`SELECT e.entity_id, e.canonical_name, e.entity_type, e.normalized_name, e.aliases_json,
    ...
    listMentionsForEntity(params) {
        const rows = this.db
          ...
          LIMIT ?`)
            .all(params.entityId, params.limit ?? 8);
        return rows.map((row) => this.toEntityMention(row));
    }
    listAliasSourcesForEntity(params) {
        const rows = this.db
            .prepare(`SELECT alias_source_id, entity_id, alias_text, normalized_alias, source_ref, confidence, created_at, metadata_json
    ...
    upsertEdge(edge) {
        const existing = this.db
            .prepare(`SELECT edge_id, src_entity_id, rel_type, dst_entity_id, scope, agent_id, confidence, valid_from, valid_to,
          ...
          LIMIT 1`)
            .get(edge.srcEntityId, edge.relType, edge.dstEntityId, edge.scope, edge.agentId, edge.relationSlot ?? null);
        if (existing) {
            ...
            WHERE edge_id = ?`)
                .run(Math.max(existing.confidence, edge.confidence), edge.validFrom ?? null, edge.validTo ?? existing.valid_to, edge.evidenceRef, edge.relationSlot ?? existing.relation_slot, edge.updatedAt, edge.materializedEpoch ?? existing.materialized_epoch, JSON.stringify({
                ...existingMetadata,
                ...incomingMetadata,
                sourceRefs,
                supportRefs,
        ...
        if (edge.relationSlot) {
            ...
            WHERE src_entity_id = ?
              ...
              AND (valid_to IS NULL OR valid_to > ?)`)
                .run(edge.validFrom ?? edge.updatedAt, edge.updatedAt, edge.srcEntityId, edge.relType, edge.scope, edge.agentId, edge.relationSlot, edge.dstEntityId, edge.validFrom ?? edge.updatedAt);
        }
        this.db
            .prepare(`INSERT INTO graph_edges(
        ...
        ON CONFLICT(edge_id) DO UPDATE SET
          ...
          metadata_json = excluded.metadata_json`)
            .run(edge.edgeId, edge.srcEntityId, edge.relType, edge.dstEntityId, edge.relationSlot ?? null, edge.scope, edge.agentId, edge.confidence, edge.validFrom ?? null, edge.validTo ?? null, edge.evidenceRef, edge.createdAt, edge.updatedAt, edge.materializedEpoch ?? 0, JSON.stringify(edge.metadataJson ?? {}));
    ...
    closeActiveSlotEdges(params) {
        const result = this.db
          ...
          WHERE src_entity_id = ?
            ...
            AND COALESCE(relation_slot, '') = COALESCE(?, '')
            AND (valid_to IS NULL OR valid_to > ?)`)
            .run(params.validTo, params.validTo, params.srcEntityId, params.relType, params.scope, params.agentId, params.relationSlot, params.validTo);
    ...
    expandNeighborhood(params) {
        ...
        for (let hop = 0; hop < params.maxHops; hop += 1) {
            ...
            const rows = this.db
                .prepare(`SELECT edge_id, src_entity_id, rel_type, dst_entity_id, scope, agent_id, confidence, valid_from,
            ...
            LIMIT ${Math.max(params.maxEdges * 2, 16)}`)
                .all(params.agentId, ...params.scopes, params.now ?? new Date().toISOString(), ...(typeof params.readEpoch === "number" ? [params.readEpoch] : []), ...frontier, ...frontier);
            frontier = [];
    ...
    pruneLowConfidence(params) {
        const result = this.db
            .prepare(`DELETE FROM graph_edges
          WHERE agent_id = ?
            AND confidence <= ?
            AND updated_at < ?`)
