...
var VectorRepo = class {
	...
	appendDocFilters(clauses, values, params, tablePrefix = "vector_docs") {
		...
		if (params.docTypes && params.docTypes.length > 0) {
			...
			clauses.push(`COALESCE(json_extract(${tablePrefix}.metadata_json, '$.memxDocType'), ${tablePrefix}.doc_kind) IN (${placeholders})`);
			values.push(...params.docTypes);
		}
	}
	upsertDocs(docs) {
		const stmt = this.db.prepare(`INSERT INTO vector_docs(
      ...
      ON CONFLICT(doc_id) DO UPDATE SET
		...
		const ftsInsert = this.db.prepare(`INSERT INTO vector_docs_fts(doc_id, doc_kind, source_id, scope, agent_id, text)
       VALUES (?, ?, ?, ?, ?, ?)`);
	...
	keywordSearch(params) {
		if (!params.query.trim() || params.scopes.length === 0) return [];
		const whereClauses = [
			"vector_docs.agent_id = ?",
			`vector_docs.scope IN (${params.scopes.map(() => "?").join(", ")})`,
		...
		return this.db.prepare(`SELECT doc_id, text, metadata_json, rank
           FROM (
          ...
          ORDER BY rank ASC
          LIMIT ${Math.max(1, params.limit)}`).all(...values, params.query).map((row) => ({
			docId: row.doc_id,
			text: row.text,
	...
	listDocs(params) {
		...
		const whereClauses = [
			"agent_id = ?",
			`scope IN (${params.scopes.map(() => "?").join(", ")})`,
	...
	upsertEmbedding(params) {
		this.db.prepare(`INSERT INTO vector_embeddings(doc_id, agent_id, scope, dimensions, embedding_json, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)
         ON CONFLICT(doc_id) DO UPDATE SET
           agent_id = excluded.agent_id,
           scope = excluded.scope,
           ...
           updated_at = excluded.updated_at`).run(params.docId, params.agentId, params.scope, params.embedding.length, JSON.stringify(params.embedding), params.updatedAt);
	}
	listEmbeddings(params) {
		if (params.scopes.length === 0) return [];
		const whereClauses = [
			"vector_embeddings.agent_id = ?",
			`vector_embeddings.scope IN (${params.scopes.map(() => "?").join(", ")})`,
			...typeof params.readEpoch === "number" ? ["vector_docs.materialized_epoch <= ?"] : []
		];
		const values = [
			params.agentId,
		...
		this.appendDocFilters(whereClauses, values, params);
		return this.db.prepare(`SELECT vector_embeddings.doc_id, vector_embeddings.scope, vector_embeddings.embedding_json, vector_embeddings.updated_at
           FROM vector_embeddings
           JOIN vector_docs ON vector_docs.doc_id = vector_embeddings.doc_id
          WHERE ${whereClauses.join("\n            AND ")}
          ORDER BY vector_embeddings.updated_at DESC
          LIMIT ${Math.max(1, params.limit)}`).all(...values).map((row) => ({
			docId: row.doc_id,
			scope: row.scope,
	...
	getDoc(docId, readEpoch) {
		const row = this.db.prepare(`SELECT doc_id, doc_kind, source_id, scope, agent_id, text, metadata_json, created_at, updated_at, materialized_epoch
           FROM vector_docs
          WHERE doc_id = ?
            ${typeof readEpoch === "number" ? "AND materialized_epoch <= ?" : ""}`).get(docId, ...typeof readEpoch === "number" ? [readEpoch] : []);
		return row ? {
			docId: row.doc_id,
			docKind: row.doc_kind,
			sourceId: row.source_id,
