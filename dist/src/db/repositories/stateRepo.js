...
export class StateRepo {
    ...
    upsert(state) {
        ...
        ON CONFLICT(agent_id, scope, key) DO UPDATE SET
          ...
          materialized_epoch = excluded.materialized_epoch`)
            .run(state.key, JSON.stringify(state.valueJson), state.scope, state.agentId, state.stateKind, state.confidence, state.sourceRef, state.updatedAt, state.expiresAt ?? null, state.materializedEpoch ?? 0);
    }
    get(params) {
    ...
    expireSessionStates(agentId, now) {
        const result = this.db
            .prepare("DELETE FROM state_kv WHERE agent_id = ? AND expires_at IS NOT NULL AND expires_at <= ?")
            .run(agentId, now);
        return Number(result.changes ?? 0);
    }
