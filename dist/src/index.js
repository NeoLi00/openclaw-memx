...
import { buildBackgroundRecallBundle, hasBackgroundRecallMaterial, retrieveEvidence, } from "./pipeline/retrieve.js";
import { sanitizeFocusedRecallQuery, summarizeBackgroundRecallBundle, } from "./pipeline/retrieveTracing.js";
...
import { buildOperationContext, MemxRuntimeManager } from "./runtime.js";
...
function fitPromptBudget(parts, maxChars) {
...
function hasStrongProtectedPromptEvidence(bundle) {
    return bundle.evidencePackets.some((packet) => packet.injected &&
        !packet.dropReason &&
        packet.coverage.filled &&
        packet.coverage.confidence >= 0.5);
}
function shouldUseCompactEvidencePrompt(bundle) {
    return (hasStrongProtectedPromptEvidence(bundle) &&
...
export function evidencePlanRuleLines(bundle) {
...
function evidencePacketPromptLine(packet) {
    ...
    const quantity = typeof packet.quantityHint === "number"
        ...
        : "";
    const support = packet.supportingTexts.length > 0
...
function buildNoRecallHint(config, queryAnalysis) {
    ...
    if (shouldSuggestExplicitRecallTool(config) && queryAnalysis.focusedQuery) {
        lines.push(`If prior context, user preference, or past work likely matters, call memory_recall with a short focused query such as: "${queryAnalysis.focusedQuery}".`);
    }
    else if (shouldSuggestExplicitRecallTool(config)) {
        lines.push("If prior context likely matters, call memory_recall with a short focused query you generate yourself.");
    }
    return lines.join("\n");
}
...
function buildMemxImplicitRecallPrompt(config, bundle, background, queryAnalysis, maxChars) {
    ...
    const packetLimit = queryAnalysis.evidencePlan?.operation.type === "derive" ||
    ...
    const followUpSection = compactEvidencePrompt
        ? null
...
function extractPromptQuery(event) {
    ...
    const promptCandidate = (() => {
        ...
        if (lastDoubleNewline > 0 && lastDoubleNewline < prompt.length - 3) {
            return stripInboundMetadata(stripInjectedHistoricalBlock(prompt.slice(lastDoubleNewline + 2).trim()));
        }
    ...
    const best = userCandidates
        ...
        const containment = candidate.includes(promptCandidate) || promptCandidate.includes(candidate) ? 0.95 : 0;
        return {
            candidate,
...
export function createMemoryMemxPlugin() {
    return {
        id: "memory-memx",
        name: "Memory (MemX)",
        ...
        register(api) {
            ...
            api.registerTool((toolCtx) => createMemxTools({ toolCtx, config, manager, logger: api.logger }), {
                names: [
            ...
            const recallHandler = async (event, ctx) => {
                ...
                try {
                    ...
                    if (!hasRecallMaterial(bundle)) {
                        if (hasBackgroundRecallMaterial(backgroundBundle)) {
                            ...
                            return cacheRecallResult({
                                systemPrompt: _logSystemPrompt(api.logger, "full-no-material-bg", buildBackgroundRecallPrompt(config, promptBackground, rawQuery, queryAnalysis, config.maxInjectedChars), {
                                    agentId: ctx.agentId,
                                    sessionKey: ctx.sessionKey,
                                    runId: ctx.runId,
                                }),
                        ...
                        return cacheRecallResult({
                            systemPrompt: _logSystemPrompt(api.logger, "full-no-material", buildNoRecallHint(config, queryAnalysis), {
                                agentId: ctx.agentId,
                                sessionKey: ctx.sessionKey,
                                runId: ctx.runId,
                            }),
                    ...
                    return cacheRecallResult({
                        systemPrompt: _logSystemPrompt(api.logger, "full-recall", buildMemxImplicitRecallPrompt(config, bundle, promptBackground, queryAnalysis, config.maxInjectedChars), {
                            agentId: ctx.agentId,
                            sessionKey: ctx.sessionKey,
                            runId: ctx.runId,
                        }),
