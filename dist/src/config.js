...
import { MEMORY_CONSENT_MODES, MEMORY_EMBEDDING_PROVIDERS, MEMORY_PII_MODES, MEMORY_SCOPE_TEMPLATES, } from "./types.js";
...
function parseConfigInternal(input) {
    ...
    const embedding = {
        ...
        dimensions: typeof rawEmbedding.dimensions === "number" && Number.isFinite(rawEmbedding.dimensions)
            ? Math.trunc(rawEmbedding.dimensions)
            : undefined,
        headers: isRecord(rawEmbedding.headers)
            ? Object.fromEntries(Object.entries(rawEmbedding.headers)
                .filter(([, value]) => typeof value === "string")
                .map(([key, value]) => [key, String(value)]))
            : undefined,
        ollamaBaseURL: typeof rawEmbedding.ollamaBaseURL === "string"
            ? rawEmbedding.ollamaBaseURL.trim()
            : undefined,
        ollamaModel: typeof rawEmbedding.ollamaModel === "string" ? rawEmbedding.ollamaModel.trim() : undefined,
        localPythonBin: typeof rawEmbedding.localPythonBin === "string"
            ? rawEmbedding.localPythonBin.trim()
            : undefined,
        localCacheDir: typeof rawEmbedding.localCacheDir === "string"
            ? rawEmbedding.localCacheDir.trim()
            : undefined,
        localDevice: rawEmbedding.localDevice === "auto" ||
    ...
    const advanced = {
        ...
        llmClassifierModel: typeof rawAdvanced.llmClassifierModel === "string"
        ...
        candidateSurfaceBudgets: {
            state: asNumber(rawAdvanced.candidateSurfaceBudgets?.state, DEFAULT_MEMORY_CONFIG.advanced.candidateSurfaceBudgets.state, issues, "advanced.candidateSurfaceBudgets.state", { min: 0, max: 16 }),
            fact: asNumber(rawAdvanced.candidateSurfaceBudgets?.fact, DEFAULT_MEMORY_CONFIG.advanced.candidateSurfaceBudgets.fact, issues, "advanced.candidateSurfaceBudgets.fact", { min: 0, max: 24 }),
            event: asNumber(rawAdvanced.candidateSurfaceBudgets?.event, DEFAULT_MEMORY_CONFIG.advanced.candidateSurfaceBudgets.event, issues, "advanced.candidateSurfaceBudgets.event", { min: 0, max: 24 }),
            task: asNumber(rawAdvanced.candidateSurfaceBudgets?.task, DEFAULT_MEMORY_CONFIG.advanced.candidateSurfaceBudgets.task, issues, "advanced.candidateSurfaceBudgets.task", { min: 0, max: 16 }),
            chunk: asNumber(rawAdvanced.candidateSurfaceBudgets?.chunk, DEFAULT_MEMORY_CONFIG.advanced.candidateSurfaceBudgets.chunk, issues, "advanced.candidateSurfaceBudgets.chunk", { min: 0, max: 24 }),
            graph: asNumber(rawAdvanced.candidateSurfaceBudgets?.graph, DEFAULT_MEMORY_CONFIG.advanced.candidateSurfaceBudgets.graph, issues, "advanced.candidateSurfaceBudgets.graph", { min: 0, max: 16 }),
            entityAlias: asNumber(rawAdvanced.candidateSurfaceBudgets?.entityAlias, DEFAULT_MEMORY_CONFIG.advanced.candidateSurfaceBudgets.entityAlias, issues, "advanced.candidateSurfaceBudgets.entityAlias", { min: 0, max: 16 }),
        },
        chunkDedupThreshold: asNumber(rawAdvanced.chunkDedupThreshold, DEFAULT_MEMORY_CONFIG.advanced.chunkDedupThreshold, issues, "advanced.chunkDedupThreshold", { min: 0, max: 1 }),
        taskIdleTimeoutMinutes: asNumber(rawAdvanced.taskIdleTimeoutMinutes, DEFAULT_MEMORY_CONFIG.advanced.taskIdleTimeoutMinutes, issues, "advanced.taskIdleTimeoutMinutes", { min: 1, max: 24 * 60 }),
        recallChunkBudget: asNumber(rawAdvanced.recallChunkBudget, DEFAULT_MEMORY_CONFIG.advanced.recallChunkBudget, issues, "advanced.recallChunkBudget", { min: 1, max: 32 }),
        recallTotalObjectBudget: asNumber(rawAdvanced.recallTotalObjectBudget, DEFAULT_MEMORY_CONFIG.advanced.recallTotalObjectBudget, issues, "advanced.recallTotalObjectBudget", { min: 4, max: 128 }),
        recallBackgroundCharReserve: asNumber(rawAdvanced.recallBackgroundCharReserve, DEFAULT_MEMORY_CONFIG.advanced.recallBackgroundCharReserve, issues, "advanced.recallBackgroundCharReserve", { min: 0, max: 6000 }),
        recallPromptBudgetFloor: asNumber(rawAdvanced.recallPromptBudgetFloor, DEFAULT_MEMORY_CONFIG.advanced.recallPromptBudgetFloor, issues, "advanced.recallPromptBudgetFloor", { min: 0, max: 4000 }),
        recallObjectiveMinWeight: asNumber(rawAdvanced.recallObjectiveMinWeight, DEFAULT_MEMORY_CONFIG.advanced.recallObjectiveMinWeight, issues, "advanced.recallObjectiveMinWeight", { min: 0, max: 1 }),
        recallObjectiveOverflowRatio: asNumber(rawAdvanced.recallObjectiveOverflowRatio, DEFAULT_MEMORY_CONFIG.advanced.recallObjectiveOverflowRatio, issues, "advanced.recallObjectiveOverflowRatio", { min: 0, max: 1 }),
        recallProbeWorkflowStrongThreshold: asNumber(rawAdvanced.recallProbeWorkflowStrongThreshold, DEFAULT_MEMORY_CONFIG.advanced.recallProbeWorkflowStrongThreshold, issues, "advanced.recallProbeWorkflowStrongThreshold", { min: 0, max: 1 }),
        recallProbeWorkflowContinuationThreshold: asNumber(rawAdvanced.recallProbeWorkflowContinuationThreshold, DEFAULT_MEMORY_CONFIG.advanced.recallProbeWorkflowContinuationThreshold, issues, "advanced.recallProbeWorkflowContinuationThreshold", { min: 0, max: 1 }),
        recallProbeFactualStrongThreshold: asNumber(rawAdvanced.recallProbeFactualStrongThreshold, DEFAULT_MEMORY_CONFIG.advanced.recallProbeFactualStrongThreshold, issues, "advanced.recallProbeFactualStrongThreshold", { min: 0, max: 1 }),
        recallProbeFactualShortQueryThreshold: asNumber(rawAdvanced.recallProbeFactualShortQueryThreshold, DEFAULT_MEMORY_CONFIG.advanced.recallProbeFactualShortQueryThreshold, issues, "advanced.recallProbeFactualShortQueryThreshold", { min: 0, max: 1 }),
        recallProbeHybridStrongThreshold: asNumber(rawAdvanced.recallProbeHybridStrongThreshold, DEFAULT_MEMORY_CONFIG.advanced.recallProbeHybridStrongThreshold, issues, "advanced.recallProbeHybridStrongThreshold", { min: 0, max: 1 }),
        recallProbeHybridModerateThreshold: asNumber(rawAdvanced.recallProbeHybridModerateThreshold, DEFAULT_MEMORY_CONFIG.advanced.recallProbeHybridModerateThreshold, issues, "advanced.recallProbeHybridModerateThreshold", { min: 0, max: 1 }),
        recallProbeEscalateThreshold: asNumber(rawAdvanced.recallProbeEscalateThreshold, DEFAULT_MEMORY_CONFIG.advanced.recallProbeEscalateThreshold, issues, "advanced.recallProbeEscalateThreshold", { min: 0, max: 1 }),
        recallProbeContinuationEscalateThreshold: asNumber(rawAdvanced.recallProbeContinuationEscalateThreshold, DEFAULT_MEMORY_CONFIG.advanced.recallProbeContinuationEscalateThreshold, issues, "advanced.recallProbeContinuationEscalateThreshold", { min: 0, max: 1 }),
    };
    ...
    const value = {
        ...
        maxInjectedChars: asNumber(rawRoot.maxInjectedChars, DEFAULT_MEMORY_CONFIG.maxInjectedChars, issues, "maxInjectedChars", { min: 200, max: 12000 }),
        captureMaxChars: asNumber(rawRoot.captureMaxChars, DEFAULT_MEMORY_CONFIG.captureMaxChars, issues, "captureMaxChars", { min: 100, max: 10000 }),
        reflectionMaxChars: asNumber(rawRoot.reflectionMaxChars, DEFAULT_MEMORY_CONFIG.reflectionMaxChars, issues, "reflectionMaxChars", { min: 100, max: 4000 }),
        reflectionMaxItems: asNumber(rawRoot.reflectionMaxItems, DEFAULT_MEMORY_CONFIG.reflectionMaxItems, issues, "reflectionMaxItems", { min: 1, max: 32 }),
        ...
        minSalienceDurable: asNumber(rawRoot.minSalienceDurable, DEFAULT_MEMORY_CONFIG.minSalienceDurable, issues, "minSalienceDurable", { min: 0, max: 1 }),
        minSalienceSession: asNumber(rawRoot.minSalienceSession, DEFAULT_MEMORY_CONFIG.minSalienceSession, issues, "minSalienceSession", { min: 0, max: 1 }),
        minUtilityForGraph: asNumber(rawRoot.minUtilityForGraph, DEFAULT_MEMORY_CONFIG.minUtilityForGraph, issues, "minUtilityForGraph", { min: 0, max: 1 }),
        maxSensitivityAllowed: asNumber(rawRoot.maxSensitivityAllowed, DEFAULT_MEMORY_CONFIG.maxSensitivityAllowed, issues, "maxSensitivityAllowed", { min: 0, max: 1 }),
        stateTtlHours: asNumber(rawRoot.stateTtlHours, DEFAULT_MEMORY_CONFIG.stateTtlHours, issues, "stateTtlHours", { min: 1, max: 24 * 365 }),
        episodicDedupWindowDays: asNumber(rawRoot.episodicDedupWindowDays, DEFAULT_MEMORY_CONFIG.episodicDedupWindowDays, issues, "episodicDedupWindowDays", { min: 1, max: 365 }),
        graphMaxHops: asNumber(rawRoot.graphMaxHops, DEFAULT_MEMORY_CONFIG.graphMaxHops, issues, "graphMaxHops", { min: 1, max: 4 }),
        maxGraphNodes: asNumber(rawRoot.maxGraphNodes, DEFAULT_MEMORY_CONFIG.maxGraphNodes, issues, "maxGraphNodes", { min: 1, max: 64 }),
        maxGraphEdges: asNumber(rawRoot.maxGraphEdges, DEFAULT_MEMORY_CONFIG.maxGraphEdges, issues, "maxGraphEdges", { min: 1, max: 128 }),
        embedding,
        advanced,
    };
    return { value: issues.length === 0 ? value : undefined, issues };
}
export const memxConfigSchema = {
    parse(input) {
