...
const FORGET_KINDS = ["doc", "event", "fact", "state"];
...
function countTable(store, table) {
    return Number(store.client.prepare(`SELECT COUNT(*) AS count FROM ${table}`).get()
        .count ?? 0);
}
        ...
        async execute(_toolCallId, rawParams) {
            try {
                ...
                const states = requestedKey
                    ? expandStateKeyAliases(requestedKey).flatMap((key) => store.stateRepo.get({
            ...
            try {
                ...
                store.client.withTransaction(() => {
                    store.stateRepo.upsert({
                        ...
                        expiresAt: durable
                            ? undefined
                            : store.stateRepo.createExpiry(updatedAt, readNumber(params, "ttlHours") ?? opCtx.config.stateTtlHours),
                    });
        ...
        async execute(_toolCallId, rawParams) {
            try {
                ...
                const facts = store.factRepo.query({
                    ...
                    includeHistorical: readBoolean(params, "includeHistorical") ??
        ...
        async execute(_toolCallId, rawParams) {
            try {
                ...
                return jsonToolResult({
                    ...
                    diagnostics: bundle.diagnostics,
                    suggestedFollowUp: bundle.routeConfidence < 0.58 ||
    ...
    const memoryInspect = {
        ...
        async execute(_toolCallId, rawParams) {
            try {
                ...
                const store = await getStore();
                const doc = store.vectorRepo.getDoc(id) ??
