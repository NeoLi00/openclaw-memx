...
import { buildOperationContext } from "../runtime.js";
...
function defaultSetupEntry(config, options) {
    ...
    const llmClassifierModel = options.llmModel?.trim() || config.advanced.llmClassifierModel?.trim() || undefined;
    const embeddingProvider = options.embeddingProvider ?? config.embedding.provider;
    const embeddingModel = options.embeddingModel?.trim() || config.embedding.model?.trim() || undefined;
    const embeddingPythonBin = options.embeddingPythonBin?.trim() || config.embedding.localPythonBin?.trim() || undefined;
    const embeddingCacheDir = options.embeddingCacheDir?.trim() || config.embedding.localCacheDir?.trim() || undefined;
...
export function applyMemxSetupToConfig(appConfig, pluginConfig, options = {}) {
    ...
    const setupEmbedding = setupConfig.embedding && typeof setupConfig.embedding === "object"
        ? setupConfig.embedding
        : {};
    const existingEmbedding = existingConfig.embedding && typeof existingConfig.embedding === "object"
        ? existingConfig.embedding
        : {};
    const setupAdvanced = setupConfig.advanced && typeof setupConfig.advanced === "object"
        ? setupConfig.advanced
        : {};
    const existingAdvanced = existingConfig.advanced && typeof existingConfig.advanced === "object"
        ? existingConfig.advanced
        : {};
    const setupEntry = {
...
export function buildMemxDoctorReport(params) {
    ...
    const entryConfig = entry.config && typeof entry.config === "object"
        ? entry.config
        : {};
    ...
    const entryAdvanced = entryConfig.advanced && typeof entryConfig.advanced === "object"
        ? entryConfig.advanced
        : {};
    const checks = [
        {
            key: "plugin_loaded",
        ...
        },
        {
            ...
            detail: legacyTopLevelConfigKeys.length === 0
        ...
        },
        {
            ...
            detail: entryAdvanced.enableTurnScheduler !== false
        ...
        },
        {
            ...
            detail: (entryAdvanced.llmClassifierEnabled ?? params.pluginConfig.advanced.llmClassifierEnabled)
                ? `LLM classifier is enabled${typeof entryAdvanced.llmClassifierModel === "string"
    ...
    return {
        ...
        configSummary: {
            ...
            llmClassifierModel: (typeof entryAdvanced.llmClassifierModel === "string"
                ? entryAdvanced.llmClassifierModel
                : params.pluginConfig.advanced.llmClassifierModel) ?? null,
            dbPath: (typeof entryConfig.dbPath === "string"
                ? entryConfig.dbPath
                : params.pluginConfig.dbPath) ?? null,
            embeddingProvider: (typeof entryConfig.embedding?.provider ===
                "string"
                ? entryConfig.embedding.provider
                : params.pluginConfig.embedding.provider) ?? null,
            embeddingModel: (typeof entryConfig.embedding?.model === "string"
                ? entryConfig.embedding.model
...
function resolveEffectivePluginConfig(appConfig, fallback) {
    const entry = appConfig.plugins?.entries?.["memory-memx"] ?? {};
    const nested = entry.config && typeof entry.config === "object"
        ? entry.config
        : {};
    const advanced = nested.advanced && typeof nested.advanced === "object"
        ? nested.advanced
        : {};
    const embedding = nested.embedding && typeof nested.embedding === "object"
        ? nested.embedding
        : {};
    return {
        ...fallback,
...
async function printStats(ctx, store) {
    const count = (table) => Number(store.client.prepare(`SELECT COUNT(*) AS count FROM ${table}`).get()
        .count ?? 0);
...
function wipeAgentMemory(ctx, store) {
    ...
    const countByAgent = (table, column = "agent_id") => Number(store.client
        .prepare(`SELECT COUNT(*) AS count FROM ${table} WHERE ${column} = ?`)
        .get(ctx.agentId).count ?? 0);
    ...
    deleted.factVersions = Number(store.client
    ...
    store.client.withTransaction(() => {
        store.client
            .prepare(`DELETE FROM fact_versions
          WHERE fact_id IN (SELECT fact_id FROM facts WHERE agent_id = ?)`)
            .run(ctx.agentId);
        ...
        orphanAliasesDeleted = Number(store.client
            .prepare(`SELECT COUNT(*) AS count
             FROM entity_aliases
            ...
            .get().count ?? 0);
        store.client
            .prepare(`DELETE FROM entity_aliases
        WHERE entity_id NOT IN (SELECT src_entity_id FROM graph_edges)
          AND entity_id NOT IN (SELECT dst_entity_id FROM graph_edges)`)
            .run();
        orphanEntitiesDeleted = Number(store.client
            .prepare(`SELECT COUNT(*) AS count
             FROM entities
            WHERE entity_id NOT IN (SELECT src_entity_id FROM graph_edges)
              AND entity_id NOT IN (SELECT dst_entity_id FROM graph_edges)`)
...
export function registerMemxCli(params) {
    ...
    command
        ...
        .option("--embedding-provider <provider>", "Embedding provider: off, openai-compatible, ollama, or sentence-transformers-local")
        ...
        .option("--local-embedding", "Shortcut for --embedding-provider sentence-transformers-local --embedding-model intfloat/multilingual-e5-small")
        .action(async (options) => {
        const requestedProvider = options.localEmbedding
            ? "sentence-transformers-local"
            : options.embeddingProvider?.trim();
        if (requestedProvider &&
            !["off", "openai-compatible", "ollama", "sentence-transformers-local"].includes(requestedProvider)) {
            throw new Error(`unsupported embedding provider: ${requestedProvider}. Expected one of off, openai-compatible, ollama, sentence-transformers-local`);
        }
        const requestedDevice = options.embeddingDevice?.trim();
        if (requestedDevice && !["auto", "cpu", "mps", "cuda"].includes(requestedDevice)) {
            throw new Error(`unsupported embedding device: ${requestedDevice}. Expected one of auto, cpu, mps, cuda`);
        }
    ...
    command
        ...
        .option("--config <file>", "Path to openclaw.json")
        .option("--deep", "Run live reasoner probes and report whether LLM or heuristic fallback was used")
        .action(async (options) => {
        const configPath = resolveConfigPath(options.config);
        const current = await readUserConfig(configPath);
    ...
    command
        .command("wipe-db")
        .description("Delete all data from the current memory-memx database file while keeping the schema")
