...
export function isAuthoritativeOutcomeResolutionMetadata(metadata) {
    if (!metadata) {
        return false;
    }
    ...
    const hasNonAssistantGrounding = booleanValue(metadata.candidateOutcomeHasNonAssistantGrounding ??
    ...
    return (hasNonAssistantGrounding &&
...
function inferOutcomeHypothesisStage(params) {
    ...
    const probationaryScore = clamp01(params.decision.promotionScore * 0.42 +
    ...
    if (params.evidence.hasNonAssistantGrounding &&
        strongPhase &&
...
export function buildOutcomeEventCandidate(task, proposal, observedAt) {
    return {
...
export function buildOutcomeHypothesisCandidate(params) {
    ...
    const confidence = clamp01(params.outcome.confidence * 0.32 +
    ...
    const usefulnessScore = clamp01(params.decision.promotionScore * 0.34 +
        params.outcome.verificationScore * 0.28 +
        params.outcome.closureScore * 0.12 +
        (evidence.hasNonAssistantGrounding ? 0.16 : 0.04) +
        (evidence.hasToolEvidence ? 0.1 : 0));
    const stabilityScore = clamp01(params.outcome.confidence * 0.24 +
        params.outcome.closureScore * 0.18 +
    ...
    const contradictionScore = clamp01(params.outcome.contradictionRisk * 0.88 + (evidence.hasNonAssistantGrounding ? 0 : 0.06));
    const metadata = {
