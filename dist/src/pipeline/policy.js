...
function derivePolicySignals(candidate, mode = "heuristic") {
    ...
    const questionLikeUserQuery = candidate.source.kind === "user" &&
    ...
    const hasAnySignalObject = cached?.preference ||
        cached?.workflow ||
    ...
    const workflows = cached?.workflows && cached.workflows.length > 0
        ? cached.workflows
        : cached?.workflow
            ? [cached.workflow]
            : (analyzed?.workflows ?? []);
...
function enrichCandidateWithLlmJudgment(candidate, judgment) {
    ...
    const suppressWorkflowHints = candidate.source.kind === "user" &&
    ...
    const existingWorkflows = candidate.structuredHints?.workflows && candidate.structuredHints.workflows.length > 0
        ? candidate.structuredHints.workflows
        : candidate.structuredHints?.workflow
            ? [candidate.structuredHints.workflow]
            : [];
    const llmWorkflows = judgment.workflows && judgment.workflows.length > 0
        ? judgment.workflows
        : judgment.workflow
            ? [judgment.workflow]
            : [];
    ...
    const llmRelations = judgment.relations && judgment.relations.length > 0
        ? judgment.relations
        : judgment.relation
            ? [judgment.relation]
            : [];
...
function hasStructuredSemanticSignal(candidate) {
    const hints = candidate.structuredHints;
    return Boolean(hints?.preference ||
        hints?.decision ||
        hints?.workflow ||
...
function semanticDraftGateScores(action, hintConfidence) {
    ...
    return {
        ...
        expectedFutureUtility: clamp01(Math.max(defaults.expectedFutureUtility - 0.08, confidenceFloor * 0.92)),
...
function semanticDraftConfidence(candidate) {
    const draft = semanticDraft(candidate);
    const assertionConfidence = maxHintConfidence(draft?.assertionDrafts.map((entry) => entry.confidence).filter((value) => typeof value === "number") ?? []);
    const correctionConfidence = maxHintConfidence(draft?.correctionDrafts.map((entry) => entry.confidence).filter((value) => typeof value === "number") ?? []);
    const relationConfidence = maxHintConfidence(draft?.relationDrafts
...
function chooseSemanticDraftAction(candidate) {
    ...
    if (correction) {
        ...
        if (materializationHint?.replacementMode === "supersede_fact" ||
        ...
        if (materializationHint?.preferEvent ||
...
function shouldCaptureDeclarativeHistory(candidate, signals) {
    ...
    const narrativeCue = timeHintCount > 0 ||
...
function finalizeDecision(params) {
    const sensitivity = clamp01(sensitivityScore(params.candidate.rawText) +
...
export function evaluatePolicyHeuristically(candidate, ctx) {
    ...
    else if ((candidate.source.kind === "tool" ||
    ...
    else if (shouldCaptureDeclarativeHistory(candidate, signals) &&
        salience >= Math.min(ctx.config.minSalienceSession, EPISODIC_MIN_SALIENCE_FALLBACK)) {
        action = "episodic_event";
...
function evaluatePolicyFromStructuredHints(candidate, ctx, explicitIntent) {
    ...
    const declarativeHistoryFloor = Math.max(0.24, Math.min(ctx.config.minSalienceSession, EPISODIC_MIN_SALIENCE_FALLBACK) - 0.08);
    let action = "ignore";
    if (signals.workflowHint && salience >= ctx.config.minSalienceDurable) {
        action = "durable_state";
    ...
    else if (candidate.structuredHints?.correction &&
...
export async function evaluatePolicy(candidate, ctx, options = {}) {
    const explicitIntent = hasExplicitRememberIntent(normalizeText(candidate.rawText));
    const questionLikeWithoutExplicitIntent = candidate.source.kind === "user" &&
        (isQuestionLike(candidate.rawText) || isHypotheticalReferenceRule(candidate.rawText)) &&
    ...
    if (ctx.config.advanced.enableTurnSemanticCompiler) {
        ...
        const semanticDraftMiss = candidate.source.kind === "user" &&
            compiledPolicy.decision.action === "ignore" &&
            compiledPolicy.decision.reasons.some((reason) => reason === "semantic-draft-adapter:no-compiler-family" ||
