...
function inferDerivedStateKind(candidate, stateKey) {
    ...
    if (stateKey === "project.active_project" &&
...
function buildPromotionMetadata(candidate, params) {
    ...
    const semanticSources = Array.isArray(candidate.metadataJson.semanticSources)
        ? candidate.metadataJson.semanticSources.filter((entry) => typeof entry === "string" && entry.trim().length > 0)
        : undefined;
    ...
    return {
        ...
        promotion: {
            ...
            derivedFromIds: params.derivedFromIds,
            derivationPolicyVersion: PROMOTION_POLICY_VERSION,
            firstPromotedAt: stringValue(promotion.firstPromotedAt) ??
...
function parseGraphHypothesisCandidate(candidate) {
    ...
    const durableRelationType = relationType === "depends_on" ||
        relationType === "blocks" ||
...
function shouldReplaceState(existing, candidate, sourceRef) {
    ...
    if (Number.isFinite(existingUpdated) &&
...
function promoteOutcomeHypothesis(store, ctx, candidate) {
    ...
    if (!parsed.alreadyEmitted) {
        const summary = writeCandidate(store, ctx, buildOutcomeEventCandidate(parsed.task, parsed.promotedOutcome, parsed.observedAt));
        if (summary.events === 0) {
            return false;
        }
    }
...
export function runAbstractionPromotion(store, ctx, options = {}) {
