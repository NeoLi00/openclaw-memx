...
function continuityScore(snapshot, incomingUserText, incomingSignals, ctx) {
    ...
    const exactProjectMatch = metadata.project && incomingSignals.project
        ? scoreSignalMatch(metadata.project, incomingSignals.project) ?? 0
        : 0;
    const exactTaskMatch = metadata.currentTask && incomingSignals.currentTask
        ? scoreSignalMatch(metadata.currentTask, incomingSignals.currentTask) ?? 0
        : 0;
    const projectTextMatch = metadata.project && incomingUserText
        ? basicSemanticSimilarity(metadata.project, incomingUserText)
        : 0;
    const taskTextMatch = metadata.currentTask && incomingUserText
        ? basicSemanticSimilarity(metadata.currentTask, incomingUserText)
        : 0;
    const softContextScore = incomingSignals.project || incomingSignals.currentTask || incomingSignals.nextAction || incomingSignals.blocker
    ...
    if (snapshot.isActive && !idleExpired) {
        continuity = Math.max(continuity, incomingSignals.project || incomingSignals.currentTask ? 0.42 : 0.34);
    }
...
export function buildDeterministicTaskProposal(params) {
    const decisionNow = effectiveDecisionNow(params.newMessages, params.ctx.now);
    const decisionCtx = decisionNow === params.ctx.now
        ? params.ctx
        : {
            ...params.ctx,
            now: decisionNow,
    ...
    const dedupedRecent = params.recentTasks.filter((task, index, array) => task.taskId !== params.activeTask?.taskId &&
    ...
    const candidateSnapshots = [
        ...
        ...dedupedRecent.map((task) => buildTaskSnapshot(task, params.recentChunksByTask[task.taskId] ?? [], false)),
    ];
...
export async function decideTaskAssignment(params) {
    ...
    if (params.taskProposal && params.taskProposal.decision !== "none") {
        ...
        if (params.taskProposal.decision === "resume" &&
        ...
        return {
            decision: params.taskProposal.decision === "resume" &&
                params.taskProposal.targetTaskId === params.activeTask?.taskId
                ? "continue"
                : params.taskProposal.decision,
            targetTaskId: params.taskProposal.decision === "resume" &&
                params.taskProposal.targetTaskId === params.activeTask?.taskId
                ? undefined
                : params.taskProposal.targetTaskId,
...
export async function shouldStartNewTask(activeTask, taskChunks, newMessages, ctx, reasoner) {
