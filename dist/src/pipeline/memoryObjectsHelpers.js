...
export function normalizeLineageRef(value) {
...
export function lineageFromMetadata(metadata, fallback) {
...
export function lineageMetadata(lineage) {
    return {
...
export function dedupeEvidenceRows(rows, limit) {
    const seen = new Set();
    const result = [];
...
export function shouldSuppressRecallText(text) {
    const normalized = normalizeSearchText(text);
    return (isBootstrapMemoryContamination(normalized) ||
        looksLikePromptInjection(normalized) ||
        containsLikelySecret(normalized));
}
export function rowsFromSearchHits(hits) {
    return hits.map((hit) => toEvidenceRow({
        ...
        provenance: typeof hit.metadata.role === "string"
            ? hit.metadata.role
...
export function describeStateValue(key, valueJson) {
    ...
    if (key.startsWith("project.") && canonicalStateKey(key) !== "project.active_project") {
        const projectCode = typeof valueJson.projectCode === "string" && valueJson.projectCode.trim()
        ...
        const historicalAliases = Array.isArray(valueJson.historicalAliases)
            ...
            : typeof valueJson.historicalAliases === "string"
                ...
                : "";
        const components = valueJson.components &&
            typeof valueJson.components === "object" &&
            !Array.isArray(valueJson.components)
            ? Object.entries(valueJson.components)
                .map(([slot, value]) => typeof value === "string" && value.trim() ? `${slot}=${value.trim()}` : "")
                .filter(Boolean)
                .join(", ")
            : "";
        const details = [
            ...
            components,
            ...formatStateDetailEntries(["projectCode", "version", "launchDate", "internalTrialDate", "historicalAliases", "components"], ["status", "note", "decision"], true),
        ].filter(Boolean);
        return details.join(" | ");
    }
    ...
    switch (canonicalStateKey(key)) {
        ...
        case "workflow.current_task":
            return [
                task ?? genericValue ?? "",
                ...formatStateDetailEntries(["task", "value"], ["status", "note", "decision", "topic", "option"], true),
            ]
                .filter(Boolean)
                .join(" | ");
...
export function formatFactLine(params) {
    ...
    const prefix = params.status === "superseded"
        ? "[previous] "
