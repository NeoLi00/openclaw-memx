import { clamp01, normalizeName, normalizeText, stableHash } from "../support.js";
...
import { stateCurrentnessFromVectorMetadata, stateCurrentnessToMetadata, stateCurrentnessVectorMetadata, } from "./stateLifecycle.js";
...
const hybridSearchCache = new WeakMap();
...
function surfaceTypeMatches(surface, hit) {
    const docType = typeof hit.metadata.memxDocType === "string" ? hit.metadata.memxDocType : undefined;
    switch (surface) {
        case "state":
...
function evaluateHitRetention(surface, hit, compiled, ctx) {
    ...
    if (compiled.evidencePlan?.operation.type === "tailor_advice" &&
        ...
        const bestResourceSlotScore = Math.max(0, ...(hit.slotMatches ?? [])
            .filter((match) => resourceSlotIds.has(match.slotId))
            .map((match) => match.score), ...(hit.bridgeMatches ?? [])
    ...
    if (ctx.sessionKey && compiled.queryShape.evidenceNeed === "workflow_context") {
        const sessionKey = typeof hit.metadata.sessionKey === "string" ? hit.metadata.sessionKey : undefined;
        if (sessionKey &&
    ...
    if (compiled.queryShape.timeframe === "current") {
        if (surface === "state") {
            const stateKey = typeof hit.metadata.stateKey === "string" ? hit.metadata.stateKey : undefined;
            if (stateKey &&
...
function toCandidateHit(surface, hit, ctx, tier) {
    ...
    const stateCurrentness = surface === "state" ? stateCurrentnessFromVectorMetadata(hit.metadata, ctx.now) : undefined;
    ...
    return {
        ...
        agentId: ctx.agentId,
        confidence: typeof hit.metadata.confidence === "number" ? clamp01(hit.metadata.confidence) : undefined,
        ...
        currentnessHint: metadata.currentnessHint === "current" ||
...
function mergeCandidateHits(hits) {
    ...
    for (const hit of hits) {
        ...
        byId.set(key, {
            ...
            confidence: typeof existing.confidence === "number" || typeof hit.confidence === "number"
...
function rerankSurfaceHits(hits, surface, compiled) {
    ...
    return [...hits].sort((left, right) => {
        ...
        const rightCurrentPenalty = surface === "fact" &&
            compiled.queryShape.timeframe === "current" &&
            right.metadata.currentnessHint === "historical"
            ? 0.08
            : 0;
        const leftCurrentPenalty = surface === "fact" &&
            compiled.queryShape.timeframe === "current" &&
            left.metadata.currentnessHint === "historical"
            ? 0.08
            : 0;
        const rightScore = right.score - rightCurrentPenalty + (detailSensitive ? rightAnchor * 0.14 : 0);
...
function eventCandidatesForEntity(store, ctx, entity, limit) {
    ...
    return [...events.values()]
        ...
        tier: "primary",
        text: event.text,
        score: clamp01(event.confidence * 0.54 +
...
function stateCandidatesForEntity(store, ctx, entity, limit) {
    const states = store.stateRepo
        ...
        .filter((state) => entityExpansionTextMatches(entity, `${state.key} ${JSON.stringify(state.valueJson)} ${describeStateValue(state.key, state.valueJson)}`))
        .sort((left, right) => right.confidence - left.confidence ||
        Date.parse(right.updatedAt) - Date.parse(left.updatedAt))
        .slice(0, limit);
    return states.flatMap((state) => {
        ...
        const expandedSupportRefs = uniqueMaintenanceRefs([
            ...(Array.isArray(currentness.stateSupportRefs)
                ? currentness.stateSupportRefs.filter((entry) => typeof entry === "string" && entry.trim().length > 0)
                : []),
        ...
        return [
            {
                ...
                score: clamp01(state.confidence * 0.4 +
                ...
                currentnessHint: currentness.currentnessHint === "current" ||
...
function graphCandidatesForEntities(store, ctx, entities, limit) {
    ...
    return graph.edges.slice(0, limit).map((edge, index) => {
        ...
        const seedBoost = seedIds.has(edge.srcEntityId ?? edge.srcNodeId) ||
...
function resolveQueryEntities(store, ctx, compiled, limit) {
    ...
    const resolvedEntities = entityQueries
        .map((query) => resolveEntityMention(store, ctx, buildEntityMention({
        ctx,
        scope: ctx.scopes[0] ?? "unknown",
        rawText: query,
        semanticRole: "query",
    ...
    const linkedEntities = resolvedEntities
        ...
        .flatMap((entity) => store.graphRepo
...
function selectSurfaceHitsForSlots(rankedHits, topN, compiled) {
    ...
    for (const slot of slots) {
        const hit = rankedHits.find((candidate) => {
            ...
            return (candidate.slotMatches ?? []).some((match) => match.slotId === slot.id && match.score >= 0.2);
        });
        if (!hit) {
            continue;
        }
...
function attachGoalMatch(hit, goal, matchedQuery, compiled) {
    ...
    const retrievalScore = hit.backend === "hybrid" || hit.backend === "embedding"
        ? clamp01(hit.score)
...
function slotsForSurface(compiled, surface) {
    return (compiled.evidencePlan?.slots ?? []).flatMap((slot) => slotLayers(slot)
...
function slotSearchQueries(slot, compiled) {
    ...
    if (isTailorAdvice) {
        if (isUserResourceSlot(slot)) {
            ...
            const subjectFallback = (slot.relationHints ?? []).length === 0 ? slotSubjectQueryVariants(slot.subjectHints) : [];
            return uniqueSearchQueries([
                ...subjectFallback,
                ...(slot.relationHints ?? []),
                ...(slot.capabilityQueries ?? []),
...
function isUserResourceSlot(slot) {
    return (slot.requiredRole === "user_resource" ||
        slot.role === "user_resource" ||
...
function hasResourceAffordanceBridge(metadata) {
    return (metadataStringArray(metadata, "domains").length > 0 ||
        metadataStringArray(metadata, "affordances").length > 0 ||
...
function bridgeMatchType(bridge) {
...
function scoreBridgeMatch(bridge, hit, matchedQuery) {
    ...
    const positiveSignalScore = bridge.positiveSignals.length > 0
        ? informativeAnchorSupport(scoringText, bridge.positiveSignals)
        : 0;
    const negativeSignalScore = (bridge.negativeSignals ?? []).length > 0
    ...
    const structuredBridgeBoost = bridge.evidenceShape === "resource_affordance" && hasResourceAffordanceBridge(hit.metadata)
        ? 0.1
        : 0;
    const negativeSignalWeight = bridge.role === "answer_value" ||
        bridge.role === "answer_event" ||
        bridge.role === "user_resource" ||
        bridge.role === "prior_advice"
        ? 0
        : 0.18;
    const score = clamp01(querySupport * 0.72 +
...
function slotMatchScore(text, slot, query, retrievalScore = 0, retrievalBackend, metadata) {
    ...
    const subjectSupport = subjectHints.length > 0
        ? informativeAnchorSupport(text, subjectHints)
        : slot.subjectHints.length > 0
            ? 0
            : 0.5;
    const relationSupport = relationHints.length > 0
    ...
    const baseScore = clamp01(Math.max(subjectSupport, querySupport * 0.82, retrievalSupport * 0.86) * 0.52 +
        relationSupport * 0.2 +
        field * 0.2 +
    ...
    if (hasStructuredResourceBridge(metadata)) {
        ...
        const structuredScore = clamp01(bridgeSimilarity * 0.72 +
            field * 0.06 +
    ...
    const bridgeScore = clamp01(bridgeSimilarity * 0.5 +
...
function selectSlotCandidateSearchHits(hits, compiled, surface) {
    ...
    for (const { slot, layer } of slotsForSurface(compiled, surface)) {
        ...
        const perSlot = hits
            .filter((hit) => (hit.slotMatches ?? []).some((match) => match.slotId === slot.id && match.layer === layer && match.score >= threshold))
            .sort((left, right) => {
            const rightScore = Math.max(0, ...(right.slotMatches ?? [])
                .filter((match) => match.slotId === slot.id && match.layer === layer)
                .map((match) => match.score));
            const leftScore = Math.max(0, ...(left.slotMatches ?? [])
                .filter((match) => match.slotId === slot.id && match.layer === layer)
                .map((match) => match.score));
...
async function goalSearchHits(store, ctx, compiled, surface, topN) {
    ...
    const retrievalScope = retrievalScopeForSurface(surface);
    const queries = goals.flatMap((goal) => goal.positiveQueries.slice(0, 3).map((query) => ({ goal, query: query.trim() })));
    const hits = [];
...
async function retrieveSurfaceHits(store, ctx, compiled, surface, topN) {
    if (surface === "entity_alias") {
        ...
        const expansionCandidates = entityExpansionCandidates(store, ctx, entityCandidates
            .map((candidate) => typeof candidate.metadata?.entityId === "string"
            ? store.graphRepo.getEntityById(candidate.metadata.entityId)
            : null)
            .filter((entity) => Boolean(entity)), topN);
    ...
    const bridgeCandidateHits = bridgeHits
        ...
        .sort((left, right) => topBridgeMatchScore(right) - topBridgeMatchScore(left) || right.score - left.score)
    ...
    const rankedCandidates = [...mergedSelectedHits, ...mergedAlternateHits]
        .map((hit) => toCandidateHit(surface, hit, ctx, mergedSelectedHits.some((selected) => selected.docId === hit.docId)
        ? "primary"
        : "alternate"))
        .filter((hit) => Boolean(hit));
    return {
...
function shouldAttemptSurfaceRecovery(compiled) {
    return (compiled.turnMode === "memory_qa" &&
        compiled.queryShape.evidenceNeed !== "workflow_context" &&
        compiled.queryShape.evidenceNeed !== "relation");
}
function recoverySurfacesForCompiled(compiled) {
    ...
    if (!compiled.candidateSurfaces.includes("chunk") &&
...
export async function generateCandidates(store, ctx, compiled) {
...
export function candidateGenerationAuditPayload(result) {
    const summarizeCandidate = (hit) => ({
        ...
        slotMatches: hit.slotMatches,
        bridgeMatches: hit.bridgeMatches,
        materializedEpoch: typeof hit.metadata?.materializedEpoch === "number"
