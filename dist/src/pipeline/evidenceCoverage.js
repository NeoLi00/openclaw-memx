...
function anchorMatches(text, anchor) {
    ...
    const textTokenSet = new Set(normalizedText
        .split(/[^\p{L}\p{N}']+/u)
        .map((token) => normalizeText(token))
        .filter(Boolean));
    return anchorTokens.every((token) => textTokenSet.has(normalizeText(token)));
}
export function evidenceCoverageForText(queryAnalysis, text) {
...
export function capScoreByEvidenceCoverage(score, coverage) {
