...
import { candidateGenerationAuditPayload, generateCandidates, } from "./candidateGeneration.js";
...
import { buildBackgroundRecallBundle as buildBackgroundRecallBundleFromObjects, collectBehavioralGuidance as collectBehavioralGuidanceFromObjects, collectAndScheduleMemoryObjectsWithBudget, collectMemoryObjects, queryMemoryFacts, routeHintFromMemoryObjectKind, scheduleMemoryObjects, topProbeSupportForRoute, topScheduledMemoryScore, toRouteEvidenceCandidatesFromObjects, } from "./memoryObjects.js";
import { dedupeEvidenceRows, formatFactLine, lineageFromMetadata, normalizeSearchText, rowsFromSearchHits, shouldSuppressRecallText, splitLabelValue, toEvidenceRow, } from "./memoryObjectsHelpers.js";
import { createMemorySelectionObjective, projectScheduledMemoryObjects, } from "./memoryObjectsProjection.js";
...
import { buildRecallAuditPayload, compareEvidenceRowsChronologically, sanitizeFocusedRecallQuery, } from "./retrieveTracing.js";
import { queryAnchorSupport } from "./semantic/heuristics.js";
import { semanticTextSimilarity } from "./semantic/textSimilarity.js";
import { extractQueryAnchors, inferEntityNames, inferTemporalSince, isQuestionLike, } from "./semantics.js";
...
export function analyzeRecallQuery(query) {
    return compileQueryDeterministically(query);
}
function recallProbeThresholds(config) {
    return {
...
export function buildBackgroundRecallBundle(store, ctx) {
    return buildBackgroundRecallBundleFromObjects(store, ctx);
}
export function hasBackgroundRecallMaterial(bundle) {
    return bundle.projectionBlocks.length > 0 || bundle.behavioralGuidance.length > 0;
}
function deriveRecallNeedLevel(params) {
    const trimmed = params.query.trim();
    const backgroundContextAvailable = hasBackgroundRecallMaterial(params.background) ||
    ...
    const workflowSupport = clamp01(Math.max(params.probe.signals.workflowSimilarity, params.probe.signals.contextAssociation * 0.88, workflowContinuity));
    const factualSupport = clamp01(params.probe.signals.factualSimilarity *
        Math.min(1, Math.max(params.probe.signals.factualCandidateCount, 0) / 2) *
        (params.probe.signals.hybridHitCount > 0 ? 0.92 : 0.35));
    const temporalSupport = clamp01(params.probe.signals.temporalSimilarity *
        Math.min(1, Math.max(params.probe.signals.hybridHitCount, 0) / 2) *
        0.84);
    const explanatorySupport = clamp01(params.probe.signals.explanatorySimilarity *
        Math.min(1, Math.max(params.probe.signals.hybridHitCount, 0) / 2) *
        0.84);
    const routeSupport = Math.max(workflowSupport, factualSupport, temporalSupport, explanatorySupport);
    const lowSupportSurface = routeSupport < Math.min(0.52, params.probe.thresholds.hybridModerate * 0.72);
    const authoritativeQueryShape = queryShape.timeframe !== "timeless" ||
        queryShape.granularity === "exact_detail" ||
        queryShape.evidenceNeed === "workflow_context";
    const continuityAuthority = !params.plan.shouldRecall &&
        queryShape.evidenceNeed === "workflow_context" &&
        params.background.tasks.length > 0 &&
    ...
    const exactDetailAuthority = queryShape.granularity === "exact_detail" &&
        (params.probe.signals.hybridHitCount >= 1 || params.probe.signals.factualCandidateCount >= 1);
    const correctedHistoricalAuthority = (queryShape.timeframe === "historical" || queryShape.timeframe === "compare") &&
        (params.probe.signals.hybridHitCount >= 1 ||
            params.probe.signals.factualCandidateCount >= 1 ||
            params.probe.signals.topProbeDocType === "event");
    const snapshotAuthority = queryShape.timeframe === "current" &&
        (params.probe.signals.factualCandidateCount >= 1 ||
    ...
    if (!trimmed) {
        ...
        reasons.push(backgroundContextAvailable
    ...
    const divergence = legacyFullRecall && needLevel !== "full"
        ? "legacy_over_recall"
...
function isPrimaryRouteType(route) {
    return Boolean(route && PRIMARY_ROUTE_TYPES.includes(route));
}
function projectionRolesForRoute(routeType) {
    switch (routeType) {
        case "workflow":
            return ["active_task"];
        case "factual":
...
function hasProjectionRole(background, role) {
    return background.projectionBlocks.some((block) => block.role === role);
}
function projectionSupportForRoute(query, background, routeType) {
    const relevantBlocks = background.projectionBlocks.filter((block) => projectionRolesForRoute(routeType).includes(block.role));
    if (relevantBlocks.length === 0) {
        return 0;
    }
    const anchors = extractQueryAnchors(query).filter((anchor) => isMeaningfulRecallAnchor(anchor));
    if (anchors.length > 0) {
        const anchored = relevantBlocks.some((block) => queryAnchorSupport(block.lines.join("\n"), anchors) >= 0.58);
        if (anchored) {
...
function shouldUseExactSnippetSupport(queryAnalysis) {
    return (queryAnalysis.evidenceFidelity === "high" ||
...
function evidenceRowFromCandidateHit(hit) {
    const observedAt = typeof hit.metadata?.observedAt === "string" ? hit.metadata.observedAt : undefined;
...
function selectComplementaryFactRowsForMainSurface(params) {
    const detailSensitive = params.queryAnalysis.answerGranularity === "detail" ||
    ...
    const selectedMeta = params.selectedFacts
        ...
        if (fact.objectValueJson?.answerBearing !== true ||
    ...
    const reserveFactRows = (params.reserveFactTrace ?? [])
        ...
        .filter((entry) => Boolean(entry));
    return dedupeEvidenceRows([...params.fallbackFacts, ...reserveFactRows], Math.max(6, params.limit * 6))
...
function preferredProjectedExactSnippets(params) {
    return params.projectedIds
        ...
        source: "projected",
        anchorSupport: params.queryAnchors.length > 0
...
function buildSourceRefFallbackSnippets(params) {
    ...
    return chunks
        .map((chunk) => ({
        chunk,
        anchorScore: params.queryAnchors && params.queryAnchors.length > 0
...
function rawScoreTextAgainstEvidenceGoals(params) {
    ...
    const anchorScore = params.queryAnalysis.anchors.length > 0
    ...
    const goalScore = goals.reduce((best, goal) => {
        ...
        const focusScore = goal.focusAnchors.length > 0
            ? informativePromptAnchorSupport(params.text, goal.focusAnchors)
            : 0;
        const surfaceBoost = params.surface && goal.preferredSurfaces.includes(params.surface) ? 0.08 : 0;
        const negativePenalty = Math.max(0, ...(goal.negativeHints ?? []).map((hint) => semanticTextSimilarity(hint, params.text) * 0.12));
        return Math.max(best, clamp01(positiveScore * 0.68 + focusScore * 0.18 + surfaceBoost - negativePenalty));
    }, 0);
    ...
    const adjustedExplicitGoalScore = tailorAdvice
        ...
        : explicitGoalScore > 0 && explicitSupport < 0.12
            ...
            : explicitGoalScore;
    const adjustedGoalScore = explicitSupport < 0.12
...
function scoreTextAgainstEvidenceGoals(params) {
    ...
    return capScoreByEvidenceCoverage(rawScore, evidenceCoverageForText(params.queryAnalysis, params.text));
}
...
function preferredSurfaceSupport(queryAnalysis, surface) {
    ...
    return (queryAnalysis.evidenceGoals ?? []).some((goal) => goal.preferredSurfaces.includes(mappedSurface))
        ? 0.08
        : 0;
}
...
function informativePromptAnchorSupport(text, anchors) {
    ...
    for (const anchor of usefulAnchors) {
        ...
        const containment = normalizedAnchor.length >= 4 && normalizedText.includes(normalizedAnchor) ? 0.94 : 0;
...
function promptEvidenceScoringTextFromMetadata(text, metadata) {
    ...
    return uniqueNonEmpty(parts.filter((part) => Boolean(part)), 16).join(" | ");
}
function promptEvidenceScoringText(candidate) {
    return uniqueNonEmpty([candidate.text, candidate.scoringText, candidate.rawText].filter((part) => typeof part === "string" && part.trim().length > 0), 6).join(" | ");
}
function slotSubjectFillThreshold(queryAnalysis) {
    ...
    const multiSlot = (queryAnalysis.evidencePlan?.slots.length ?? 0) > 1 ||
...
function promptEvidenceNonSubjectSlotHitCount(queryAnalysis, candidate) {
    ...
    const focusAnchors = new Set([...queryAnchors, ...(queryAnalysis.evidenceGoals ?? []).flatMap((goal) => goal.focusAnchors)]
        .map((anchor) => normalizeText(anchor))
        .filter(Boolean));
    const hits = new Set();
    const slotCoverage = candidate.slotCoverage ??
    ...
    for (const coverage of slotCoverage) {
        ...
        const subjectHints = new Set([...(slot?.subjectHints ?? []), ...focusAnchors]
...
function promptEvidenceRequiresAnswerSlotSupport(queryAnalysis) {
    ...
    return (operationType === "return_value" ||
...
function slotCoverageForText(queryAnalysis, text) {
    return (queryAnalysis.evidencePlan?.slots ?? []).map((slot) => {
        ...
        const subjectScore = subjectHints.length > 0
            ? informativePromptAnchorSupport(text, subjectHints)
            : slot.subjectHints.length > 0
                ? 0
                : 0.5;
        const relationScore = relationHints.length > 0
        ...
        const semanticSupport = Math.max(querySimilarity, rawScoreTextAgainstEvidenceGoals({ text, queryAnalysis }));
        ...
        const relationHits = relationHints.filter((hint) => informativePromptAnchorSupport(text, [hint]) >= 0.52);
        ...
        const coverageScore = isTailorAdviceMemory
            ? clamp01(subjectScore * 0.22 + relationScore * 0.46 + field.score * 0.16 + semanticSupport * 0.16)
            : clamp01(subjectScore * 0.58 + relationScore * 0.18 + field.score * 0.24);
        const semanticStructuralSupport = subjectScore >= Math.max(0.24, subjectThreshold - 0.18) ||
            relationScore >= 0.32 ||
        ...
        const missingRequired = filled
            ...
            : [
                ...(isTailorAdviceMemory
                    ? []
                    : subjectHints.filter((hint) => informativePromptAnchorSupport(text, [hint]) < subjectThreshold)),
                ...field.missing,
            ];
        return {
            slotId: slot.id,
...
function isTailorAdviceMemorySlot(queryAnalysis, slotId) {
    ...
    return (queryAnalysis.evidencePlan?.operation.type === "tailor_advice" &&
...
function isQueryContextHit(queryAnalysis, hit) {
    ...
    return (normalizedTextContainsExactPhrase(queryContext, normalizedHit) ||
...
function pruneTailorAdviceMemorySlotCoverage(params) {
    ...
    const requiredSemanticSupport = isTailorAdviceResourceSlot(params.queryAnalysis, params.coverage.slotId)
        ? 0.34
        : 0.42;
...
function slotCoverageWithCandidateMatches(queryAnalysis, text, hit) {
    ...
    return baseCoverage.map((coverage) => {
        ...
        const threshold = queryAnalysis.evidencePlan?.operation.type === "tailor_advice"
            ? resourceSlot
                ? 0.16
                : 0.34
            : 0.52;
        ...
        const highConfidenceSemanticMatch = bestMatch.score >= 0.86 && hit.retrievalBackend === "embedding";
        const semanticSlotMatch = specificSupport >= 0.18 ||
            matchedQuerySupport >= 0.42 ||
            (bestMatch.score >= 0.82 && matchedQuerySupport >= 0.32);
        const structuredResourceMatch = resourceSlot && candidateHitHasStructuredResourceSignal(hit) && bestMatch.score >= 0.16;
...
function cleanPromptEvidenceText(text) {
    ...
    if (rest) {
        ...
        const restRepeatsSummary = firstNormalized.length >= 24 &&
            semanticTextSimilarity(firstLine, firstRestSentence.slice(0, 260)) >= 0.82;
        const uninformativeSummary = /^assistant\s+explanatory\s+response$/iu.test(firstLine) ||
            firstNormalized === "assistant explanatory response";
        if (restStartsWithSummary ||
...
function rawAssistantExplanatorySummary(text) {
    ...
    const withoutRole = firstLine.replace(/^(?:\[(?:user|assistant|tool|memory)\]|(?:user|assistant|tool|memory))\s*:\s*/iu, "");
...
function promptEvidenceRole(candidate) {
    ...
    if (/^(?:\[assistant\]|assistant)\s*:/iu.test(text) ||
...
function promptEvidenceAnswerSpecificity(queryAnalysis, candidate) {
    ...
    const highSemanticSupport = (candidate.semanticScore ?? candidate.goalScore) >= 0.82 || candidate.goalScore >= 0.82
        ? 0.42
        : 0;
...
function promptEvidenceAnswerSlotStrength(queryAnalysis, candidate) {
    ...
    return clamp01(Math.max(0.52, Math.min(0.86, bestSlotCoverage)) + Math.min(0.18, nonSubjectHitCount * 0.055));
}
function promptEvidenceSameSourceQuality(queryAnalysis, candidate) {
    ...
    return clamp01(promptEvidenceAnswerSpecificity(queryAnalysis, candidate) * 0.42 +
...
function shouldReplacePromptEvidence(existing, candidate, queryAnalysis) {
    ...
    if (candidate.goalScore !== existing.goalScore) {
        const authorityDelta = promptEvidenceAuthorityRank(candidate) - promptEvidenceAuthorityRank(existing);
...
function promptEvidencePriority(params) {
    ...
    const goalScore = params.queryAnalysis.evidencePlan?.operation.type === "tailor_advice"
        ? semanticScore
        : capScoreByEvidenceCoverage(semanticScore, evidenceCoverageForText(params.queryAnalysis, params.text));
    const sourceBoost = {
        candidate: 0.1,
        selected: 0.08,
        support_ref: 0.07,
    ...
    const surfaceBoost = params.surface === "chunk" || params.surface === "snippet"
        ? params.queryAnalysis.evidenceFidelity === "high" ||
    ...
    const focusPenalty = (params.queryAnalysis.evidenceFidelity === "high" ||
        params.queryAnalysis.answerGranularity === "detail") &&
        ...
        : semanticScore < 0.4 && focusSupport < 0.38
            ? 0.08
            : 0;
    const weakUncoveredGoalPenalty = params.queryAnalysis.evidencePlan?.operation.type === "tailor_advice" &&
    ...
    const priority = clamp01(goalScore * 0.52 +
...
function metadataHasStructuredResourceSignal(metadata) {
    ...
    return (metadata.signalKind === "resourceAssertion" ||
        typeof metadata.resourceType === "string" ||
...
function textHasStructuredResourceMarker(text) {
    const normalized = normalizeText(text);
    return (normalized.includes("user.resource") ||
...
function promptEvidenceCapabilitySupport(queryAnalysis, candidate) {
    ...
    const matchedSlots = (candidate.slotCoverage ??
        slotCoverageForText(queryAnalysis, promptEvidenceScoringText(candidate)))
    ...
    const capabilityHints = uniqueNonEmpty(matchedSlots.flatMap((slot) => slot.capabilityQueries && slot.capabilityQueries.length > 0
        ? slot.capabilityQueries
        : (slot.relationHints ?? [])), 12);
...
function promptEvidenceInjectionLimit(queryAnalysis, ranked) {
    ...
    if (queryAnalysis.evidencePlan?.operation.type &&
    ...
    if (queryAnalysis.answerMode === "count_aggregate") {
        return Math.max(1, Math.min(3, queryAnalysis.evidenceCoverage?.minProtectedItems ?? 2, ranked.length));
    }
...
function scorePromptEvidenceForInjection(queryAnalysis, candidate) {
    ...
    const operationType = queryAnalysis.evidencePlan?.operation.type;
    const roleWeight = operationType === "tailor_advice"
        ...
        : slotEvidenceRole === "answer_value" || slotEvidenceRole === "answer_event"
            ...
            : slotEvidenceRole === "user_resource"
                ...
                : slotEvidenceRole === "time_constraint"
                    ? -0.04
                    : 0;
    const sourceTrace = candidate.sourceRef || (candidate.mergedSourceRefs?.length ?? 0) > 0 ? 0.04 : 0;
    const structuredResource = slotEvidenceRole === "user_resource" &&
        promptEvidenceHasStructuredResourceSignal(candidate) &&
        (capabilitySupport >= 0.2 || goalQuerySupport >= 0.24 || semanticScore >= 0.32)
        ? 0.1
        : 0;
    const resourceCapabilityPenalty = slotEvidenceRole === "user_resource" &&
        promptEvidenceHasStructuredResourceSignal(candidate) &&
        ...
        ? 0.24
        : 0;
    const assistantPenalty = promptEvidenceRole(candidate) === "assistant" &&
        !queryAllowsAssistantAuthoredEvidence(queryAnalysis)
        ? 0.16
        : 0;
    const queryContextOnlyPenalty = operationType === "tailor_advice" &&
    ...
    const injectionScore = clamp01(candidate.priority * 0.34 +
        semanticScore * 0.2 +
...
function promptEvidenceInjectionSelectionReason(candidate, index) {
    const role = candidate.slotEvidenceRole ? `:${candidate.slotEvidenceRole}` : "";
    const score = typeof candidate.injectionScore === "number" ? `:${candidate.injectionScore.toFixed(3)}` : "";
...
function sourceRefsFromCandidateMetadata(metadata) {
    ...
    if (maintenanceContract && typeof maintenanceContract === "object") {
        refs.push(...metadataStringArray(maintenanceContract, "sourceRefsForExpansion"));
    }
...
function sourceExpansionNeighborRefs(sourceRef, window = 2) {
    ...
    if (parts[0] === "user" && parts[1] === "lme" && parts.length >= 5) {
        ...
        const turn = numericSuffix(turnPart);
        if (turn !== undefined) {
            pushTurnWindow(["lme", parts[2], parts[3]], turn, ["user", "assistant"], undefined, turnPart);
        }
    }
    if (parts[0] === "agentmem" && parts.length >= 4) {
        ...
        const turn = numericSuffix(turnPart);
        if (turn !== undefined) {
            pushTurnWindow(["agentmem", parts[1], parts[2]], turn, ["user", "assistant"], ["user", "agentmem", parts[1], parts[2]], turnPart);
        }
    }
    if (parts[0] === "user" && parts[1] === "agentmem" && parts.length >= 5) {
        ...
        const turn = numericSuffix(turnPart);
        if (turn !== undefined) {
            pushTurnWindow(["agentmem", parts[2], parts[3]], turn, ["user", "assistant"], ["user", "agentmem", parts[2], parts[3]], turnPart);
        }
    }
...
function maintenanceObjectExpansionRefs(store, sourceRef, seen = new Set()) {
    if (!sourceRef || seen.has(sourceRef)) {
        return [];
    }
    seen.add(sourceRef);
    if (sourceRef.startsWith("abstraction_candidate:")) {
        const candidate = store.abstractionRepo.getById(sourceRef.slice("abstraction_candidate:".length));
        if (!candidate) {
            return [];
        }
...
function sourceExpansionSeedScore(candidate) {
    const roleBonus = candidate.slotEvidenceRole === "answer_value" || candidate.slotEvidenceRole === "answer_event"
        ? 0.18
        : candidate.slotEvidenceRole === "query_context"
            ...
            : candidate.slotEvidenceRole === "user_resource" ||
                ...
                ? 0.1
                : 0;
    const sourceTraceBonus = candidate.sourceRef || (candidate.mergedSourceRefs?.length ?? 0) > 0 ? 0.06 : 0;
    return clamp01(Math.max(candidate.priority, candidate.goalScore, candidate.semanticScore ?? 0) +
        roleBonus +
        sourceTraceBonus +
...
function sourceExpansionPromptEvidenceCandidates(params) {
    ...
    for (const chunk of chunks) {
        const segments = chunk.content.length > SOURCE_SEGMENT_TARGET_CHARS
...
function shouldRetainSourceExpansionAnswerCandidate(queryAnalysis, candidate) {
    ...
    const score = Math.max(candidate.injectionScore ?? 0, candidate.priority, candidate.goalScore, candidate.semanticScore ?? 0, promptEvidenceAnswerSlotStrength(queryAnalysis, candidate), promptEvidenceAnswerSpecificity(queryAnalysis, candidate));
    return score >= 0.42;
}
function keepRetainedPromptEvidenceCandidates(params) {
    ...
    const retainedAnswers = params.candidates
        .filter((candidate) => shouldRetainSourceExpansionAnswerCandidate(params.queryAnalysis, candidate))
        .slice(0, 8);
    const retainedContext = params.candidates
        .filter((candidate) => retainedAnswers.some((answer) => {
        ...
        const role = candidate.slotEvidenceRole ??
            promptEvidenceSlotRoleForCandidate(params.queryAnalysis, candidate);
        return (promptEvidenceRefsAdjacent(answer, candidate, 2) &&
    ...
    for (const candidate of reserve) {
        ...
        const replaceIndex = [...result]
            ...
            .reverse()
            .find(({ entry }) => !shouldRetainSourceExpansionAnswerCandidate(params.queryAnalysis, entry))?.index;
...
function controlPromptEvidenceCandidates(params) {
    ...
    if (!slots.some((slot) => evidencePlanLayersForSlot(slot).some((layer) => layer === "control" ||
        layer === "strategy" ||
        layer === "abstraction" ||
        layer === "belief"))) {
        return [];
...
function buildPromptEvidenceCandidates(params) {
    ...
    for (const fact of params.facts) {
        candidates.push(promptEvidenceFromRow({
            row: fact,
            surface: "fact",
            source: "selected",
            queryAnalysis: params.queryAnalysis,
    ...
    const ordered = candidates.sort((left, right) => {
        ...
        const nonSubjectHitDelta = promptEvidenceNonSubjectSlotHitCount(params.queryAnalysis, right) -
            promptEvidenceNonSubjectSlotHitCount(params.queryAnalysis, left);
        if (nonSubjectHitDelta !== 0 &&
            promptEvidenceRequiresAnswerSlotSupport(params.queryAnalysis) &&
            Math.abs(priorityDelta) <= 0.32) {
            return nonSubjectHitDelta;
        }
        const answerSlotDelta = promptEvidenceAnswerSlotStrength(params.queryAnalysis, right) -
        ...
        const specificityDelta = promptEvidenceAnswerSpecificity(params.queryAnalysis, right) -
    ...
    for (const candidate of ordered) {
        ...
        if (seen.has(key)) {
            const sameSourceIndex = selected.findIndex((entry) => {
                return ((entry.sourceRef ?? "") === (candidate.sourceRef ?? "") &&
    ...
    const selectedWithRoles = ranked.map((candidate, index) => {
        ...
        const missingRequired = (candidate.coverage?.missingRequired.length ?? 0)
            ? candidate.coverage.missingRequired
            : evidenceCoverageForText(params.queryAnalysis, candidate.text).missingRequired;
        const selectionReason = missingRequired.length > 0
...
function buildEvidencePlanAudit(params) {
    ...
    const statsBySlotLayer = new Map();
    ...
    return {
        ...
        slots: plan.slots.map((slot) => {
            ...
            const selectedEvidence = params.promptEvidence.filter((entry) => (entry.slotCoverage ?? []).some((coverage) => coverage.slotId === slot.id && (coverage.filled || coverage.coverageScore >= 0.3)));
            ...
            const bestCoverage = Math.max(0, ...selectedEvidence.flatMap((entry) => (entry.slotCoverage ?? [])
                .filter((coverage) => coverage.slotId === slot.id)
                .map((coverage) => coverage.coverageScore)));
            const missingFields = uniqueNonEmpty(selectedEvidence.flatMap((entry) => (entry.slotCoverage ?? [])
                .filter((coverage) => coverage.slotId === slot.id)
            ...
            return {
                ...
                sourceRefs: uniqueNonEmpty(selectedEvidence
...
function renderedPromptLineAudit(bundle) {
    return bundle.evidencePackets
        ...
        .sort((left, right) => (right.grade?.finalScore ?? right.coverage.confidence) -
        ...
        const lines = (packet.displayLines ?? [packet.primaryText]).filter((line) => line.trim().length > 0);
...
function supportRefsFromFactObjectValueJson(value) {
    ...
    return Array.isArray(refs)
        ? uniqueNonEmpty(refs.filter((entry) => typeof entry === "string"))
        : [];
}
function supportRefsFromSelectedFacts(store, facts, queryAnchors) {
    ...
    const selectedFacts = rankedFacts
        .filter((entry, index) => entry.anchorScore >= Math.max(0.38, bestAnchorScore - 0.08) ||
    ...
    for (const factRow of selectedFacts) {
        const factId = factRow.lineage?.canonicalKind === "fact"
...
function buildSupportRefChunkRows(params) {
    ...
    return dedupeEvidenceRows(chunks
        .map((chunk) => ({
        row: chunkEvidenceRowFromConversationChunk(chunk, params.queryAnchors),
        anchorScore: params.queryAnchors.length > 0
            ? queryAnchorSupport(chunk.content, params.queryAnchors)
            : 1,
    }))
...
function resolveReferentialQueryAnchors(store, ctx, query) {
    ...
    const authoritativeActiveProject = typeof activeProjectState?.valueJson.project === "string" &&
    ...
    if (/(?:前一个项目|上一个项目|previous project|last project)/iu.test(query)) {
        const previousProject = projectMentionSequence[1] ??
            projectCandidates.find((candidate) => normalizeText(candidate) !== normalizeText(activeProject)) ??
            projectCandidates[1] ??
            "";
        if (previousProject) {
    ...
    for (const hint of sideHints) {
        const ranked = recentTasks
            ...
            const score = references.reduce((best, reference) => Math.max(best, reference.includes(hint) || hint.includes(reference)
                ? 0.96
                : queryAnchorSupport(reference, [hint])), 0);
            return {
                task,
                references,
                score,
...
function groundingForRouteEntry(routeType, topEntry) {
    if (!topEntry) {
        return 0;
    }
...
function candidateCountForRoute(scheduled, topEntry) {
...
async function buildProbeSurface(store, ctx, query, searchQuery, background) {
    ...
    const workflowObjective = selectionObjectiveForRoute("workflow", searchQuery, ctx.now, ctx.sessionKey);
    const factualObjective = selectionObjectiveForRoute("factual", searchQuery, ctx.now, ctx.sessionKey);
    const temporalObjective = selectionObjectiveForRoute("temporal", searchQuery, ctx.now, ctx.sessionKey);
    const explanatoryObjective = selectionObjectiveForRoute("explanatory", searchQuery, ctx.now, ctx.sessionKey);
    ...
    const supersetObjective = selectionObjectiveForRoute("temporal", searchQuery, ctx.now, ctx.sessionKey);
    ...
    const taskAssociation = topScheduledMemoryScore(workflowScheduled, (entry) => entry.object.kind === "task");
    const stateAssociation = topScheduledMemoryScore(workflowScheduled, (entry) => entry.object.kind === "state");
    ...
    const workflowContinuity = clamp01(taskAssociation * 0.62 + stateAssociation * 0.26 + workflowSimilarity * 0.12);
    const contextAssociation = computeContextAssociation({
        taskAssociation,
        stateAssociation,
        topProbeScore: Math.max(workflowSimilarity, factualSimilarity, temporalSimilarity, explanatorySimilarity),
    });
    const workflowContextSupport = clamp01(workflowSimilarity * 0.72 + contextAssociation * 0.2 + workflowContinuity * 0.08);
    const explanatoryStructureStrength = explanatoryProbe.topEntry
        ? Math.max(explanatoryProbe.topEntry.object.profile.relationDensity, explanatoryProbe.topEntry.object.profile.connectivity)
        : 0;
    let explanatorySupport = explanatorySimilarity;
    if (explanatoryProbe.topEntry &&
...
function computeContextAssociation(params) {
    return clamp01(params.taskAssociation * 0.58 + params.stateAssociation * 0.24 + params.topProbeScore * 0.18);
}
function routeModerateThreshold(route, thresholds) {
    switch (route) {
        case "workflow":
...
function stripStructuredLabel(text) {
    const trimmed = text.trim();
...
function uniqueFocusedFragments(fragments, limit) {
    const seen = new Set();
    const result = [];
...
function focusedStateKeyFragment(stateKey) {
    switch (stateKey) {
...
function buildSummaryEvidenceFragments(routeType, summary) {
    const fragments = [];
...
function buildProjectionContextFragments(background, routeType) {
    return uniqueFocusedFragments(background.projectionBlocks
        .filter((block) => projectionRolesForRoute(routeType).includes(block.role))
        .flatMap((block) => block.lines.map(stripStructuredLabel)), 2);
}
function buildGateFocusedQuery(params) {
    ...
    const evidenceContext = uniqueFocusedFragments([...evidenceFragments, ...projectionContext], params.routeType === "explanatory" ? 4 : 3).join(" ");
...
export async function runShallowRecall(store, ctx, query, searchQuery = query, background = buildBackgroundRecallBundle(store, ctx)) {
    const trimmed = query.trim();
    if (!trimmed) {
        return {
            searchQuery,
            ...
            routeSummaries: Object.fromEntries(PRIMARY_ROUTE_TYPES.map((routeType) => [
                routeType,
                {
                    routeType,
                    support: 0,
    ...
    const routeSummaries = Object.fromEntries(surface.routeSurfaces.map((entry) => {
        const projectionSupport = projectionSupportForRoute(trimmed, background, entry.route);
        const projectionWeight = entry.route === "workflow"
            ? 0.92
            : entry.route === "factual"
                ? 0.8
        ...
        if (entry.route === "workflow" && surface.workflowContinuity >= 0.42) {
            effectiveSupport = clamp01(effectiveSupport + surface.contextAssociation * 0.08 + surface.workflowContinuity * 0.06);
        }
        if (entry.route === "workflow" &&
            projectionSupport >= 0.45 &&
            background.projectionBlocks.some((block) => block.role === "active_task" || block.role === "active_blocker")) {
        ...
        return [
            entry.route,
            {
                ...
                contradictionPressure: clamp01(Math.max(topEntry?.object.belief?.contradictionScore ?? 0, topEntry?.graphPenalty ?? 0)),
                grounding: Math.max(groundingForRouteEntry(entry.route, topEntry), projectionSupport * 0.88),
...
export function evaluateRecallQualityGate(params) {
    const topRoute = isPrimaryRouteType(params.shallow.routeHint)
        ...
        : undefined;
    const planJudgmentUnavailable = params.plan.judgmentMode === "degraded" || params.plan.judgmentMode === "disabled";
    if (!topRoute) {
        return {
    ...
    const routeAgreement = routeVotes.length > 0
    ...
    const supportDensity = clamp01(topSummary.support * 0.56 +
    ...
    const freshnessFloor = topRoute === "workflow"
        ? 0.22
        : topRoute === "factual"
            ? 0.05
            : topRoute === "temporal"
    ...
    if (supportDensity < downgradeSupportFloor ||
    ...
    if (supportDensity < acceptSupportDensityFloor ||
    ...
    return {
        decision: "accept_full",
        confidence: clamp01(supportDensity * 0.42 +
            routeAgreement * 0.18 +
            freshness * 0.1 +
            grounding * 0.18 +
...
export async function runRecallProbe(store, ctx, query, searchQuery = query, background = buildBackgroundRecallBundle(store, ctx)) {
    const thresholds = recallProbeThresholds(ctx.config);
    const trimmed = query.trim();
    if (!trimmed) {
        return {
    ...
    const signals = {
        ...
        backgroundTaskCount: background.tasks.length,
        backgroundStateCount: background.states.length,
        factualCandidateCount: surface.routeSurfaces
    ...
    if (surface.workflowContextSupport >= thresholds.workflowContinuation) {
        reasons.push(`probe:workflow-object:${surface.workflowContextSupport.toFixed(2)}/${surface.taskAssociation.toFixed(2)}/${surface.stateAssociation.toFixed(2)}`);
    }
    ...
    if (surface.explanatorySupport >=
        ...
        const explanatoryTopEntry = surface.routeSurfaces.find((entry) => entry.route === "explanatory")
            ?.support.topEntry;
        reasons.push(`probe:explanatory-object:${surface.explanatorySupport.toFixed(2)}/${Math.max(explanatoryTopEntry?.object.profile.relationDensity ?? 0, explanatoryTopEntry?.object.profile.connectivity ?? 0).toFixed(2)}`);
    }
    if (topProbeScore >= thresholds.hybridModerate) {
        reasons.push(`probe:hybrid-top:${topProbeScore.toFixed(2)}`);
    }
    probeScore = topProbeScore;
    if (strongestRouteCandidate &&
    ...
    const shouldEscalate = probeScore >= thresholds.escalate ||
    ...
    const focusedQuery = hintedRoute === "workflow" && background.tasks[0] && surface.workflowContinuity >= 0.45
...
function buildScoreDrivenRouteEvidenceDecision(routeType, candidates, judgmentMode) {
    ...
    const weighted = candidates.map((candidate) => {
        ...
        const confidence = typeof candidate.confidence === "number" ? clamp01(candidate.confidence) : 0.6;
        const weightedScore = clamp01(candidate.score * (0.72 + fit * 0.28) * (0.88 + confidence * 0.12));
        return {
            candidate,
            fit,
            weightedScore,
    ...
    const aggregateSupport = clamp01(top * 0.54 + averageTop * 0.2 + roleFit * 0.12 + confidence * 0.08 + coverage * 0.06);
    const topWeight = routeType === "temporal" || routeType === "explanatory"
        ? 0.94
        : routeType === "workflow"
            ? 0.9
            : 0.88;
...
function candidateRoleShare(candidates, role) {
    ...
    const matchedScore = topCandidates.reduce((sum, candidate) => sum + (candidate.role === role ? Math.max(candidate.score, 0) : 0), 0);
...
function buildQueryDrivenRouteDecision(evaluations, queryAnalysis, reasons) {
    ...
    const lowContextGenericQuery = queryAnalysis.turnMode === "mixed" &&
    ...
    const confidence = clamp01(top.queryWeight * 0.76 +
        top.evaluation.evidenceSupport * 0.16 +
        top.evaluation.finalScore * 0.08);
    if (lowContextGenericQuery
...
function reservedObjectiveRoutes(evaluations, routeDecision) {
    ...
    if (routeDecision.routeType === "mixed") {
        return new Set([top?.routeType, second?.routeType].filter(Boolean));
    }
    return new Set(routeDecision.routeType === "workflow" ||
...
function buildObjectiveBudgets(ctx, evaluations, routeDecision) {
    ...
    const normalizedSeed = active.reduce((sum, entry) => sum + entry.rawScore, 0) > 0
        ? seeded
        : seeded.map((entry) => ({
            ...entry,
            activated: true,
    ...
    return {
        objectiveBudgets: Object.fromEntries(weighted.map((entry) => [
            entry.routeType,
            {
                routeType: entry.routeType,
                weight: entry.weight,
...
export async function planRecallAllocation(store, ctx, query, searchQuery, hybridHits, queryAnalysis) {
    ...
    const scoreDrivenEvaluations = PRIMARY_ROUTE_TYPES.map((routeType) => {
        ...
        const finalScore = clamp01(priorWeights[routeType] * priorBlendWeight +
    ...
    const evaluations = scoreDrivenEvaluations;
    const ordered = [...evaluations].sort((left, right) => right.finalScore - left.finalScore);
    const reasons = [
        ...PRIMARY_ROUTE_TYPES.map((routeType) => `queryWeight:${routeType}:${(resolvedQueryAnalysis.routeWeights[routeType] ?? 0).toFixed(2)}`),
        ...ordered.map((entry) => `evidence:${entry.routeType}:final=${entry.finalScore.toFixed(2)} support=${entry.evidenceSupport.toFixed(2)} candidates=${entry.candidateCount}`),
    ];
    const routeDecision = queryDrivenAuthority
        ...
        : buildRouteDecision(evaluations, reasons, {
            ...
            preferHistoricalLookup: resolvedQueryAnalysis.queryShape.timeframe === "historical" ||
            ...
            preferChunkEvidence: resolvedQueryAnalysis.queryShape.evidenceNeed === "chunk" ||
...
function deriveProjectionOptions(ctx, plan, query, route, queryAnalysis) {
    ...
    let [stateLimit, taskLimit] = workflowBudget > 0 ? splitBudget(Math.max(2, workflowBudget), 0.5) : [0, 0];
    if (route.routeType === "workflow" && workflowBudget > 0) {
        stateLimit = Math.max(stateLimit, 3);
        taskLimit = Math.max(taskLimit, 1);
    }
    const alternateLimit = Math.max(0, Math.min(6, plan.globalOverflowObjects +
    ...
    return {
        ...
        preferTemporalEvents: route.routeType === "temporal" ||
            queryShape?.timeframe === "historical" ||
            queryShape?.timeframe === "compare",
        stateLimit,
        taskLimit,
        factLimit: factualBudget > 0 || explanatoryBudget > 0
            ? Math.max(factualBudget, explanatoryBudget > 0 ? 2 : 0, factualBudget > 0 ? 1 : 0, allowHistoricalFacts ? 2 : 0)
            : 0,
        eventLimit,
        graphLimit: explanatoryBudget > 0 ? Math.max(1, explanatoryBudget) : 0,
        alternateLimit,
        recallChunkBudget: eventLimit > 0
            ? Math.min(ctx.config.advanced.recallChunkBudget, eventLimit, queryAnalysis?.evidenceFidelity === "high" ||
                queryShape?.timeframe === "compare" ||
                queryShape?.granularity === "exact_detail"
                ? 3
                : eventLimit)
...
function sectionPromptBudget(bundle, routeType, fallback) {
    const planned = bundle.budgetPlan?.objectiveBudgets[routeType]?.promptChars;
    return Math.max(0, planned ?? fallback);
}
function sliceAlternates(bundle) {
    if (bundle.routeConfidence >= 0.6 &&
...
export async function retrieveEvidence(store, ctx, query, searchQuery = query, auditOptions = {}) {
    const behavioralGuidance = collectBehavioralGuidanceFromObjects(store, ctx);
    const queryAnalysis = auditOptions.queryAnalysis ??
    ...
    if (referentResolution.anchors.length > 0) {
        for (const routeType of PRIMARY_ROUTE_TYPES) {
            allocationPlan.focusedQueries[routeType] = appendAnchorsToQuery(allocationPlan.focusedQueries[routeType] || retrievalQuery, referentResolution.anchors);
        }
    }
    ...
    const allowHistoricalFacts = queryAnalysis.queryShape.timeframe === "historical" ||
        queryAnalysis.queryShape.timeframe === "compare";
    ...
    if (alternates.length === 0) {
        alternates = dedupeEvidenceRows([...candidateAlternateRows, ...filterBootstrapRows(rowsFromSearchHits(hybridHits))].filter((entry) => !shouldSuppressRecallText(entry.text)), 6);
    }
    ...
    const hasAnchoredWorkflowTask = route.routeType === "workflow" &&
        queryAnalysis.queryShape.evidenceNeed === "workflow_context" &&
    ...
    const supportLinkedSnippetCandidates = shouldUseExactSnippetSupport(queryAnalysis)
        ? buildSourceRefFallbackSnippets({
            ...
            limit: 3,
            queryAnchors,
        }).map((entry) => ({ ...entry, source: "support_ref" }))
        : [];
    ...
    const selectedExactSnippets = shouldUseExactSnippetSupport(queryAnalysis)
        ? rankedExactSnippetCandidates.length > 0
            ? rankedExactSnippetCandidates.map((entry) => {
                ...
                const matchedHit = candidateGenerationResult?.searchHits.find((hit) => hit.docId === snippetId) ??
                    rawHybridHits.find((hit) => hit.docId === snippetId);
                return {
                    snippetId,
                    text: entry.text,
                    sourceRef: typeof matchedHit?.metadata.sourceRef === "string"
    ...
    if (shouldUseExactSnippetSupport(queryAnalysis) &&
    ...
    emitContradictionSignals(store, ctx, {
        query,
        routeType: route.routeType === "mixed" || route.routeType === "unknown"
            ? "explanatory"
            : route.routeType,
        graphEdges: bundle.graph.edges,
    });
    if (ctx.config.advanced.enableTelemetryAudit) {
        const activePromptEvidence = bundle.promptEvidence.filter((entry) => !entry.dropReason && entry.role !== "alternate");
        ...
        const selected = {
            ...
            promptEvidence: bundle.promptEvidence.map((entry) => ({
                ...
                rawText: entry.rawText && entry.rawText !== entry.text
            ...
            rankedPromptEvidence: bundle.promptEvidence
                .filter((entry) => !entry.dropReason)
                .sort((left, right) => (right.injectionScore ?? right.priority) - (left.injectionScore ?? left.priority))
                .map((entry) => ({
                id: entry.id,
                surface: entry.surface,
                sourceRef: entry.sourceRef,
...
function evidencePacketRenderLine(packet, bundle) {
    ...
    const quantity = typeof packet.quantityHint === "number"
...
export function renderEvidenceBundle(bundle, maxChars) {
    ...
    const packetEvidenceLines = bundle.evidencePackets
        ...
        if (displayLines.length > 0) {
            return displayLines.map((line) => line.trim().startsWith("-") ? line.trim() : `- ${line.trim()}`);
        }
