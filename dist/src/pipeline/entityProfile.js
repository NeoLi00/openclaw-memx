...
export function buildEntityProfileDoc(store, ctx, entity) {
    ...
    const aliases = [
        ...new Set([...entity.aliases, ...aliasSources.map((entry) => entry.aliasText)].filter(Boolean)),
    ];
    ...
    const relationNeighborIds = [
        ...new Set(graph.edges
            ...
            .filter((entityId) => Boolean(entityId) && entityId !== entity.entityId)),
    ];
    const metadata = {
...
export function refreshEntityProfileDocs(store, ctx, entityIds) {
