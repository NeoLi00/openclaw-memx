import { clamp01, normalizeText, nowIso, objectRecord, stableHash, truncateText, } from "../support.js";
...
import { buildMaintenanceContractMetadata, summarizeMaintenanceContractDiagnostics, } from "./maintenanceContract.js";
...
function selectAbstractionCandidatesByType(candidatesByType) {
    ...
    for (const candidate of leftovers) {
        ...
        selectedIds.add(candidate.candidateId);
        selected.push(candidate);
        const typeStats = stats.byType[candidate.abstractionType] ??
...
function buildBeliefMap(beliefs) {
    const entries = beliefs
        ...
        .map((belief) => [`${belief.memoryKind}:${belief.contentRef}`, belief]);
...
function supportDiversityScore(entries) {
    ...
    return clamp01(Math.min(0.45, (entries.length - 1) * 0.12) +
...
function temporalPersistenceScore(entries) {
    ...
    return clamp01(Math.min(0.5, spreadDays / 7) +
...
function contradictionPressure(entries, siblingGroups) {
    ...
    const competingConfidence = siblingGroups
        ...
        .reduce((sum, group) => sum + group.reduce((groupSum, entry) => groupSum + entry.confidence, 0), 0);
    const localConflict = totalConfidence + competingConfidence > 0
        ? competingConfidence / (totalConfidence + competingConfidence)
        : 0;
    return clamp01(localConflict * 0.75 + average(entries.map((entry) => entry.contradictionScore)) * 0.25);
}
function conceptSupportDiversity(entries) {
    const supportKinds = new Set(entries.map((entry) => entry.supportKind));
    const relationFamilies = new Set(entries.map((entry) => entry.relationLabel));
    return clamp01(Math.min(0.32, Math.max(0, entries.length - 1) * 0.08) +
...
function conceptTemporalPersistence(entries) {
    ...
    const observedAt = entries.map((entry) => entry.observedAt).sort();
    const spreadDays = daysBetween(observedAt[0], observedAt.at(-1));
    return clamp01(Math.min(0.54, spreadDays / 30) +
...
function conceptSummary(entityName, relationLabels) {
    const relationSummary = relationLabels.length > 0 ? relationLabels.join(", ") : "stable supporting relations";
    return truncateText(`A stable concept cluster appears around ${entityName}, linking ${relationSummary}.`, 220);
}
function conceptCandidateStage(params) {
    if (params.contradictionScore >= 0.46) {
        return "quarantined";
    }
    if (params.confidence >= 0.82 &&
...
function graphHypothesisSupportDiversity(entries) {
    const supportKinds = new Set(entries.map((entry) => entry.supportKind));
    const sessions = new Set(entries.map((entry) => entry.sessionKey).filter(Boolean));
    return clamp01(Math.min(0.4, Math.max(0, entries.length - 1) * 0.1) +
...
function graphHypothesisTemporalPersistence(entries) {
    ...
    const observedAt = entries.map((entry) => entry.observedAt).sort();
    const spreadDays = daysBetween(observedAt[0], observedAt.at(-1));
    return clamp01(Math.min(0.5, spreadDays / 10) +
...
function graphHypothesisContradictionPressure(entries, siblingGroups) {
    ...
    const localConflict = totalConfidence + competingConfidence > 0
        ? competingConfidence / (totalConfidence + competingConfidence)
        : 0;
    return clamp01(localConflict * 0.72 + average(entries.map((entry) => entry.contradictionScore)) * 0.28);
}
...
function pushInferredGraphSupport(supports, params) {
...
function buildGraphHypothesisCandidate(params) {
    ...
    const stabilityScore = clamp01(average(params.supports.map((entry) => entry.stabilityScore)) * 0.34 +
        temporalPersistence * 0.26 +
        supportDiversity * 0.16 +
        (params.relationClass === "observed" ? 0.12 : 0.06) +
        (1 - contradictionScore) * 0.12);
    const confidence = clamp01(average(params.supports.map((entry) => entry.confidence)) * 0.42 +
    ...
    return {
        ...
        supportBeliefIds: [
            ...new Set(params.supports
                .map((entry) => entry.beliefId)
                .filter((beliefId) => Boolean(beliefId))),
        ],
        confidence,
        usefulnessScore,
        stabilityScore,
...
function buildGraphHypothesisCandidates(store, ctx, beliefMap, recentEvents, _activeTasks) {
    ...
    return [...grouped.values()]
        .map((group) => buildGraphHypothesisCandidate({
        agentId: ctx.agentId,
        scope: group[0].scope,
...
function structuredWorkflowGuidanceSemanticSource(fact) {
    ...
    if (fact.predicate === WORKFLOW_GUIDANCE_PREDICATE ||
        semanticFamily === "strategy_like" ||
        semanticFamily === "workflow") {
        return "upstream_structured";
    }
    if (stringValue(guidance?.guidanceText) &&
...
function explicitWorkflowSupportChunks(store, fact) {
    const guidance = objectRecord(fact.objectValueJson?.guidance);
    const supportRefs = stringSet(objectRecord(guidance)?.supportContentRefs ?? fact.objectValueJson?.supportContentRefs);
...
function resolutionGroundingSignal(params) {
    ...
    const promotionScore = typeof metadata.candidateResolutionPromotionScore === "number" &&
    ...
    const score = clamp01((explicitEvidenceCount > 0 ? 0.36 : 0) +
        ...
        promotionScore * 0.24);
    return {
        grounded: (explicitEvidenceCount > 0 || hasToolEvidence || promotionScore >= 0.74 || hasOutcomeKey) &&
            score >= 0.28 &&
            complexity <= 0.42 &&
...
function buildExplicitWorkflowPatternCandidates(store, ctx, beliefMap) {
    ...
    for (const fact of explicitFacts) {
        ...
        const supportSummary = supportChunks.length > 0
            ? truncateText(supportChunks
        ...
        const belief = beliefMap.get(`fact:${fact.factId}`);
        const explicitInstruction = fact.canonicalSubject === "user" &&
            fact.status === "active" &&
        ...
        const usefulnessScore = clamp01((belief?.usefulnessScore ?? 0.58) * 0.64 +
            fact.confidence * 0.18 +
            (belief?.stage === "active" ? 0.18 : 0.08) +
            (explicitInstruction ? 0.08 : 0) +
            (supportChunks.length > 0 ? 0.08 : 0));
        const stabilityScore = clamp01((belief?.stabilityScore ?? 0.62) * 0.58 +
            (belief?.stage === "active" ? 0.18 : 0.08) +
            (1 - contradictionScore) * 0.24 +
            (explicitInstruction ? 0.06 : 0) +
            (supportChunks.length > 0 ? 0.06 : 0));
        const confidence = clamp01(average([fact.confidence, belief?.posteriorConfidence ?? fact.confidence]) * 0.72 +
...
function buildGroundedWorkflowPatternCandidates(store, ctx, beliefMap) {
    ...
    for (const task of recentTasks) {
        ...
        const supportChunkIds = [
            ...supportChunks
                .filter((chunk) => chunk.role === "user" || chunk.role === "assistant" || chunk.role === "tool")
        ...
        const hasGroundedEvidence = hasToolEvidence ||
        ...
        const usefulnessScore = clamp01((belief?.usefulnessScore ?? 0.58) * 0.44 +
            (hasToolEvidence ? 0.2 : 0.1) +
            (hasUserEvidence ? 0.12 : 0.04) +
            (phase === "resolved" ? 0.12 : 0.08) +
            (hasGroundedResolution ? 0.08 : 0) +
            (1 - contradictionScore) * 0.12);
        const stabilityScore = clamp01((belief?.stabilityScore ?? 0.6) * 0.42 +
            (hasToolEvidence ? 0.18 : 0.06) +
            (hasAssistantEvidence ? 0.08 : 0) +
            (phase === "resolved" ? 0.12 : 0.08) +
            (hasGroundedResolution ? 0.08 : 0) +
            (1 - contradictionScore) * 0.2);
        const confidence = clamp01(average([belief?.posteriorConfidence ?? 0.68, task.status === "completed" ? 0.76 : 0.7]) *
            0.56 +
...
function factMatchesConceptSeed(fact, seedName) {
    ...
    return (normalizeText(fact.canonicalSubject) === normalizedSeed ||
...
function buildConceptSupportEntries(params) {
    ...
    const edgeSupports = params.graph.edges.map((edge) => {
        ...
        const contentRef = edge.sourceKind === "stored"
        ...
        return {
            ...
            confidence: clamp01(average([edge.confidence, belief?.posteriorConfidence ?? (isSeedIncident ? 0.7 : 0.62)])),
            ...
            contradictionScore: belief?.contradictionScore ?? (edge.relType === "contradicts" ? 0.42 : 0.08),
            beliefId: belief?.beliefId,
        };
    });
    const supportBeliefIds = [
        ...new Set([...factSupports, ...edgeSupports]
            .map((entry) => entry.beliefId)
            .filter((beliefId) => Boolean(beliefId))),
    ];
...
function upsertConceptGraphEdge(edges, edge) {
    ...
    edges.set(key, {
        ...
        updatedAt: Date.parse(existing.updatedAt ?? "") >= Date.parse(edge.updatedAt ?? "")
            ? existing.updatedAt
            : edge.updatedAt,
...
function buildConceptCandidate(params) {
    ...
    const graphCoverage = clamp01((params.graph.edges.length + params.graph.nodes.length + params.graph.pathCandidates.length) /
        18);
    const structuralStrength = clamp01(relationDensity * 0.34 +
    ...
    const usefulnessScore = clamp01(average(supports.map((entry) => entry.usefulnessScore)) * 0.58 +
        pathScoreAverage * 0.18 +
        relationDensity * 0.14 +
        supportDiversity * 0.1);
    const stabilityScore = clamp01(average(supports.map((entry) => entry.stabilityScore)) * 0.42 +
        temporalPersistence * 0.18 +
        structuralStrength * 0.22 +
        supportDiversity * 0.08 +
        (1 - contradictionScore) * 0.1);
    const confidence = clamp01(average(supports.map((entry) => entry.confidence)) * 0.34 +
...
function buildDerivedStateCandidate(params) {
    ...
    const usefulnessScore = clamp01(average(params.supports.map((entry) => entry.usefulnessScore)));
    const stabilityScore = clamp01(average(params.supports.map((entry) => entry.stabilityScore)) * 0.35 +
    ...
    const repeatedCandidates = (await deriveWorkflowPatternSummaries(store, ctx)).map((pattern) => {
        const stage = workflowCandidateStage({
            ...
            groundedByRoles: Array.isArray(pattern.metadataJson.groundedByRoles)
                ? pattern.metadataJson.groundedByRoles.filter((role) => typeof role === "string" && role.trim().length > 0)
                : undefined,
...
export async function runAbstractionJobs(store, ctx, options = {
    refineWithLlm: false,
}) {
    ...
    try {
        ...
        const factFamilies = Number(store.client
        ...
        const siblingGroupsByState = new Map();
        ...
        const derivedStateCandidates = [...groupedByKey.values()]
            .map((group) => buildDerivedStateCandidate({
            agentId: ctx.agentId,
            scope: group[0].scope,
        ...
        for (const candidate of refinement.candidates) {
            ...
            candidate.metadataJson = buildMaintenanceContractMetadata({
                ...
                semanticSources: Array.isArray(candidate.metadataJson.semanticSources)
                    ? candidate.metadataJson.semanticSources.filter((source) => typeof source === "string" && source.trim().length > 0)
                    : undefined,
                authoritySource: options.refineWithLlm === true ? "llm_upgrade" : "deterministic_aggregated",
                generatedFrom: typeof candidate.metadataJson.generatedFrom === "string" ||
        ...
        const semanticSources = [
            ...new Set(refinement.candidates.flatMap((candidate) => {
        ...
        const stats = {
            ...
            authoritySources: options.refineWithLlm === true
                ? ["deterministic_aggregated", "llm_upgrade"]
                : ["deterministic_aggregated"],
            semanticSources: options.refineWithLlm === true
