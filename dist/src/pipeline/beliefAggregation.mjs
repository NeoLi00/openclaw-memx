...
function sourceReliabilityFromMarker(marker) {
	switch (marker) {
		case "tool": return SOURCE_RELIABILITY.tool;
		case "user": return SOURCE_RELIABILITY.user;
		case "assistant": return SOURCE_RELIABILITY.assistant;
		case "synthesized": return SOURCE_RELIABILITY.synthesized;
		default: return SOURCE_RELIABILITY.default;
	}
}
...
function fallbackSourceReliability(memoryKind) {
	return FALLBACK_SOURCE_RELIABILITY[memoryKind] ?? FALLBACK_SOURCE_RELIABILITY.default;
}
function stagePolicy(memoryKind) {
	switch (memoryKind) {
		case "state": return {
			probationaryPosterior: .54,
			activePosterior: .72,
			quarantineContradiction: .78,
			supersessionGap: .12,
			decayAfterDays: 14,
			minSupportForProbationary: .34,
			minSupportForActive: .52
		};
		case "task": return {
			probationaryPosterior: .58,
			activePosterior: .72,
			quarantineContradiction: .8,
			supersessionGap: .14,
			decayAfterDays: 21,
			minSupportForProbationary: .32,
			minSupportForActive: .46
		};
		case "fact": return {
			probationaryPosterior: .66,
			activePosterior: .82,
			quarantineContradiction: .72,
			supersessionGap: .14,
			decayAfterDays: 90,
			minSupportForProbationary: .38,
			minSupportForActive: .38
		};
		case "event": return {
			probationaryPosterior: .54,
			activePosterior: .7,
			quarantineContradiction: .84,
			supersessionGap: .16,
			decayAfterDays: 45,
			minSupportForProbationary: .32,
			minSupportForActive: .48
		};
		case "graph_edge": return {
			probationaryPosterior: .6,
			activePosterior: .76,
			quarantineContradiction: .76,
			supersessionGap: .14,
			decayAfterDays: 120,
			minSupportForProbationary: .36,
			minSupportForActive: .52
		};
		case "chunk": return {
			probationaryPosterior: .48,
			activePosterior: .7,
			quarantineContradiction: .88,
			supersessionGap: .18,
			decayAfterDays: 10,
			minSupportForProbationary: .3,
			minSupportForActive: .5
		};
		case "strategy": return {
			probationaryPosterior: .7,
			activePosterior: .86,
			quarantineContradiction: .68,
			supersessionGap: .12,
			decayAfterDays: 180,
			minSupportForProbationary: .45,
			minSupportForActive: .64
		};
	}
}
function loadContentSnapshot(store, agentId, memoryKind, contentRef) {
	...
	switch (memoryKind) {
		case "state": {
			const row = store.client.prepare(`SELECT scope, confidence, source_ref AS sourceRef, updated_at AS updatedAt
             FROM state_kv
            WHERE agent_id = ?
              AND scope || ':' || key = ?
            LIMIT 1`).get(agentId, contentRef);
			if (!row) return null;
			return {
				scope: row.scope,
		...
		case "task": {
			const row = store.client.prepare(`SELECT scope, started_at AS startedAt, updated_at AS updatedAt, status
             FROM conversation_tasks
            WHERE agent_id = ?
              AND task_id = ?
            LIMIT 1`).get(agentId, contentRef);
			if (!row) return null;
			const priorConfidence = row.status === "active" ? .72 : .64;
			return {
				scope: row.scope,
				priorConfidence,
		...
		case "fact": {
			const row = store.client.prepare(`SELECT scope, confidence, created_at AS createdAt, updated_at AS updatedAt
             FROM facts
            WHERE agent_id = ?
              AND fact_id = ?
            LIMIT 1`).get(agentId, contentRef);
		...
		case "event": {
			const row = store.client.prepare(`SELECT scope, confidence, observed_at AS observedAt, source_kind AS sourceKind
		...
		case "graph_edge": {
			const row = store.client.prepare(`SELECT scope, confidence, evidence_ref AS evidenceRef, created_at AS createdAt, updated_at AS updatedAt
             FROM graph_edges
            WHERE agent_id = ?
              AND edge_id = ?
            LIMIT 1`).get(agentId, contentRef);
		...
		case "chunk": {
			const row = store.client.prepare(`SELECT scope, role, created_at AS createdAt, updated_at AS updatedAt
...
function posteriorConfidence(params) {
	const contradictionPenalty = Math.min(Math.sqrt(params.contradictionScore) * POSTERIOR_CONTRADICTION_WEIGHT, .25);
	const demotionPenalty = Math.min(Math.sqrt(params.demotion) * POSTERIOR_DEMOTION_WEIGHT, .12);
	const correctionPenalty = Math.min(Math.sqrt(params.correction) * POSTERIOR_CORRECTION_WEIGHT, .22);
	const staleDecayPenalty = Math.min(Math.sqrt(params.staleDecay) * POSTERIOR_STALE_DECAY_WEIGHT, .1);
	return clamp01(params.priorConfidence + (params.usefulnessScore - POSTERIOR_MIDPOINT) * POSTERIOR_USEFULNESS_WEIGHT + (params.stabilityScore - POSTERIOR_MIDPOINT) * POSTERIOR_STABILITY_WEIGHT + params.outcomeSupportScore * POSTERIOR_OUTCOME_WEIGHT + params.selfConsistency * POSTERIOR_CONSISTENCY_WEIGHT + params.temporalStability * POSTERIOR_TEMPORAL_WEIGHT + params.promotion * POSTERIOR_PROMOTION_WEIGHT - correctionPenalty - contradictionPenalty - demotionPenalty - staleDecayPenalty + params.repeatedUse * POSTERIOR_REPEATED_USE_WEIGHT);
}
function signalEvidenceSupportScore(summary) {
	return clamp01(summary.retrievalSupportAvg * .26 + summary.futureUsefulnessAvg * .22 + summary.outcomeFeedbackAvg * .16 + summary.assistantGroundingAvg * .1 + summary.selfConsistencyAvg * .1 + summary.temporalStabilityAvg * .08 + summary.repeatedUseAvg * .06 + summary.promotionAvg * .08);
}
...
function entityIdFromScopedSemanticKey(semanticKey) {
	return semanticKey.match(/^entity:([^:]+):/u)?.[1];
}
function canonicalTargetContentRef(contentRef, semanticKey) {
	return isEntityScopedSemanticKey(semanticKey) ? void 0 : contentRef;
}
...
function taskPredicateFromSemanticKey(signal) {
	if (signal.memoryKind === "task" && signal.contentRef) {
		...
		if (taskOutcomeMatch?.[1]) return `outcome:${normalizeText(taskOutcomeMatch[1])}`;
	}
	return normalizeText(signal.semanticKey).replace(/:/g, ".");
}
function resolveBeliefTargetKey(store, ctx, scope, signal) {
	const base = {
	...
	if (isEntityScopedSemanticKey(signal.semanticKey)) return {
		...base,
		contentRef: void 0,
	...
	if (graphMatch) {
		...
		if (srcId && relType && dstId && store.graphRepo.getEntityById(srcId) && store.graphRepo.getEntityById(dstId)) return {
			...base,
			contentRef: void 0,
	...
	if (factMatch) {
		...
		if (subject && predicate) {
			...
			if (resolution.method !== "uncertain") return {
				...base,
				contentRef: void 0,
	...
	if (stateMatch?.[1]) {
		const resolution = resolveEntityMention(store, ctx, buildEntityMention({
			...
			supportText: signal.semanticKey,
			observedAt: signal.createdAt,
			metadataJson: { generatedFrom: "belief-state-key" }
		}), {
			createIfMissing: false,
			persist: false
		});
	...
	if (signal.memoryKind === "task" && signal.contentRef) return {
		...base,
...
function canonicalizeSignalEntityTarget(store, ctx, scope, signal) {
	...
	if (resolved.semanticKey === signal.semanticKey && resolved.contentRef === signal.contentRef && resolved.resolutionMethod === "unchanged") return {
		...signal,
		metadataJson: {
			...signal.metadataJson,
...
function signalSupportContentRef(signal) {
	if (signal.contentRef) return signal.contentRef;
	const original = signal.metadataJson.originalContentRef;
	return typeof original === "string" && original.trim() ? original.trim() : void 0;
}
...
function mergeGroupsWithExisting(signals, existingBeliefs) {
	const groups = /* @__PURE__ */ new Map();
	for (const group of groupSignals(signals)) groups.set(targetGroupKey({
...
function aggregateBeliefs(store, ctx, options = {}) {
	const existingBeliefs = store.beliefRepo.listByAgent({ agentId: ctx.agentId });
	const deltaSignals = options.signalWindow ? store.auditRepo.listSignals({
		agentId: ctx.agentId,
	...
	const dueBeliefs = existingBeliefs.filter((belief) => (!options.scopes || options.scopes.includes(belief.scope)) && typeof belief.reevaluationDueAt === "string" && belief.reevaluationDueAt <= ctx.now);
	if (deltaSignals.length === 0 && dueBeliefs.length === 0 && existingBeliefs.length === 0) return {
		beliefsUpserted: 0,
		signalsProcessed: 0,
	...
	const touchedTargetKeys = /* @__PURE__ */ new Set();
	const touchedFamilyKeys = /* @__PURE__ */ new Set();
	...
	if (options.signalWindow && touchedEntityTargetKeys.size > 0) for (const signal of canonicalizeSignalEntityTargets(store, ctx, resolverScope, store.auditRepo.listSignals({
		agentId: ctx.agentId,
		...options.signalWindow.until ? { until: options.signalWindow.until } : {}
	}))) {
		...
		if (touchedEntityTargetKeys.has(key)) signalsById.set(signal.signalId, signal);
	}
	const signals = [...signalsById.values()];
	const relevantExistingBeliefs = options.signalWindow ? existingBeliefs.filter((belief) => touchedTargetKeys.has(beliefTargetKey({
		memoryKind: belief.memoryKind,
		contentRef: belief.contentRef,
		semanticKey: belief.semanticKey
	...
	if (signals.length === 0 && relevantExistingBeliefs.length === 0) return {
		beliefsUpserted: 0,
	...
	for (const group of mergeGroupsWithExisting(signals, relevantExistingBeliefs)) {
		...
		if (!firstSignal && !existingKeyBelief) continue;
		...
		const summary = group.signals.length ? summarizeSignals(group.signals) : {
		...
		const priorConfidence = snapshot?.priorConfidence ?? existing?.priorConfidence ?? fallbackPriorConfidence(group.memoryKind);
		const sourceReliability = snapshot?.sourceReliability ?? existing?.sourceReliability ?? fallbackSourceReliability(group.memoryKind);
		const usefulnessScore = group.signals.length ? clamp01(summary.retrievalSupportAvg * .35 + summary.futureUsefulnessAvg * .25 + summary.repeatedUseAvg * .14 + Math.min(summary.useCount / 5, 1) * .26) : existing?.usefulnessScore ?? 0;
		const stabilityScore = group.signals.length ? clamp01(sourceReliability * .45 + summary.selfConsistencyAvg * .3 + summary.temporalStabilityAvg * .15 + summary.retrievalSupportAvg * .1) : existing?.stabilityScore ?? 0;
		const contradictionScore = clamp01(summary.contradictionAvg);
		const outcomeSupportScore = clamp01(summary.outcomeFeedbackAvg * .75 + summary.assistantGroundingAvg * .25);
		...
		const firstSeenAt = minIso([
			existing?.firstSeenAt,
			snapshot?.firstSeenAt,
			firstSignal?.createdAt
		]) ?? ctx.now;
		...
		const supportScore = clamp01(usefulnessScore * .35 + stabilityScore * .22 + outcomeSupportScore * .12 + summary.selfConsistencyAvg * .08 + summary.temporalStabilityAvg * .05 + signalEvidenceScore * .35);
		const inactiveDays = daysBetween(summary.lastUsedAt ?? lastSeenAt, ctx.now);
		const record = {
			beliefId,
			agentId: ctx.agentId,
			scope,
			...
			metadataJson: {
				...
				targetKeyAudit: [...new Map(group.signals.map((signal) => signal.metadataJson.beliefTargetResolution).filter((value) => Boolean(value)).map((value) => [JSON.stringify(value), value])).values()],
				...
				supportContentRefs: [...new Set(group.signals.map((signal) => signalSupportContentRef(signal)).filter(Boolean))],
	...
	for (const provisional of provisionalRecords) {
		...
		const strongDemotion = provisional.summary.demotionAvg >= .88;
		const moderateDemotion = provisional.summary.demotionAvg >= .58;
		const strongCorrection = provisional.summary.correctionAvg >= .74;
		const moderateCorrection = provisional.summary.correctionAvg >= .48;
		const explicitStaleDecay = provisional.summary.staleDecayAvg >= .58;
		const contradictionTriggered = provisional.record.contradictionScore >= policy.quarantineContradiction || strongDemotion && provisional.record.contradictionScore >= Math.max(policy.quarantineContradiction * .55, .32);
		let stage = "candidate";
		...
		} else if (previousStage === "superseded") if (provisional.record.posteriorConfidence >= policy.probationaryPosterior && provisional.supportScore >= policy.minSupportForProbationary) {
			stage = "probationary";
		...
		} else if (moderateDemotion) if (stale || previousStage === "active" || previousStage === "probationary" || previousStage === "decaying") {
			stage = "decaying";
			reason = "demotion-soft-decay";
		} else if (provisional.record.posteriorConfidence >= policy.probationaryPosterior && provisional.supportScore >= policy.minSupportForProbationary) {
			stage = "probationary";
		...
		else if (provisional.record.posteriorConfidence >= policy.activePosterior && provisional.supportScore >= policy.minSupportForActive && (!stale || provisional.record.memoryKind === "fact" || provisional.record.stabilityScore >= .82)) {
			stage = "active";
			reason = "strong-support";
		} else if (stale && (existingById.get(provisional.record.beliefId)?.stage === "active" || existingById.get(provisional.record.beliefId)?.stage === "probationary" || existingById.get(provisional.record.beliefId)?.stage === "decaying")) {
			stage = "decaying";
	...
	for (const bucket of families.values()) {
		...
		const dominant = [...bucket].filter((entry) => entry.record.stage !== "quarantined").sort((left, right) => {
			if (right.record.posteriorConfidence !== left.record.posteriorConfidence) return right.record.posteriorConfidence - left.record.posteriorConfidence;
		...
		for (const provisional of bucket) {
			if (provisional.record.beliefId === dominant.record.beliefId || provisional.record.stage === "quarantined" || dominant.record.posteriorConfidence - provisional.record.posteriorConfidence < policy.supersessionGap || dominant.record.posteriorConfidence < policy.probationaryPosterior) continue;
	...
	for (const provisional of provisionalRecords) {
		...
		if (provisional.record.stage === "quarantined") {
			...
			provisional.record.reevaluationDueAt = provisional.record.reevaluationDueAt ?? addDays(ctx.now, 30);
			beliefsSuperseded += 1;
		} else if (provisional.summary.demotionAvg >= .58) provisional.record.reevaluationDueAt = addDays(ctx.now, 7);
		else if (provisional.record.contradictionScore >= .45) provisional.record.reevaluationDueAt = addDays(provisional.record.lastSeenAt, 7);
		if (provisional.record.reevaluationDueAt) beliefsNeedingReevaluation += 1;
		if (previousStage && previousStage !== provisional.record.stage) if (stageRank[provisional.record.stage] > stageRank[previousStage]) beliefsPromoted += 1;
		else beliefsDemoted += 1;
		provisional.record.metadataJson = {
			...provisional.record.metadataJson,
			stageReason: provisional.stageReason,
			previousStage: previousStage ?? null,
			stageTransitionAt: previousStage && previousStage !== provisional.record.stage ? ctx.now : void 0
		};
