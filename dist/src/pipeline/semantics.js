...
import { analyzeCorrectionHint, analyzeRecallQueryShape, canonicalStateKey, expandStateKeyAliases, extractTimeHints, extractQueryAnchors, hasExplicitRememberIntent, inferEntityNames, isDeicticWorkflowReferenceQuery, isBroadTemporalQuery, isLowValueChatter, isQuestionLike, normalizedEntityId, normalizeGraphRelationType, parseAllRelations, parseRelation, predicateHint, queryAnchorSupport, seedEntityNamesFromQuery, semanticRoleHint, stripLead, tokenizeSearchTerms, wantsProjectProfileSnapshot, wantsCurrentFactualSnapshot, wantsHistoricalFacts, } from "./semantic/heuristics.js";
import { judgePreferenceSignal, judgeQueryRoute, judgeRelationHint, judgeAllRelationHints, judgeWorkflowState, routeExplanatory, routeFactual, routeTemporal, routeWorkflow, } from "./semantic/judges.js";
...
export function inferTemporalSince(query, now) {
...
export function parsePreferenceSignal(text) {
...
export function canonicalizePreferencePredicate(value) {
...
export function canonicalizePreferenceHint(hint) {
    if (!hint) {
        return null;
    }
...
export function parseWorkflowState(text) {
...
function decisionConfidence(text) {
    ...
    const durableInstructionCue = /\b(?:must use|always use|never use|default to|stick with|we are going with|from now on|keep using)\b/iu.test(stripped) ||
        /(?:必须用|只能用|默认(?:改成|设为|采用|用)?|以后(?:都)?用|今后(?:都)?用|统一用|固定用|记着(?:以后)?|优先按这个顺序)/u.test(stripped);
    const explicitDecision = durableInstructionCue ||
        /\b(?:we decided(?: to)?|decision is|constraint is)\b/iu.test(stripped) ||
        /(?:决定(?:用|采用|改用|选用)?|约束(?:是|为)?)/u.test(stripped);
    const defaultedPreference = Boolean(preference) &&
    ...
    const historicalNarrativeCue = /(?:之前|后来|最早|第一次|上次|last time|earlier|before|initially)/iu.test(stripped) ||
        timeHintCount > 0;
    if ((!explicitDecision && !defaultedPreference) ||
        correction ||
...
export function analyzeSemanticHints(text) {
    ...
    const correction = analyzeCorrectionHint({
        ...
        predicate: (relation?.predicate === "related_to" && relation.rawPredicate ? relation.rawPredicate : undefined) ??
            relation?.predicate ??
...
export function hasPreferenceHint(text) {
    return analyzeSemanticHints(text).preference !== null;
}
export function hasTaskStateHint(text) {
    return analyzeSemanticHints(text).workflow !== null;
}
export function hasRelationHint(text) {
    return analyzeSemanticHints(text).relation !== null;
}
export function hasDecisionHint(text) {
    return analyzeSemanticHints(text).decision !== null;
}
export function analyzeQueryShape(query) {
