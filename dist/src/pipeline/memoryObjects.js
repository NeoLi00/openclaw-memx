...
import { TASK_METADATA_WORKFLOW_SNAPSHOT_DESCRIPTORS, isSnapshotFactualStateKey, sanitizeTaskMetadata, } from "./authority.js";
import { filterBootstrapRows, isBootstrapMemoryContamination } from "./bootstrapFilter.js";
import { BACKGROUND_ACTIVE_TASKS_LIMIT, BACKGROUND_STATE_ROWS_LIMIT, BEHAVIORAL_GUIDANCE_LIMIT, STRATEGY_GUIDANCE_LIMIT, STRATEGY_MAX_CONTRADICTION, } from "./constants.js";
...
import { dedupeEvidenceRows, describeStateValue, formatFactLine, lineageFromMetadata, normalizeSearchText, rowsFromSearchHits, shouldSuppressRecallText, splitLabelValue, toEvidenceRow, } from "./memoryObjectsHelpers.js";
import { buildWorkingProjectionBlocks, createMemorySelectionObjective, } from "./memoryObjectsProjection.js";
import { canonicalStateKey, inferTemporalSince, inferEntityNames, predicateHint, queryAnchorSupport, seedEntityNamesFromQuery, } from "./semantics.js";
...
function eventStructuredSummary(event) {
    ...
    const summary = metadataString(objectRecord(event.metadataJson.memxTemporalFacet)?.summary) ??
...
function eventRows(events) {
    return events.map((event, index) => toEvidenceRow({
        ...
        score: clamp01(Math.max(0.2, 1 - index * 0.08) +
...
function workflowActiveTasks(store, ctx, limit = 4) {
    const scopedSessionTasks = ctx.sessionKey
        ? ctx.scopes
            .map((scope) => store.taskRepo.getActive({
            agentId: ctx.agentId,
            scope,
...
function chunkRows(hits) {
    return hits.map((hit, index) => {
        const chunkId = typeof hit.metadata.chunkId === "string"
            ? hit.metadata.chunkId
            : hit.docId.replace(/^event:chunk:/, "");
        const role = typeof hit.metadata.role === "string" ? hit.metadata.role : "memory";
        const assistantWeight = role === "assistant" && typeof hit.metadata.assistantWeight === "number"
            ? hit.metadata.assistantWeight
            : undefined;
        const text = normalizeSearchText(hit.text);
        const newline = text.indexOf("\n");
        const displayText = newline >= 0 && newline < text.length - 1 ? text.slice(newline + 1).trim() || text : text;
        return toEvidenceRow({
            id: chunkId,
            text: `[${role}] ${displayText}`,
            score: Math.max(0.12, (assistantWeight != null ? hit.score * Math.max(0.35, assistantWeight) : hit.score) -
                index * 0.04),
...
export function collectBehavioralGuidance(store, ctx) {
    ...
    const facts = store.factRepo
        ...
        const priorityDiff = backgroundBeliefPriority(rightBelief) - backgroundBeliefPriority(leftBelief);
...
function collectStrategyGuidance(store, ctx) {
    return store.strategyRepo
        ...
        const leftExplicitOnly = left.metadataJson.explicitInstruction === true &&
            left.metadataJson.groundedResolution !== true
            ? 1
            : 0;
        const rightExplicitOnly = right.metadataJson.explicitInstruction === true &&
            right.metadataJson.groundedResolution !== true
            ? 1
            : 0;
        const leftPriority = leftGrounded * 0.18 +
            left.usefulnessScore * 0.34 +
            left.stabilityScore * 0.24 +
            left.confidence * 0.18 -
            leftExplicitOnly * 0.08;
        const rightPriority = rightGrounded * 0.18 +
    ...
    return [...dedupedStates.values()].flatMap((state, index) => {
        ...
        return [
            toEvidenceRow({
                ...
                score: Math.max(0.18, currentness.currentnessScore * 0.76 + state.confidence * 0.16 - index * 0.04),
                scope: state.scope,
...
export function buildBackgroundRecallBundle(store, ctx) {
    const beliefIndex = buildBeliefIndex(store, ctx.agentId, ctx.readEpoch);
    const behavioralGuidance = collectBehavioralGuidance(store, ctx).slice(0, BEHAVIORAL_GUIDANCE_LIMIT);
    ...
    const tasks = dedupeEvidenceRows(filterBootstrapRows(taskRows(activeTasks))
        ...
        .sort((left, right) => backgroundBeliefPriority(beliefIndex.byContentRef.get(right.id)) -
        backgroundBeliefPriority(beliefIndex.byContentRef.get(left.id))), 3);
    ...
    const states = dedupeEvidenceRows([...canonicalStates, ...filterBootstrapRows(derivedTaskMetadataStates)]
        .filter((entry) => includeMinimalBackgroundState(entry))
        .filter((entry) => !looksLikePromptInjection(entry.text) && !containsLikelySecret(entry.text))
        .filter((entry) => includeBackgroundBelief(beliefIndex.byContentRef.get(entry.id)))
        .sort((left, right) => backgroundBeliefPriority(beliefIndex.byContentRef.get(right.id)) -
        backgroundBeliefPriority(beliefIndex.byContentRef.get(left.id))), BACKGROUND_STATE_ROWS_LIMIT);
...
function semanticProfileForObject(params) {
    ...
    const entityCount = typeof params.attrs.entityCount === "number"
    ...
    const relationSignal = clamp01((params.attrs.relational ? 0.78 : 0) +
        (params.kind === "graph_path" ? 0.16 : 0) +
        edgeFactor * 0.34 +
        nodeFactor * 0.08);
    const relationDensity = clamp01((params.kind === "graph_path" ? 0.42 : 0.08) +
        relationSignal * 0.34 +
        entityFactor * 0.16 +
        edgeFactor * 0.18);
    const connectivity = clamp01((params.kind === "graph_path" ? 0.38 : 0.06) +
        edgeFactor * 0.28 +
        nodeFactor * 0.22 +
        relationSignal * 0.12);
    // Session affinity: boost scores for objects from the current session
    const sessionMatch = params.currentSessionKey &&
        params.attrs.sessionKey &&
        params.currentSessionKey === params.attrs.sessionKey;
    const continuityScore = clamp01((params.attrs.activeTask ? 0.34 : 0) +
    ...
    const explanationScore = clamp01(similarity * 0.2 +
...
function routeAlignedBeliefStability(routeType, belief) {
    switch (routeType) {
        ...
        case "explanatory":
            return clamp01(belief.stabilityScore * 0.64 +
...
function applyBeliefAdjustment(object, objective, score, reasons) {
    ...
    const supportScore = clamp01(belief.posteriorConfidence * 0.5 +
        belief.usefulnessScore * 0.22 +
        routeAlignedBeliefStability(objective.routeType, belief) * 0.18 +
        belief.outcomeSupportScore * 0.1);
    const contradictionPenalty = belief.contradictionScore * (objective.routeType === "explanatory" ? 0.12 : 0.24);
    let adjusted = score;
    switch (belief.stage) {
        case "active":
...
function backgroundBeliefPriority(belief) {
    ...
    return clamp01(belief.posteriorConfidence * 0.48 +
...
function taskObjects(store, ctx, objective, hybridHits) {
    ...
    const anchoredRecentTasks = store.taskRepo
        ...
        if (references.some((reference) => normalizeText(reference) && normalizedObjectiveQuery.includes(normalizeText(reference)))) {
            return true;
        }
    ...
    const rows = selectedTasks.length > 0
        ? taskRows(selectedTasks)
        : rowsFromSearchHits(hybridHits.filter((hit) => hit.metadata.memxDocType === "task").slice(0, 4));
...
function chunkObjects(objective, hybridHits) {
    return filterBootstrapRows(chunkRows(hybridHits
        .filter((hit) => hit.metadata.memxDocType === "chunk" &&
        // Pure chatter (≤5 chars after stripping role prefix) has no recall value.
        normalizeSearchText(hit.text).length > 5)
        .slice(0, 8))).map((row) => buildMemoryObject({
        kind: "chunk",
        row,
        objective,
        attrs: {
...
function isShadowedGraphEntity(candidate, matches) {
    ...
    return matches.some((other) => {
        ...
        return (other.normalizedName.includes(candidate.normalizedName) &&
...
function graphAnchorTextsForObject(object) {
    switch (object.kind) {
        case "fact":
            return [
                object.row.text,
        ...
        case "state": {
            ...
            if (stateKey === "workflow.current_task" ||
...
function collectGraphHypothesisSeedNodeIds(params) {
    ...
    const names = new Set(seedEntityNamesFromQuery(params.objective.query).map((name) => normalizeText(name)));
...
function collectGraphSeedEntityIds(params) {
    ...
    const hasStrongContextSeeds = [...seeds.values()].some((entry) => entry.fromActiveContext && entry.score >= 0.78);
...
function expandGraphSupportObjects(params) {
    ...
    const hasRelationalFactSupport = [...expanded.values()].some((object) => object.kind === "fact" && object.attributes.relational);
    if (!hasRelationalFactSupport && params.objective.routeType === "explanatory") {
        const subjectNames = new Set(seedEntityNamesFromQuery(params.objective.query));
        for (const object of params.supportObjects) {
            if (object.attributes.factSubject) {
...
function augmentGraphWithSupportObjects(params) {
    ...
    const linkedEntityIdsByObject = new Map();
    ...
    for (const candidate of params.graphHypotheses) {
        ...
        const hasStoredEquivalent = [...edges.values()].some((edge) => edge.sourceKind === "stored" &&
        ...
        const confidence = clamp01(candidate.confidence * (metadata.relationClass === "observed" ? 0.96 : 0.88) +
    ...
    for (const object of params.supportObjects) {
        ...
        const entityNames = new Set(graphAnchorTextsForObject(object)
        ...
        const linkedIds = linkedEntityIdsByObject.get(object.objectId) ?? new Set();
    ...
    for (let index = 0; index < factObjects.length; index += 1) {
        ...
        for (let offset = index + 1; offset < factObjects.length; offset += 1) {
            ...
            if (left.attributes.factSubject !== right.attributes.factSubject ||
    ...
    const edgeList = [...edges.values()].sort((left, right) => right.confidence - left.confidence ||
...
function graphObjects(store, ctx, objective, supportObjects) {
    ...
    const hasRelationalSupport = graphSupportObjects.some((object) => (object.kind === "event" || object.kind === "fact") && object.attributes.relational === true);
    const includeCandidateHypotheses = hasRelationalSupport ||
...
export function collectMemoryObjects(store, ctx, objective, hybridHits) {
    ...
    return attachBeliefsToObjects(store, ctx, dedupeObjects([...states, ...tasks, ...facts, ...events, ...graph, ...chunks, ...alternates]));
}
function objectiveScoreForObject(object, objective) {
    ...
    if (objective.routeType === "workflow") {
        score = clamp01(profile.workflowScore * 0.44 +
        ...
        score = clamp01(profile.factualScore * 0.48 +
        ...
        score = clamp01(profile.temporalScore * 0.46 +
        ...
        score = clamp01(profile.explanationScore * 0.28 +
...
function graphRouteFitForPath(pathObject, routeType) {
    ...
    const structuralRatio = edges.filter((edge) => GRAPH_STRUCTURAL_RELATION_TYPES.has(edge.relType)).length /
    ...
    switch (routeType) {
        case "workflow":
            return clamp01(heterogeneous * 0.22 +
                relationFit * 0.12 +
        ...
        case "factual":
            return clamp01(heterogeneous * 0.14 +
                relationFit * 0.18 +
                structuralRatio * 0.14 +
        ...
        case "explanatory":
            return clamp01(heterogeneous * 0.28 +
                relationFit * 0.22 +
...
function buildGraphGuidance(candidates, objective) {
    const directSupportByObjectId = new Map();
    const paths = [];
    ...
    for (const entry of candidates) {
        ...
        const pathWeight = clamp01(entry.objectiveScore * 0.58 + routeFit * 0.26 + candidate.score * 0.16);
        if (pathWeight < 0.16) {
            continue;
        }
        strongestSupport = Math.max(strongestSupport, pathWeight);
        const directObjectIds = new Set((entry.object.graphNodes ?? [])
            .map((node) => node.sourceObjectId)
            .filter((nodeId) => typeof nodeId === "string" && Boolean(nodeId)));
        for (const objectId of directObjectIds) {
            directSupportByObjectId.set(objectId, Math.max(directSupportByObjectId.get(objectId) ?? 0, pathWeight));
        }
        paths.push({
...
function graphContextSupportForObject(object, guidance) {
    ...
    const objectEntities = new Set(inferEntityNames(object.row.text)
    ...
    for (const path of guidance.paths) {
        ...
        const nodeNameOverlap = matchedNodeCount === 0 ? 0 : clamp01(matchedNodeCount / Math.max(1, path.nodeNames.length));
...
function applyGraphGuidanceToCandidate(entry, objective, guidance) {
    ...
    const penalty = guidance.strongestSupport >= 0.72 &&
        totalSupport < 0.12 &&
...
export function scheduleMemoryObjects(objects, objective) {
...
export function collectAndScheduleMemoryObjects(store, ctx, objective, hybridHits) {
...
function activeObjectiveBudgets(plan) {
    return Object.entries(plan.objectiveBudgets)
...
function beliefBudgetPriority(entry) {
    ...
    const stageBonus = belief.stage === "active"
        ? 0.12
            ...
            : belief.stage === "candidate"
                ...
                : belief.stage === "decaying"
                    ...
                    : belief.stage === "superseded"
                        ? -0.16
                        : -0.3;
    return clamp01(entry.objectiveScore +
        stageBonus +
...
function buildBudgetAggregate(plan, scheduledByRoute) {
    ...
    for (const [routeType, scheduled] of Object.entries(scheduledByRoute)) {
...
function strongestRouteForAggregate(aggregate) {
    const ordered = Object.entries(aggregate.routeScores).sort((left, right) => (right[1] ?? 0) - (left[1] ?? 0));
    return ordered[0]?.[0];
}
function toSelectionTraceEntry(aggregate, selectionReason) {
    return {
        ...
        selectionReason: aggregate.object.kind === "graph_path" && aggregate.object.row.provenance
...
export function collectAndScheduleMemoryObjectsWithBudget(store, ctx, plan, hybridHits) {
    ...
    for (const [routeType, budget] of objectiveEntries) {
        const objective = selectionObjectiveForRoute(routeType, plan.focusedQueries[routeType], ctx.now, ctx.sessionKey);
    ...
    if (overflowBudget > 0) {
        overflowCandidates = [...aggregates.values()]
            ...
            .filter((aggregate) => plan.routeDecision.routeType === "factual"
...
export function topScheduledMemoryScore(scheduled, predicate) {
    let best = 0;
    for (const entry of scheduled) {
...
function probeSupportForEntry(entry, routeType) {
    ...
    switch (routeType) {
        case "workflow": {
            const kindBias = entry.object.kind === "task" ? 0.1 : entry.object.kind === "state" ? 0.08 : 0.02;
            return clamp01(entry.objectiveScore * 0.58 +
                graphSupport * 0.12 -
                graphPenalty * 0.08 +
        ...
        case "factual": {
            ...
            return clamp01(entry.objectiveScore * 0.62 +
                graphSupport * 0.12 -
                graphPenalty * 0.08 +
        ...
        case "temporal": {
            const kindBias = entry.object.kind === "event" ? 0.1 : entry.object.kind === "chunk" ? 0.08 : 0.02;
            return clamp01(entry.objectiveScore * 0.62 +
                graphSupport * 0.16 -
                graphPenalty * 0.08 +
        ...
        case "explanatory": {
            const relationBridge = entry.object.kind === "graph_path"
                ? 0.12
                : entry.object.kind === "fact" && entry.object.attributes.relational
                    ? 0.1
                    : 0.02;
            return clamp01(entry.objectiveScore * 0.44 +
                graphSupport * 0.14 -
                graphPenalty * 0.08 +
...
export function topProbeSupportForRoute(scheduled, routeType) {
    let topEntry;
    let support = 0;
    for (const entry of scheduled) {
...
export function routeHintFromMemoryObjectKind(kind) {
    switch (kind) {
        case "state":
        case "task":
            return "workflow";
...
export function toRouteEvidenceCandidatesFromObjects(scheduled, limit) {
