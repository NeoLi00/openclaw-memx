...
export function classifyStateLifecycle(input) {
...
export function evaluateStateCurrentness(input) {
    ...
    const resolvedTransient = isResolvedTransient(input.valueJson) &&
    ...
    const currentnessScore = clamp01(lifecycleBase * 0.36 +
        freshness * 0.28 +
        sourceTrace * 0.2 +
        durableFit * 0.16 -
        hardPenalty -
    ...
    return {
        ...
        hardExclusions,
        softPenalties,
        answerEligibleByDefault: hardExclusions.length === 0 &&
            rawSupportRefs.length > 0 &&
            lifecycleKind !== "task_checkpoint" &&
            lifecycleKind !== "derived_maintenance",
    };
}
export function stateCurrentnessVectorMetadata(state, now = state.updatedAt) {
...
export function stateCurrentnessFromVectorMetadata(metadata, now) {
...
export function stateCurrentnessToMetadata(currentness) {
    const currentnessHint = currentness.hardExclusions.length > 0
        ? "historical"
...
export function stateSupportRefs(input) {
    const refs = new Set();
...
function stateSupersededBy(valueJson) {
    ...
    if (!value)
        return undefined;
    return (stringValue(value.supersededBy) ??
...
function freshnessScore(observedAt, nowIso, lifecycleKind) {
    ...
    const halfLifeHours = lifecycleKind === "durable_profile"
        ? 24 * 180
        : lifecycleKind === "derived_maintenance"
            ? 24 * 21
            : lifecycleKind === "task_checkpoint"
