...
export function splitSourceText(text, params) {
    const targetChars = Math.max(400, params?.targetChars ?? SOURCE_SEGMENT_TARGET_CHARS);
    const overlapChars = Math.min(Math.max(0, params?.overlapChars ?? SOURCE_SEGMENT_OVERLAP_CHARS), targetChars - 1);
    if (!text.trim()) {
        return [];
    }
...
export function sourceGroupIdForChunk(chunk) {
    return `source_group:${stableHash([chunk.agentId, chunk.scope, chunk.sourceRef])}`;
}
export function buildSourceSegmentsForChunk(chunk) {
