...
function projectActiveBlockerBlock(states) {
    const blockerRows = states.filter((row) => row.id.endsWith("workflow.blocker"));
    return projectionBlock("active_blocker", "Current Working Context", blockerRows
        .slice(0, 2)
        .map((row) => `Active blocker: ${truncateText(stateValueFromRow(row), 180)}`), blockerRows.slice(0, 2).map((row) => row.id));
}
function projectOpenRiskBlock(states) {
    const riskRows = states.filter((row) => {
        const lowered = `${row.id} ${row.text}`.toLowerCase();
        return (lowered.includes("workflow.blocker") &&
    ...
    return projectionBlock("open_risk", "Current Working Context", riskRows.slice(0, 2).map((row) => `Open risk: ${truncateText(stateValueFromRow(row), 180)}`), riskRows.slice(0, 2).map((row) => row.id));
}
...
function temporalEventPriority(row) {
    ...
    if (/(?:失败|成功|修复|解决|超时|error|failed|success|fixed|resolved|timeout|deployed|rollback)/u.test(text)) {
        score += 2;
    }
...
export function projectScheduledMemoryObjects(scheduled, options) {
    const states = [];
    const tasks = [];
    const facts = [];
    const events = [];
    ...
    const graphPathCandidates = new Map();
    const graphPaths = [];
    const recalledChunkIds = [];
    const recalledChunkTexts = [];
    const seenRows = new Set();
    for (const entry of scheduled) {
        if (!shouldProjectBeliefBoundEntry(entry, options.routeType, options.routeConfidence, options.allowHistoricalFacts)) {
            continue;
        }
        ...
        if ((entry.object.kind === "event" || entry.object.kind === "chunk") &&
        ...
        if (entry.object.kind === "graph_path" && graphPaths.length < options.graphLimit) {
            ...
            if (entry.object.graphPathCandidate) {
                graphPathCandidates.set(entry.object.graphPathCandidate.pathId, entry.object.graphPathCandidate);
            }
...
export function createMemorySelectionObjective(routeType, query, now, currentSessionKey) {
    return {
        routeType,
        query,
        now,
