...
function sourceRefsForEntry(entry) {
    const metadataSourceRef = typeof entry.metadata?.sourceRef === "string" ? entry.metadata.sourceRef : undefined;
    const metadataSupportRefs = Array.isArray(entry.metadata?.supportRefs)
        ? entry.metadata.supportRefs.filter((value) => typeof value === "string" && value.trim().length > 0)
        : [];
    const metadataLineage = entry.metadata?.lineage && typeof entry.metadata.lineage === "object"
        ? entry.metadata.lineage
        : undefined;
    const metadataLineageSourceRef = typeof metadataLineage?.sourceRef === "string" ? metadataLineage.sourceRef : undefined;
    return [
        ...new Set([
            entry.sourceRef,
...
function evidenceUnitFromEntry(queryAnalysis, entry) {
    ...
    return {
        ...
        neighborRefs: entry.metadata?.sourceExpansion === true && Array.isArray(entry.metadata.neighborOf)
            ? entry.metadata.neighborOf.filter((value) => typeof value === "string" && value.trim().length > 0)
            : [],
        normalizedNeighborRefs: normalizeSourceRefs(entry.metadata?.sourceExpansion === true && Array.isArray(entry.metadata.neighborOf)
...
function unitHasAnswerRole(unit) {
    return unit.roles.some((role) => role === "answer_value" ||
        role === "answer_event" ||
        role === "user_resource" ||
        role === "prior_advice");
}
function unitLooksLikeAnswer(queryAnalysis, unit) {
    ...
    return (unit.origin === "canonical_fact" ||
        unit.origin === "event" ||
...
function compactResourceDisplay(text) {
    const resource = /(?:^|\|\s*)resource\s*=\s*([^|]+)/iu.exec(text)?.[1]?.trim() ??
    ...
    const support = /supportText\s*=\s*([^|]+)/iu.exec(text)?.[1]?.trim() ??
...
function displayLabelForAnswerUnit(unit) {
    ...
    if (unit.roles.includes("query_context") &&
...
function classifyPacketUnits(params) {
    ...
    const selectedCanAnswerDespiteQuestion = selectedRole === "user_resource" || selectedRole === "prior_advice";
    const selectedIsQuestion = !selectedCanAnswerDespiteQuestion && entryIsQueryLikeEvidence(params.queryAnalysis, params.selectedEntry);
    const contextAnswerUnits = contextUnits.filter((unit) => unitLooksLikeAnswer(params.queryAnalysis, unit));
    const selectedAssistantAnswer = selectedUnit.authorRole === "assistant" &&
        !selectedIsQuestion &&
        unitLooksLikeAnswer(params.queryAnalysis, selectedUnit);
    if (selectedIsQuestion ||
        ((selectedRole === "query_context" || selectedRole === "time_constraint") &&
            !selectedAssistantAnswer)) {
        const answerUnits = contextAnswerUnits.filter((unit) => (unit.authorRole === "assistant" || unitHasAnswerRole(unit)) &&
...
function packetDisplayLines(params) {
    ...
    const units = params.answerUnits.length > 0
        ? params.answerUnits
    ...
    for (const unit of units) {
        ...
        const line = contextText && semanticTextSimilarity(answerText, contextText) < 0.78
...
function slotCoverageScore(entry) {
    const explicitCoverage = entry.coverage &&
        ...
        : 0;
    return Math.max(explicitCoverage, ...(entry.slotCoverage ?? []).map((coverage) => coverage.coverageScore * (coverage.missingRequired.length > 0 ? 0.45 : 1)));
}
...
function bridgeNegativeSignalScore(entry) {
    return Math.max(0, ...(entry.bridgeMatches ?? [])
        .filter((match) => match.role !== "answer_value" &&
...
function evidenceShapeFitScore(queryAnalysis, entry, shapes) {
    ...
    const bridgeMatchRequired = shapes.some((shape) => shape === "validation_evidence" ||
        shape === "status_answer" ||
    ...
    const planMatch = Math.max(0, ...bridges
...
function missingRequiredAfterContext(entry, contextCandidates) {
    ...
    const isCovered = (value) => {
        ...
        return entries.some((candidate) => {
            ...
            const tokenCoverage = valueTokens.length > 0
...
function inferredSlotRole(queryAnalysis, entry) {
    ...
    const coverageRole = (entry.slotCoverage ?? [])
        ...
        const inferredAnswerRole = !explicitRole && slot && answerSlotIds.has(slot.id)
    ...
    if (bridgeRole &&
        ...
        const coverageIsContext = coverageRole === "query_context" || coverageRole === "time_constraint";
...
function hasStructuredResourceSignal(entry) {
    return Boolean(entry.slotEvidenceRole === "user_resource" ||
...
function hasResourceAffordanceSignal(entry) {
    return Boolean(bridgeShapeScore(entry, ["resource_affordance"]) > 0 ||
        metadataStringArray(entry.metadata, "domains").length > 0 ||
        metadataStringArray(entry.metadata, "affordances").length > 0 ||
        (typeof entry.metadata?.resourceType === "string" &&
            entry.metadata.resourceType.trim().length > 0) ||
...
function isAssistantAcknowledgement(entry) {
    ...
    return (entry.metadata?.semanticRole === "assistant_acknowledgement" ||
...
function isStateEvidence(entry) {
    return (entry.metadata?.memxDocType === "state" ||
...
function stateHardExclusions(entry, now) {
    ...
    const lifecycleKind = typeof entry.metadata?.stateLifecycleKind === "string"
    ...
    const supportOnlyBlockers = lifecycleKind === "derived_maintenance" && rawSupportRefs.length === 0
...
function stateSoftPenalties(entry) {
    ...
    const score = typeof entry.metadata?.stateCurrentnessScore === "number"
...
function authorityScore(entry) {
    const stateLifecycleKind = typeof entry.metadata?.stateLifecycleKind === "string"
        ? entry.metadata.stateLifecycleKind
        : undefined;
    if (isStateEvidence(entry)) {
        const stateScore = stateLifecycleKind === "derived_maintenance"
            ? 0.42
    ...
    const author = entryAuthorRole(entry);
    const surfaceScore = entry.surface === "fact"
        ? 0.86
        : entry.surface === "event"
            ? 0.78
            : entry.surface === "chunk"
                ...
                : 0.58;
    const sourceTrace = entry.sourceRef || (entry.mergedSourceRefs?.length ?? 0) > 0 ? 0.08 : 0;
    const sourceScore = entry.source === "selected"
        ? 0.05
...
function sameSemanticChainSupport(entry, contextCandidates) {
    ...
    for (const candidate of contextCandidates) {
        ...
        const sharedFamily = candidateRefs.some((sourceRef) => families.has(sourceFamilyRef(sourceRef)));
        if (sharedFamily) {
...
function contextBindingScore(params) {
    ...
    const filledRoles = new Set([params.entry, ...params.contextCandidates]
    ...
    const hasContext = filledRoles.has("query_context") ||
        ...
        (params.entry.slotCoverage ?? []).some((coverage) => {
            const slot = (params.queryAnalysis.evidencePlan?.slots ?? []).find((candidate) => candidate.id === coverage.slotId);
    ...
    const samePacketSupport = params.contextCandidates.some((candidate) => (inferredSlotRole(params.queryAnalysis, candidate) === "query_context" ||
    ...
    const contextSupport = Math.max(contextSemanticSupport >= 0.24 ? sameChainSupport : 0, samePacketSupport && params.contextCandidates.length > 0 ? 0.42 : 0, contextSemanticSupport);
    ...
    if (operation === "tailor_advice" && params.slotRole === "prior_advice") {
        const topicSimilarity = Math.max(semanticTextSimilarity(params.queryAnalysis.queryText, params.entry.text), semanticTextSimilarity(params.queryAnalysis.focusedQuery ?? "", params.entry.text));
    ...
    if (lookupMode) {
        const sameEntryContextAndAnswer = (entryRoles.has("answer_value") || entryRoles.has("answer_event")) &&
            entryRoles.has("query_context");
...
function hasStrongAnswerPlanningMatch(queryAnalysis, entry) {
    return (slotCoverageScore(entry) >= 0.62 ||
...
function queryContextSupportScore(queryAnalysis, entry) {
    ...
    const contextCoverage = Math.max(0, ...(entry.slotCoverage ?? [])
        .filter((coverage) => {
        const slot = (queryAnalysis.evidencePlan?.slots ?? []).find((candidate) => candidate.id === coverage.slotId);
    ...
    return clamp01(planSemantic * 0.5 + contextCoverage * 0.28 + bridgeContext * 0.14 + bridgePositive * 0.08);
}
...
function answerDomainScore(queryAnalysis, entry) {
    ...
    const answerCoverage = Math.max(0, ...(entry.slotCoverage ?? [])
        .filter((coverage) => {
        const slot = (queryAnalysis.evidencePlan?.slots ?? []).find((candidate) => candidate.id === coverage.slotId);
        const role = evidenceSlotRequiredRole(slot);
        return (role === "answer_value" ||
            role === "answer_event" ||
            (operationType(queryAnalysis) === "tailor_advice" &&
    ...
    if (operationType(queryAnalysis) === "tailor_advice") {
        ...
        const blended = answerCoverage * 0.22 +
            bridgeAnswer * 0.12 +
            bridgePositive * 0.18 +
            slotSemantic * 0.12 +
            needFit * 0.28 +
            goalSemantic * 0.08;
        const corroborationBoost = Math.min(0.14, [
            answerCoverage >= 0.28,
            bridgePositive >= 0.28,
            slotSemantic >= 0.28,
            needFit >= 0.28,
    ...
    const blended = answerCoverage * 0.36 +
        bridgeAnswer * 0.22 +
        bridgePositive * 0.14 +
        slotSemantic * 0.1 +
        goalSemantic * 0.18;
...
function hasTemporalPlanning(queryAnalysis) {
    return (queryAnalysis.queryShape.timeframe === "historical" ||
        queryAnalysis.answerMode === "count_aggregate" ||
        operationType(queryAnalysis) === "aggregate" ||
        (queryAnalysis.evidencePlan?.slots ?? []).some((slot) => evidenceSlotRequiredRole(slot) === "time_constraint" ||
...
function temporalFitScore(params) {
    ...
    if (params.entry.observedAt ||
        params.contextCandidates.some((entry) => Boolean(entry.observedAt))) {
        return 0.78;
    }
    const timeBridgeScore = Math.max(bridgeRoleScore(params.entry, ["time_constraint"]), bridgeShapeScore(params.entry, ["time_constraint"]), ...params.contextCandidates.map((entry) => Math.max(bridgeRoleScore(entry, ["time_constraint"]), bridgeShapeScore(entry, ["time_constraint"]))));
...
function softPenaltyReasons(params) {
    ...
    const filledRoles = new Set([params.entry, ...params.contextCandidates]
        .map((candidate) => inferredSlotRole(params.queryAnalysis, candidate))
        .filter(Boolean));
    const hasAnswerRole = params.slotRole === "answer_value" ||
    ...
    if (operation === "return_value" &&
        ...
        !(queryAsksSensitiveValue(params.queryAnalysis) &&
            statesUnavailableSensitiveValue(params.queryAnalysis, params.entry)) &&
        Math.max(queryContextSupportScore(params.queryAnalysis, params.entry), 0, ...params.contextCandidates.map((candidate) => queryContextSupportScore(params.queryAnalysis, candidate))) < 0.24) {
    ...
    if (params.queryAnalysis.queryShape.timeframe === "historical" &&
    ...
    if ((params.queryAnalysis.answerMode === "count_aggregate" || operation === "aggregate") &&
    ...
    if ((operation === "return_value" || params.queryAnalysis.answerMode === "attribute_lookup") &&
    ...
    if (querySeeksCausalAnswer(params.queryAnalysis) &&
        (params.slotRole === "answer_value" || filledRoles.has("answer_value")) &&
    ...
    if (queryAsksCurrentStatus(params.queryAnalysis) &&
        (params.slotRole === "answer_value" || filledRoles.has("answer_value")) &&
...
function answerScore(params) {
    ...
    const causalFit = querySeeksCausalAnswer(params.queryAnalysis)
        ? Math.max(directCausalExplanationScore(params.queryAnalysis, params.entry), causalExplanationScore(params.queryAnalysis, params.entry) * 0.55)
        : 0;
    if (params.slotRole === "query_context") {
    ...
    if (params.slotRole === "answer_value" || params.slotRole === "answer_event") {
        const base = clamp01(domain * (causalFit > 0 ? 0.34 : 0.42) +
            bridgeAnswer * 0.18 +
            semantic * 0.12 +
            coverage * 0.12 +
            causalFit * 0.18 +
    ...
    const evidenceCoverageScore = params.entry.coverage && params.entry.coverage.requiredHits.length > 0
        ? Math.max(0.5, params.entry.coverage.coverageScore)
        : 0;
    const surfaceAnswer = params.entry.surface === "fact" || params.entry.surface === "event"
        ? 0.46
        : params.entry.surface === "chunk"
            ? 0.34
            : 0.28;
    return clamp01(surfaceAnswer * 0.2 +
        semantic * 0.18 +
        domain * 0.26 +
        bridgeAnswer * 0.14 +
        causalFit * 0.12 +
...
function gradeCandidate(params) {
    ...
    const filledRoles = new Set([params.entry, ...params.contextCandidates]
        .map((candidate) => inferredSlotRole(params.queryAnalysis, candidate))
        .filter(Boolean));
    const hasAnswerRole = slotRole === "answer_value" ||
    ...
    const softPenaltyScore = clamp01(Math.min(0.48, softPenaltyEntries.reduce((sum, penalty) => sum + penalty.weight, 0)));
    const eligibilityRole = slotRole === "user_resource"
        ? "resource"
    ...
    const roleFit = slotRole === "answer_value" || slotRole === "answer_event"
        ? 1
        : operation === "tailor_advice" &&
            (slotRole === "user_resource" || slotRole === "prior_advice")
            ? 0.95
    ...
    const contextMultiplier = operation === "return_value" || params.queryAnalysis.answerMode === "attribute_lookup"
        ? 0.45 + 0.55 * contextBinding
        : 1;
    const baseScore = operation === "aggregate" || params.queryAnalysis.answerMode === "count_aggregate"
        ? answerDomain * 0.45 +
            retrievalScore * 0.2 +
            sourceTrace * 0.15 +
            temporalFit * 0.1 +
        ...
        : operation === "tailor_advice"
            ...
            : hasTemporalPlanning(params.queryAnalysis) &&
                ...
                : (answer * 0.36 +
                    ...
                    roleFit * 0.08) *
                    contextMultiplier;
    const finalScore = clamp01(baseScore - softPenaltyScore - bridgeNegative * 0.06 - Math.min(0.5, blockers.length * 0.28));
    return {
        blockers,
...
function contextCandidatesForGroup(params) {
    ...
    const userQuestionBridge = entryAuthorRole(params.answerEntry) === "user" &&
        (entryIsQuestionLike(params.answerEntry) ||
            entryIsQueryLikeEvidence(params.queryAnalysis, params.answerEntry));
    const assistantAnswerBridge = entryAuthorRole(params.answerEntry) === "assistant" &&
        operation !== "aggregate" &&
    ...
    const bindAdjacentNonContext = operation !== "aggregate" &&
        params.queryAnalysis.answerMode !== "count_aggregate" &&
        ...
        if (sourceFamilies.size > 0 && entryFamilies.length > 0) {
            if (entryFamilies.some((family) => sourceFamilies.has(family))) {
                if (role === "query_context") {
                    return (sourceRefsAdjacent(bindingAnswerRefs, entryRefs, 2) &&
                        queryContextSupportScore(params.queryAnalysis, entry) >= 0.18);
                }
                return (bindAdjacentNonContext &&
                    (role !== answerRole ||
    ...
    return [...new Map([...local, ...related].map((entry) => [entryKey(entry), entry])).values()]
        ...
        const leftAdjacent = sourceRefsAdjacent(bindingAnswerRefs, bindingSourceRefsForEntry(left), 2)
            ? 1
            : 0;
        const rightAdjacent = sourceRefsAdjacent(bindingAnswerRefs, bindingSourceRefsForEntry(right), 2)
            ? 1
            : 0;
...
function packetFromGroup(params) {
    ...
    const hasAnswerDisplayLine = displayLines.some((line) => line.startsWith("[answer]") ||
    ...
    const packetDirectCauseScore = querySeeksCausalAnswer(params.queryAnalysis)
        ? Math.max(evidenceShapeFitScore(params.queryAnalysis, selected.entry, ["causal_explanation"]), evidenceShapeFitScoreForTexts(params.queryAnalysis, [
            ...displayLines,
            ...unitGroups.answerUnits.map((unit) => `${unit.displayText} ${unit.rawText}`),
        ], ["causal_explanation"]))
        : 0;
    const packetSuppliesDirectCause = packetDirectCauseScore >= 0.72 &&
        directCausalExplanationScore(params.queryAnalysis, selected.entry) < 0.5;
    const baseSoftPenalties = packetSuppliesDirectCause
        ? selected.graded.softPenalties.filter((reason) => reason !== "weak-causal-explanation" && reason !== "query-like-answer-candidate")
    ...
    const packetCausalGrade = packetSuppliesDirectCause
        ? {
            ...
            finalScore: Math.max(selected.graded.grade.finalScore, clamp01(0.46 +
    ...
    const slotIds = [
        ...new Set([selected.entry, ...selected.contextCandidates].flatMap((entry) => matchedSlotIds(entry))),
    ];
    ...
    const missing = [
        ...new Set([
            ...
            ...slotIds
                ...
                .filter(() => !selected.entry.observedAt &&
                !selected.contextCandidates.some((candidate) => candidate.observedAt))
                .map(() => "observedAt"),
        ].filter(Boolean)),
    ];
    const role = adjustedEligibility.role === "answer" ||
    ...
    return {
        ...
        supportingTexts: displayLines.length > 1
        ...
        observedAt: selected.entry.observedAt ??
            selected.contextCandidates.find((entry) => entry.observedAt)?.observedAt,
        resolvedDate: selected.entry.observedAt?.slice(0, 10) ??
        ...
        grade: adjustedGrade,
        selectionReason: selected.graded.blockers.length > 0
            ? `excluded:${selected.graded.blockers.join(",")}`
            : `ranked:${adjustedEligibility.role}:${adjustedGrade.finalScore.toFixed(3)}${adjustedSoftPenalties.length > 0
        ...
        dropReason: selected.graded.blockers.length > 0 ? selected.graded.blockers.join(",") : undefined,
    };
}
function highLevelSupportPackets(params) {
    ...
    for (const candidate of params.candidateGenerationResult?.layerCandidates ?? []) {
        const sourceRefs = [
            ...new Set([candidate.sourceRef, ...(candidate.sourceRefs ?? [])].filter((value) => Boolean(value))),
        ];
        packets.push({
            ...
            blockedBy: sourceRefs.length === 0
                ? ["high-level-memory-missing-source-ref"]
                : ["source-evidence-not-selected"],
            dropReason: sourceRefs.length === 0
                ? "high-level-memory-missing-source-ref"
                : "source-evidence-not-selected",
            hardExclusions: sourceRefs.length === 0
                ? ["high-level-memory-missing-source-ref"]
                : ["source-evidence-not-selected"],
        });
    }
...
function packetHasAnswerDisplayForQuery(queryAnalysis, packet) {
    ...
    return packet.displayLines.some((line) => line.startsWith("[answer]") ||
        line.startsWith("[resource]") ||
...
function selectInjectedPackets(queryAnalysis, packets) {
    ...
    const selectedDistinct = new Set();
    const selectedAnswerDisplays = new Set();
    ...
    const nonContrastAnswerAvailable = ranked.some((packet) => !packetHasSoftPenalty(packet, "negative-contrast-without-query-context") &&
    ...
    const rankedForSelection = nonContrastAnswerAvailable
        ? ranked.filter((packet) => !packetHasSoftPenalty(packet, "negative-contrast-without-query-context"))
        : ranked;
    ...
    const scoredRankedForSelection = positiveScoreAvailable
        ...
        : rankedForSelection;
    const aggregateMode = queryAnalysis.answerMode === "count_aggregate" || operationType(queryAnalysis) === "aggregate";
    const answerDisplayAvailable = scoredRankedForSelection.some((packet) => packetHasAnswerDisplayForQuery(queryAnalysis, packet));
    const answerDisplayRankedForSelection = answerDisplayAvailable
        ? scoredRankedForSelection.filter((packet) => packetHasAnswerDisplayForQuery(queryAnalysis, packet))
    ...
    const slotCoverageMode = operationType(queryAnalysis) === "derive" || operationType(queryAnalysis) === "compare";
    if (slotCoverageMode) {
...
function markPromptEvidenceFromPackets(params) {
    ...
    const mapped = params.promptEvidence.map((entry) => {
        ...
        return {
            ...
            text: injectedEntry && packet.displayLines && packet.displayLines.length > 0
                ? packet.displayLines.join("\n")
                : entry.text,
            mergedSourceRefs: injectedEntry && (packet.allSourceRefs?.length ?? 0) > 0
...
function layerCountsForSlot(slot, candidateGenerationResult) {
    ...
    for (const layer of slotLayers(slot)) {
        const count = candidateGenerationResult?.slotLayerStats
...
function auditForSlot(params) {
    ...
    const injectedForSlot = slotPackets.filter((packet) => params.injectedPacketIds.has(packet.packetId));
    const missing = injectedForSlot.length >= Math.max(1, params.slot.minEvidence)
        ? []
...
function evidencePacketAudit(params) {
    ...
    return {
        ...
        sourceExpansion: units
            .filter((unit) => (unit.supportRefs?.length ?? 0) > 0 ||
...
export function assembleEvidencePackets(input) {
