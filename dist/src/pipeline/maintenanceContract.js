...
export function uniqueMaintenanceRefs(values) {
    return [
        ...new Set(values
...
export function sourceRefsFromMaintenanceMetadata(metadata) {
    if (!metadata) {
        return [];
    }
...
export function summarizeMaintenanceContractDiagnostics(metadataEntries) {
    const recallLayers = new Map();
    ...
    for (const metadata of metadataEntries) {
        ...
        const sourceRef = (typeof metadata.sourceRef === "string" && metadata.sourceRef.trim()) ||
            (typeof contract?.sourceRef === "string" && contract.sourceRef.trim()) ||
            undefined;
        const recallLayer = (typeof metadata.recallLayer === "string" && metadata.recallLayer.trim()) ||
            (typeof contract?.recallLayer === "string" && contract.recallLayer.trim()) ||
            undefined;
        const recallVisible = metadata.recallVisible === true ||
...
export function buildMaintenanceContractMetadata(params) {
    ...
    const materializedEpoch = typeof params.materializedEpoch === "number" && Number.isFinite(params.materializedEpoch)
        ...
        : typeof existing.materializedEpoch === "number" &&
            ...
            : undefined;
    const derivationPolicyVersion = params.derivationPolicyVersion ??
