...
import { clamp01, isValidEntityName, normalizeName, normalizeText, normalizedTerms, objectRecord, stableHash, truncateText, } from "../support.js";
import { shouldDeriveRelationFact, shouldMaterializePreferenceFact, sanitizeWorkflowHint, } from "./authority.js";
import { describeStateValue } from "./memoryObjectsHelpers.js";
import { isProjectProfileStateKey, projectAliasVariants, projectCodeFromStateKey, projectNamesMatch, resolveProjectReference, } from "./projectIdentity.js";
import { canonicalStateKey, inferEntityType, isQuestionLike, looksLikeBareInstructionalGuidance, looksLikeBareMemoryUseInstruction, } from "./semantic/heuristics.js";
import { canonicalizePreferenceHint, parsePreferenceSignal } from "./semantics.js";
...
function extractRelativeMonthRefs(text, observedAt) {
    ...
    const matches = [
        ...text.matchAll(/(前年|去年|今年|明年|this year|last year|next year)\s*([一二三四五六七八九十]{1,3}|\d{1,2})月/giu),
    ];
    const resolved = [];
    for (const match of matches) {
        ...
        if (marker === "前年")
            yearOffset = -2;
        else if (marker === "去年" || marker === "last year")
            yearOffset = -1;
        else if (marker === "明年" || marker === "next year")
            yearOffset = 1;
...
function extractHistoricalAliases(text) {
    const aliases = [
        ...
        ...text.matchAll(/(?:代号|别名|旧称|旧名|内部名|alias|codename)\s*(?:是|为|叫)?\s*([A-Za-z][A-Za-z0-9_.-]{1,40})/giu),
    ]
...
function mergeStringArray(existing, incoming) {
    const base = Array.isArray(existing)
        ? existing.filter((entry) => typeof entry === "string" && entry.trim().length > 0)
        : [];
...
function inferProjectComponentUpdate(update, base) {
    ...
    const target = typeof update.target === "string" && update.target.trim() ? update.target.trim() : undefined;
    const replacement = typeof update.replacement === "string" && update.replacement.trim()
        ? update.replacement.trim()
        : undefined;
    const slot = typeof update.componentRole === "string" && update.componentRole.trim()
    ...
    const resolvedSlot = slot ??
        Object.entries(baseComponents).find(([, value]) => typeof value === "string" && target && normalizeName(value) === normalizeName(target))?.[0];
    if (!resolvedSlot) {
        return normalized;
    }
...
function sanitizeProjectProfileValue(value, base) {
    const sanitized = {};
    const projectCode = typeof value.projectCode === "string" && value.projectCode.trim()
    ...
    const version = typeof value.version === "string" && value.version.trim()
        ...
        : typeof base?.version === "string" && base.version.trim()
            ? base.version.trim()
            : undefined;
    const launchDate = typeof value.launchDate === "string" && value.launchDate.trim()
    ...
    for (const [rawKey, rawValue] of Object.entries(value)) {
        ...
        if (key === "historicalAliases") {
            const aliases = Array.isArray(rawValue)
                ? rawValue.filter((entry) => typeof entry === "string" && entry.trim().length > 0)
...
function deriveProjectProfileNote(params) {
    ...
    const detailKeys = Object.keys(params.currentValue).filter((key) => !["projectCode", "status"].includes(key));
...
function resourceVectorTextFromRecord(record, fallback = {}) {
    ...
    const resource = typeof record.resource === "string" && record.resource.trim()
        ? record.resource
        : fallback.resource;
    const ownership = typeof record.ownershipStatus === "string" && record.ownershipStatus.trim()
...
function resourceText(assertion) {
    const status = assertion.ownershipStatus === "recently_acquired"
        ? "recently acquired"
...
function relationSummaryText(relation) {
    const basePredicate = relation.predicate === "related_to" && relation.rawPredicate?.trim()
...
function structuredEntityTerms(candidate) {
    return new Set((candidate.structuredHints?.entities ?? [])
        .flatMap((entity) => normalizedTerms(entity.name, { minLength: 3 }))
        .filter(Boolean));
}
function structuredTimeTerms(candidate) {
    return new Set((candidate.structuredHints?.timeHints ?? [])
...
function buildObservationShadowExcerpt(candidate) {
    const fragments = [...semanticDraftSupportTexts(candidate), candidate.rawText.trim()].filter(Boolean);
    const clauses = fragments.flatMap((fragment) => fragment
        .split(/[。.!！？?；;\n]/u)
        .map((entry) => entry.trim())
        .filter(Boolean));
    const ranked = [...new Map(clauses.map((clause) => [normalizeText(clause), clause])).values()]
        .map((clause) => ({
        clause,
        score: clauseSignalScore(candidate, clause),
    }))
        .filter((entry) => entry.score >= 3 && !isQuestionLike(entry.clause))
        .sort((left, right) => right.score - left.score || right.clause.length - left.clause.length);
    if (ranked.length === 0) {
        return undefined;
    }
    return truncateText(ranked
        .slice(0, 2)
...
function observationShadowTimeframe(candidate) {
    ...
    if (materializationHint?.timeframeHint === "current" ||
        materializationHint?.timeframeHint === "historical" ||
        materializationHint?.timeframeHint === "compare") {
        return materializationHint.timeframeHint;
    }
    const draft = getSemanticDraft(candidate);
    const draftTimeframe = draft?.assertionDrafts
        .map((entry) => entry.timeframeHint)
        .find((entry) => entry === "current" || entry === "historical" || entry === "compare");
    if (draftTimeframe) {
        return draftTimeframe;
    }
...
function shouldMaterializeObservationShadowFact(params) {
    ...
    const hasAnswerBearingFamily = hasSemanticDraftFamily(candidate, "event_like") ||
        hasSemanticDraftFamily(candidate, "fact_like") ||
        candidate.classification === "episodic-event";
    ...
    if (looksLikeBareMemoryUseInstruction(candidate.rawText)) {
        return false;
    }
    if (looksLikeBareInstructionalGuidance(candidate.rawText) &&
        (candidate.structuredHints?.semanticDraft?.assertionDrafts.length ?? 0) === 0) {
        return false;
    }
...
function buildObservationShadowFact(ctx, candidate) {
    ...
    const draft = getSemanticDraft(candidate);
    const fallbackSourceRef = typeof candidate.metadata?.sourceRef === "string" &&
        candidate.metadata.sourceRef.trim().length > 0
        ? candidate.metadata.sourceRef.trim()
        : undefined;
    ...
    return buildFact({
        ...
        objectValueJson: {
            ...
            supportRefs: [
                ...new Set([
                    ...(draft?.supportSpans ?? []).map((entry) => entry.sourceRef).filter(Boolean),
...
function buildEventStructuredSummary(candidate) {
    ...
    const workflows = hints?.workflows && hints.workflows.length > 0
        ? hints.workflows
        : hints?.workflow
            ? [hints.workflow]
            : [];
    const relations = hints?.relations && hints.relations.length > 0
        ? hints.relations
        : hints?.relation
            ? [hints.relation]
            : [];
    if (relations.length > 0) {
        return truncateText(relations
            .slice(0, 2)
            .map((relation) => `${relationSummaryLabel(relation)}: ${relationSummaryText(relation)}`)
            .join(" | "), 220);
    }
    if (workflows.length > 0) {
        return truncateText(workflows
            .slice(0, 2)
            .map((workflow) => {
    ...
    if (hints?.preference?.object?.trim()) {
        ...
        return truncateText(`Preference: ${preference?.predicate ?? hints.preference.predicate}: ${hints.preference.object.trim()}`, 220);
    }
...
function getWorkflowHints(candidate) {
    ...
    const raw = candidate.structuredHints?.workflows && candidate.structuredHints.workflows.length > 0
...
function getRelationHints(candidate) {
    ...
    if (candidate.structuredHints?.relations && candidate.structuredHints.relations.length > 0) {
        return candidate.structuredHints.relations.filter((relation) => relation.polarity !== "negated");
    }
    ...
    return {
        ...
        families: [
            ...new Set([
                ...
                ...((draft.relationDrafts?.length ?? 0) > 0 ? ["relation_like"] : []),
            ]),
        ],
        timeframes: [...new Set(draft.assertionDrafts.map((entry) => entry.timeframeHint))],
        slotHints: [
            ...new Set(draft.assertionDrafts.flatMap((entry) => Array.isArray(entry.slotHints) ? entry.slotHints.filter(Boolean) : [])),
        ],
...
function semanticDraftConsumedShape(candidate) {
...
function shouldMaterializeDecisionSummary(candidate, decision, preference, relations, correction) {
    if (candidate.classification !== "stable-fact" ||
        !decision ||
        preference ||
        relations.length > 0 ||
        correction ||
    ...
    if ((candidate.structuredHints?.timeHints?.length ?? 0) > 0 ||
        candidate.rawText.trim().length > 160) {
        return false;
    }
    return (DURABLE_DECISION_SUMMARY_PATTERN.test(candidate.rawText) ||
...
function shouldMaterializeObservedEvent(candidate, relations) {
    ...
    if (candidate.structuredHints?.correction &&
    ...
    if (relations.length > 0) {
        ...
        return relations.some((r) => !STRUCTURAL_TOPOLOGY_PREDICATES.has(r.predicate) ||
...
function buildVectorDoc(params) {
    const defaultLineage = params.kind === "edge"
        ? {
...
export function normalizeCandidate(candidate, ctx) {
    ...
    const knownProjectNames = [
        ...
        ...workflowHints.map((workflow) => typeof workflow.value.projectCode === "string"
    ...
    const seenRelationFacts = new Set();
    const projectProfiles = new Map();
    const projectProfileProtectedPaths = new Map();
    const projectProfileBase = (key, projectCode) => projectProfiles.get(key) ??
    ...
    if (correction?.targetKind === "project_profile") {
        const correctionProjectCode = typeof currentProjectProfile?.projectCode === "string" &&
            currentProjectProfile.projectCode.trim()
            ? currentProjectProfile.projectCode.trim()
        ...
        if (correctionProjectCode) {
            ...
            if (correctionUpdate) {
                ...
                projectProfileProtectedPaths.set(stateKey, new Set([
                    ...(projectProfileProtectedPaths.get(stateKey) ?? new Set()),
    ...
    const rewriteEntityReferences = (fromId, toId) => {
    ...
    const pushEntity = (name, type, aliases = []) => {
        const normalizedName = normalizeName(name);
        const wantsProject = type === "project" ||
            (canonicalCurrentProject ? normalizeName(canonicalCurrentProject) === normalizedName : false);
        const nextType = wantsProject
            ? "project"
            : (type ?? inferEntityType(name) ?? "unknown");
    ...
    const pushResourceAssertion = (assertion) => {
        ...
        if (!outputs.facts.some((entry) => entry.predicate === fact.predicate &&
    ...
    for (const relation of relations) {
        ...
        const inferredObjectType = relation.predicate === "related_to" &&
            relation.rawPredicate &&
            TEMPORAL_RELATION_RAW_PREDICATES.has(relation.rawPredicate)
            ? "person"
            : undefined;
        const srcEntity = pushEntity(relation.subject, relation.relationSlot ? "project" : relation.subject === subjectUser() ? "person" : undefined, subjectAliases);
    ...
    if (correction &&
        ...
        if (!outputs.facts.some((entry) => entry.canonicalSubject === correctionFact.canonicalSubject &&
    ...
    if (shouldMaterializeObservationShadowFact({
        ...
        if (observationShadowFact) {
            outputs.facts.push(observationShadowFact);
        }
    }
    for (const workflow of workflowHints) {
        const canonicalProjectCode = projectCodeFromStateKey(workflow.key) ||
        ...
        if (isProjectProfileStateKey(workflow.key)) {
            const projectCode = projectCodeFromStateKey(normalizedWorkflow.key) ??
        ...
        if (stateKind === "session") {
            state.expiresAt = new Date(new Date(candidate.observedAt).getTime() + ctx.config.stateTtlHours * 60 * 60 * 1000).toISOString();
        }
        const existingIndex = outputs.states.findIndex((entry) => entry.key === state.key);
        if (existingIndex >= 0) {
            outputs.states[existingIndex] = state;
    ...
    for (const event of outputs.events) {
        const structuredSummary = typeof event.metadataJson.memxStructuredSummary === "string" &&
            event.metadataJson.memxStructuredSummary.trim()
            ? event.metadataJson.memxStructuredSummary.trim()
            : event.text;
        const detailExcerpt = typeof event.metadataJson.memxRetrievalDetailExcerpt === "string" &&
...
export function computeConfidence(candidate) {
    ...
    if (candidate.source.kind === "tool")
        boolBase += 0.12;
    if (candidate.policy.explicitIntent)
        boolBase += 0.12;
    if (candidate.structuredHints?.relationHint)
        boolBase += 0.08;
    ...
    const hasPolicyScores = typeof p.salienceScore === "number" && typeof p.expectedFutureUtility === "number";
    if (!hasPolicyScores) {
        return clamp01(boolBase);
    }
    const policyScore = p.salienceScore * 0.4 +
