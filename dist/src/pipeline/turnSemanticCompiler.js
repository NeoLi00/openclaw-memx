...
function deterministicCompile(params) {
    ...
    for (const message of messages) {
        ...
        for (const family of families) {
            assertionDrafts.push({
                ...
                slotHints: structuredHints.workflow?.key ? [structuredHints.workflow.key] : undefined,
    ...
    const summaryChunks = messages.map((message, index) => ({
...
export function frameHintsForSourceRef(frame, sourceRef) {
    if (!frame) {
        return undefined;
    }
    ...
    const slotHints = [
        ...new Set(relevantDrafts.flatMap((entry) => Array.isArray(entry.slotHints) ? entry.slotHints.filter(Boolean) : [])),
    ];
    ...
    return {
        ...
        ...((semanticDraft.resourceAssertions?.length ?? 0) > 0
            ? { resourceAssertions: semanticDraft.resourceAssertions }
            : {}),
        ...((semanticDraft.adviceSignals?.length ?? 0) > 0
            ? { adviceSignals: semanticDraft.adviceSignals }
            : {}),
        semanticDraft,
        materializationHint: draftMaterializationHint(semanticDraft),
    };
}
export async function compileTurnSemantics(params) {
