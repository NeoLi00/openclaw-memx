...
function strongNonEmbeddingSupport(candidate) {
    return clamp01(candidate.exactNameScore * 0.2 +
...
function resolutionResult(params) {
    const resolvedEntityId = params.method === "uncertain" && params.createdEntity !== true
        ? undefined
...
export function buildEntityMention(params) {
...
export function resolveEntityMention(store, ctx, mention, options = {}) {
    ...
    const projectCandidate = mention.proposedType === "project" ||
    ...
    const inAmbiguousProfileBand = topProfile &&
    ...
    const nameSearchCandidates = store.graphRepo
    ...
    const candidates = [...profileCandidates, ...nameSearchCandidates]
        ...
        .filter((candidate, index, all) => all.findIndex((entry) => entry.entity.entityId === candidate.entity.entityId) === index);
...
export function resolveEntityMentions(store, ctx, mentions, options = {}) {
