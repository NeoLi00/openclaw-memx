...
export function prototypeSimilarity(text, prototypes, stopwords) {
...
export function tokenizeSearchTerms(text, stopwords) {
...
export function looksLikeBareMemoryUseInstruction(text) {
...
export function looksLikeBareInstructionalGuidance(text) {
    const stripped = stripMemoryArtifactPrefix(text);
    if (!stripped) {
        return false;
    }
...
export function isLowValueChatter(text) {
...
export function extractTimeHints(text) {
...
export function trimCapturedValue(value) {
    return value
        .trim()
        .replace(/^[“"'`]+/, "")
        .replace(/[”"'`]+$/, "")
 ...
 */
export function inferEntityType(name, predicateHint) {
    ...
    if (/\.(?:js|ts|py|rb|go|rs|java|kt|swift)$/i.test(lower))
        return "tool";
    if (/[-_](?:cli|sdk|api|lib|plugin|server|client)$/i.test(lower))
        return "tool";
    if (/[-_](?:db|cache|queue|broker|store)$/i.test(lower))
        return "service";
    // Predicate-based inference
    if (predicateHint) {
        if (predicateHint === "owner_of")
            return "person";
        if (predicateHint === "uses" || predicateHint === "depends_on")
            return "tool";
    }
    return "unknown";
}
function latinEntityMatches(text) {
    const matches = text.match(/\b[A-Za-z][A-Za-z0-9_.-]{1,}(?:\s+[A-Za-z][A-Za-z0-9_.-]{1,}){0,3}/g) ?? [];
...
function taggedEntityMatches(text) {
    return [
        ...text.matchAll(/(?:project|repo|package|module|项目|仓库|包|模块)\s+([A-Za-z0-9_.\-\p{Script=Han}][A-Za-z0-9_.\-\p{Script=Han}\s]{1,58})/giu),
    ]
        .map((match) => {
...
export function normalizeGraphRelationType(value) {
...
function looksLikePersonName(value) {
    ...
    return (/^[A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2}$/u.test(trimmed) ||
 ...
 */
export function parseAllRelations(text) {
    const results = [];
    const seen = new Set();
...
export function inferEntityNames(text) {
...
function anchorSpecificity(name) {
    ...
    return (Math.min(0.52, compact.length / 18) +
...
export function wantsHistoricalFacts(query) {
    if (/(?:change over time|changed over time|how did my .* change|did my .* change over time)/iu.test(query)) {
        return true;
    }
...
export function wantsCurrentFactualSnapshot(query) {
    if (/(?:我现在在做什么|我刚才在做什么|当前任务|下一步|blocker|卡点|卡在哪|working on|next step|what am i doing|what was i doing)/iu.test(query)) {
        return false;
    }
    ...
    if (/(?:现在仍然有效|当前有效|目前有效|仍然有效的信息|现在有哪些有效|最终结论|现有结论|当前结论|现在用的什么|当前用的什么|当前技术栈|still valid|still true|current valid|current info|final conclusion|current conclusion)/iu.test(query)) {
        return true;
    }
    return (prototypeSimilarity(query, [
...
function inferCorrectionTargetKind(params) {
    ...
    if (/(?:版本|组件|技术栈|stack|component|version|launch date|发布日期|上线时间)/iu.test(params.text)) {
        return "project_profile";
    }
    return "unknown";
}
...
export function analyzeCorrectionHint(params) {
    ...
    const timeCorrectionTimeframe = /(?:不是|而不是)/u.test(text) &&
        extractTimeHints(text).length >= 1 &&
        /(今年|去年|前年|this year|last year|next year|公测|试运行|内测|上线)/iu.test(text);
    const hasChangeCue = Boolean(explicitReplacement || switched || negatedCurrent || explicitPrior) ||
    ...
    const summaryRestatementOnly = SUMMARY_REQUEST_PATTERN.test(text) &&
        !explicitCurrentAuthority &&
        !switched &&
        !negatedCurrent &&
    ...
    const timeframe = compareTimeframe
        ? "compare"
    ...
    const nextValue = priorValue && rawNextValue && normalizeText(priorValue) === normalizeText(rawNextValue)
        ? ""
        : rawNextValue;
...
export function analyzeRecallQueryShape(query) {
    const anchors = extractQueryAnchors(query);
    const currentCue = wantsCurrentFactualSnapshot(query) ||
        CURRENT_SNAPSHOT_QUERY_PATTERN.test(query) ||
        /(?:当前配置|当前设定|当前状态|现在仍然有效的设定|现在仍然有效的信息|当前有哪些|目前有哪些|哪些仍然有效)/iu.test(query);
    ...
    const compare = COMPARISON_CORRECTION_PATTERN.test(query) ||
        explicitCompare ||
        ((current || historical) &&
            /(?:与当前区分|区分当前和历史|不是当前|而不是当前|still valid|current vs history)/iu.test(query));
    const timeframe = compare
        ? "compare"
        : historical && !current
            ? "historical"
            : current
                ...
                : historical
                    ? "historical"
                    : "timeless";
    const granularity = EXACT_DETAIL_QUERY_PATTERN.test(query)
        ? "exact_detail"
        : "summary";
    ...
    if (isDeicticWorkflowReferenceQuery(query) || WORKFLOW_CONTEXT_QUERY_PATTERN.test(query)) {
        ...
        evidenceNeed =
            /(?:发生|时间线|history|timeline|event|后来|公测|上线|时间|什么时候|哪次|最早|历史信息|旧称|别名|代号)/iu.test(query)
                ? "event_history"
                : "factual_history";
    }
...
export function wantsProjectProfileSnapshot(query) {
...
export function isDeicticWorkflowReferenceQuery(query) {
    return WORKFLOW_DEICTIC_REFERENCE_PATTERN.test(query);
}
export function isBroadTemporalQuery(query) {
    if (!/(?:happened|recent|history|historical|timeline|event|events|before|after|earlier|previous|old|发生|最近|历史|事件|时间线|之前|以前|后来|当时|曾经|旧版|最早)/iu.test(query)) {
        return false;
    }
    return (prototypeSimilarity(query, [
        "what happened",
...
export function predicateHint(query) {
    const relation = parseRelation(query);
    if (relation) {
...
export function semanticRoleHint(text) {
...
export function normalizedEntityId(name, type = "unknown") {
