...
function surfaceSimilarityFromNormalized(normalizedLeft, normalizedRight) {
    ...
    const containmentScore = shorterLength >= 24 &&
...
function classifyLabelFamily(label) {
    ...
    if (normalized === "workflow.current_task" ||
...
function structuredSimilarity(left, right) {
    ...
    const familyBonus = left.labelFamily &&
        right.labelFamily &&
        left.labelFamily === right.labelFamily &&
        Math.max(valueScore, crossScore, labelScore) >= 0.14
        ? 0.16
        : 0;
    const shortStructuredBoost = Math.min(left.raw.length, right.raw.length) <= 32 &&
        left.value &&
        right.value &&
...
export function ngramSimilarity(left, right, n = 3) {
...
export function basicSemanticSimilarity(left, right) {
