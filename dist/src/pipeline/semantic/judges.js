import { cleanProjectName, isDeicticWorkflowReferenceQuery, isLowValueChatter, isQuestionLike, looksLikeProjectDescriptor, parseAllRelations, parseRelation, prototypeSimilarity, stripLead, trimCapturedValue, wantsProjectProfileSnapshot, } from "./heuristics.js";
...
function confidenceFromScore(score, base = 0.42) {
...
function hasResponseTargetContext(text) {
    return (/\b(?:answer|response|reply|output|format|style|tone|explanation)\b/iu.test(text) ||
...
function languagePreference(text) {
    ...
    if (/(?:纯中文|只用中文|就用中文|就纯中文|只要中文|默认中文|中文回答|answer in chinese)/iu.test(stripped)) {
        return {
    ...
    if (/(?:纯英文|只用英文|就用英文|就纯英文|只要英文|默认英文|英文回答|english(?:-only)? answers?|answer in english)/iu.test(stripped)) {
        return {
            predicate: "prefers_language",
...
function codingPreference(text) {
    ...
    if (directiveContext &&
        /\b(?:spaces?\s+(?:not|instead|over)\s+tabs?|用\s*空格|空格缩进|[24]\s*spaces?)\b/iu.test(stripped)) {
        return {
            predicate: "prefers_indentation",
            object: "spaces",
            confidence: 0.76,
...
function experientialStylePreference(text) {
    ...
    const positiveReceptionCue = /(?:比较|更)?容易(?:跟上|理解|消化|记住)|好(?:跟上|理解)|works for me|easier to follow|helps me understand/iu;
    const followThisCue = /(?:按|照).{0,8}(?:这个|这种).{0,8}(?:来|继续|展开)/u;
    if ((referentialStyleCue.test(stripped) && positiveReceptionCue.test(stripped)) ||
...
export function judgePreferenceSignal(text) {
    if (!text.trim() || isLowValueChatter(text)) {
        return null;
    }
    return (outputOrderPreference(text) ??
...
export function judgeWorkflowState(text) {
    const stripped = stripLead(text);
    if (!stripped ||
    ...
    if (project?.[1]) {
        ...
        if (projectName) {
            if (!looksLikeProjectDescriptor(projectName) &&
...
export function judgeRelationHint(text) {
...
export function judgeAllRelationHints(text) {
...
export function judgeQueryRoute(query) {
    ...
    const scores = {
        workflow: prototypeSimilarity(query, [...ROUTE_PROTOTYPES.workflow]) + (cueMatches.workflow ? 0.24 : 0),
        factual: prototypeSimilarity(query, [...ROUTE_PROTOTYPES.factual]) + (cueMatches.factual ? 0.24 : 0),
        explanatory: prototypeSimilarity(query, [...ROUTE_PROTOTYPES.explanatory]) +
            (cueMatches.explanatory ? 0.24 : 0),
        temporal: prototypeSimilarity(query, [...ROUTE_PROTOTYPES.temporal]) + (cueMatches.temporal ? 0.24 : 0),
    };
...
export function routeFactual(query) {
    return judgeQueryRoute(query).routeType === "factual";
}
export function routeExplanatory(query) {
    return judgeQueryRoute(query).routeType === "explanatory";
}
export function routeTemporal(query) {
